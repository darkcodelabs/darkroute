...
public API under `/api/v1/` (see [Public API](#public-api)), and two Access-gated administrative
...
[`docs/public/API.md` §1.3](docs/public/API.md); the console to try it in is
[api.darkroute.ai](https://api.darkroute.ai).
...
See [Road monitoring](docs/road-monitoring.md) for equipment layers enabled by default, source coverage and on-demand camera photos.
See the [complete data inventory](docs/data-inventory.md) for every published dataset, direct downloads, separate counts, freshness, and data that exists only on a device. Camera totals exclude monitoring equipment, agency deployments and reports.
...
| `SUBMISSION_PHOTOS`      | Pages | R2 binding for `PUT/GET /api/v1/photo`; absent → `503`, nothing stored. |
| `SUBMISSIONS_TOKEN`      | Pages | Secret. GitHub token that lets `POST /api/v1/submit` open pull requests; absent → `503`. |
| `SUBMISSIONS_REPO`       | Pages | Optional; where submissions land. Default `darkcodelabs/darkroute`. |
| `NOMINATIM_URL`, `VALHALLA_URL` | Pages | Optional geocoder / router overrides for `/api/v1/place` and `/api/v1/route`. |
...
in IndexedDB. Network privacy is not absolute: a camera-tile request reveals a roughly 15 km square,
PMTiles range requests to the tile host reveal the viewport, and a sequence of requests reveals more
than any one of them. The full accounting is in
[`docs/public/THREAT-MODEL.md`](docs/public/THREAT-MODEL.md).

**Search runs on the phone.** Typing in the search bar matches against the cameras already cached on
the device - street, cross street, operator, id and a landmark index, so "home depot" finds the
readers around one. There is no autocomplete request, no geocoder call and no search-as-you-type;
`SearchBar.test.tsx` spies on the global `fetch` and asserts it is never called through a fully typed
query, because an autocomplete is a keystroke logger you have agreed to.

**Routing is one press, to our own endpoint.** The phone holds cameras, not addresses, so looking a
place up and planning a route are real network requests - and each is a key somebody presses,
labelled with what it sends. `GET /api/v1/route` carries your position, your destination and the
readers to avoid; it is not sent on a timer, not on movement, and not because you set a destination.
It goes to `darkroute.ai`, which asks OpenStreetMap's router on your behalf, so the router is handed
a pair of coordinates by a server instead of by a person with an IP address. The response is
`no-store`. Which readers are on the returned line is then worked out **on the phone**, so nothing is
ever told which cameras you cared about. The old "route around all 9" handed your origin and
destination to Google Maps as via-points; that is gone, and each reader is now a polygon the router
may not enter.
...
The design is finished; this repository implements it instead of redesigning it. The historical
...
set and its tests are the public, reviewable authority for what ships.
...
`/* GAP: slug */`, and record the decision in the commit that ships it. Never guess, never
extrapolate, never borrow a value from another project.
