 ...
 *
 * A WAY IS ADMITTED, AND IT IS ADMITTED AS A POINT.
 *
 * The pinned capture transform already reduces a qualifying way to a single
 * Point at the mean of its own member nodes -- the members it retained from
 * `>; out skel qt;` -- so by the time a feature reaches here a way and a node
 * are the same shape and every test below is the same test. What differs is
 * one thing only: which OSM namespace answers for the id, which
 * `cameraIdFor` writes into the id itself.
 *
 * Anything that is neither is refused rather than guessed at. A relation has
 * no geometry the capture retains and there is nothing to average.
 */
export function capturedFeatureToRecord(feature) {
  const props = feature?.properties ?? {};
  if (props.osmType !== 'node' && props.osmType !== 'way') return null;
  ...
  const directionDeg =
    typeof props.direction === 'number' && Number.isFinite(props.direction)
      ...
      : null;
  return {
    id: cameraIdFor(props.osmType, props.osmId),
    lat,
    lon,
    directionDeg,
...
function validateDirectSourceReview(review) {
  ...
  if (
    !(
      ...
      (hasExactKeys(releaseInputs, ['context', 'geofence', 'predecessor', 'tombstones']) &&
        isReleaseContextIdentity(releaseInputs.context))
    ) ||
  ...
  if (
    ...
    roleCounts.data < 1 ||
    /*
     * THE SAME RULE AS `validateResponseRecords`, AND IT HAS TO STAY THE SAME.
     *
     * A CA/MX cross-check that no areas-capable mirror would answer is recorded
     * as unobtained and contributes no response - the owner's 2026-09-07 ruling,
     * with `foreignCandidateMatches` going null rather than zero because we did
     * not look. Leaving this arm at "exactly two" meant such a capture could be
     * written and then never approved: the review path re-counts the roles here
     * and would have refused every one of them. Two neighbours remains the
     * ceiling; the floor is that the topology must account for whichever
     * subtraction responses the bundle does carry, which it does.
     */
    !Number.isSafeInteger(roleCounts.subtraction) ||
    roleCounts.subtraction < 0 ||
  ...
  if (
    ...
    transformation.wayCentroids > transformation.outputElements + transformation.tombstonesBlocked ||
...
export function assertVersionedSourceRecords(cameras, label = 'source archive') {
  for (const camera of cameras) {
    if (
      ...
      throw new Error(`${label} contains an invalid, unversioned, or untimestamped OSM element`);
    }
  }
  return cameras;
}
...
export function toOverpassElements(cameras) {
  ...
  return [...cameras]
    ...
    .map((camera) => {
      ...
      // The element's OWN namespace and number, read back out of the id that
      // recorded them. Emitting `node` for a way here would hand the release
      // builder an id it would resolve against the wrong OSM namespace, which
      // is the exact confusion the namespaced id exists to prevent.
      const ref = parseCameraId(camera.id);
      return {
        type: ref.type,
        id: ref.osmId,
        lat: camera.lat,
 ...
 *
 * A QUALIFYING WAY IS CARRIED, AS A POINT AT ITS OWN CENTROID.
 *
 * This function used to drop every feature whose `osmType` was not `node`, and
 * that was not a filter -- it was eighteen live Flock, Motorola and LVT readers
 * in fourteen states falling out of a counter-surveillance dataset silently,
 * because a volunteer had drawn the reader onto the road it watches instead of
 * dropping a pin beside it. The tag test they pass is the same tag test a node
 * passes; the capture's own pinned transform already reduced each of them to a
 * single Point at the mean of its retained member nodes. There was nothing to
 * decide except whether to keep them.
 *
 * A dropped way is worse than an absent one, because a node-only prover then
 * resolves its id in the node namespace, finds a deleted node or an untagged
 * vertex or nothing at all, and reports the camera as PROVABLY REMOVED. The
 * namespaced id (`osm:w<id>`) is what stops that: it says which namespace the
 * next question belongs in.
 *
 * `excludedUnsupportedElements` is what remains of the old exclusion -- an
 * element type this pipeline genuinely cannot place, which today means a
 * relation. It is not expected to be non-zero, and it counts rather than
 * throws so a capture that starts returning one is visible in the receipt
...
export function transformCapturedCollection(collection, tombstones, countyIndex) {
  ...
  let excludedUnsupportedElements = 0;
  let wayCentroids = 0;
  const sourceRecords = [];
  for (const feature of collection.features) {
    const osmType = feature?.properties?.osmType;
    if (osmType !== 'node' && osmType !== 'way') {
      excludedUnsupportedElements += 1;
      continue;
    }
    const capturedCamera = capturedFeatureToRecord(feature);
    if (capturedCamera === null) {
      throw new Error('direct capture element did not produce a camera record');
    }
...
async function main() {
  ...
  console.log(
    `  ${String(transformation.sourceFeatures)} decoded; ` +
      `${String(transformation.excludedUnsupportedElements)} unsupported elements and ` +
      `${String(transformation.excludedTerritory)} points outside 50 states/DC/PR excluded; ` +
      `${String(extracted)} territorial cameras ` +
      `(${String(transformation.wayCentroids)} of them way centroids); ` +
      `${String(cameras.length)} after the tombstone floor`,
  );
