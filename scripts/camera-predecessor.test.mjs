...
describe('type-less predecessor ids answered by the way namespace', () => {
  /*
   * The predecessor inventory was written by a publisher with no field for
   * "and this one is a way", so `osm:9232283` in a legacy tile is the SAME
   * OpenStreetMap element the current pipeline publishes as `osm:w9232283`.
   * Reading the two as different ids reports a camera that is still standing
   * as one this baseline lost, and the cutover is then asked to write a
   * removal that never happened.
   */
  const evidence = evidenceWith(['osm:9232283', 'osm:10', 'osm:1516934727']);

  it('treats a way-namespaced live id as covering its type-less predecessor id', () => {
    const live = new Set(['osm:10', 'osm:w9232283', 'osm:w1516934727']);
    assert.equal(predecessorIdIsCovered('osm:9232283', live), true);
    assert.equal(predecessorIdIsCovered('osm:1516934727', live), true);
    assert.equal(predecessorIdIsCovered('osm:10', live), true);
    assert.deepEqual(
      predecessorIdsMissingFromLiveSet(evidence, [...live]),
      [],
    );
    assert.deepEqual(uncoveredPredecessorIds(evidence, [...live], []), []);
    assert.deepEqual(wayAliasedPredecessorIds(evidence, [...live]), [
      'osm:1516934727',
      'osm:9232283',
    ]);
    assert.equal(assertPredecessorCoverage(evidence, [...live], []), evidence);
  });

  it('still reports a predecessor id neither namespace publishes', () => {
    assert.deepEqual(predecessorIdsMissingFromLiveSet(evidence, ['osm:w9232283', 'osm:10']), [
      'osm:1516934727',
    ]);
  });

  it('never reinterprets a stated way id as a node id', () => {
    // The alias is one-directional. A generation published under the
    // namespaced scheme SAYS which namespace it means, and a stated namespace
    // is not up for reinterpretation by a later prover.
    const stated = evidenceWith(['osm:w9232283']);
    assert.deepEqual(predecessorIdsMissingFromLiveSet(stated, ['osm:9232283']), [
      'osm:w9232283',
    ]);
    assert.deepEqual(predecessorIdsMissingFromLiveSet(stated, ['osm:w9232283']), []);
  });
});
