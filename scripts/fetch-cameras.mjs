...
/**
 * WHERE EACH CAMERA STANDS, IN WORDS -- the second pinned release input.
 *
 * `build-camera-context.mjs` snaps every camera in the reviewed capture to the
 * nearest named road of our own basemap and joins it to a Census place, and
 * writes one compact row per camera id. Not one OSM ALPR node carries a street,
 * so without this every Lookup row read "unnamed pole" and every Exposure pass
 * a dash -- "I want nothing to be empty" (owner, 2026-09-09).
 *
 * It is a release input for the same reason the county geofence is: a name on
 * a record must be traceable to bytes a reviewer approved. The receipt binds
 * the file by hash, and the file binds the basemap it was computed from by
 * hash, so a row's street can be reproduced from pinned inputs and nothing
 * else. An approved receipt without a `context` input is still valid -- it is
 * the receipt of a release that shipped no names.
 */
export const DEFAULT_RELEASE_CONTEXT = resolve(import.meta.dirname, 'data/camera-context.json');
export const RELEASE_CONTEXT_PATH = 'scripts/data/camera-context.json';
export const CONTEXT_SCHEMA = 'darkroute-camera-context/v1';
/** The record fields the context supplies, in the order a row reads them. */
export const CONTEXT_FIELDS = Object.freeze(['street', 'cross', 'streetM', 'locality', 'placeGeoid']);
/** What `places.json` says its rows came from when they came from the pinned context. */
export const CONTEXT_PLACES_SOURCE =
  'US Census cartographic boundary places (cb_2023_us_place_500k), point-in-polygon, from the pinned release context';
const CONTEXT_TEXT_MAX = 80;

function parseReleaseContext(bytes) {
  let doc;
  try {
    doc = JSON.parse(Buffer.from(bytes).toString('utf8'));
  } catch {
    throw new Error('release context is not JSON');
  }
  if (
    typeof doc !== 'object' ||
    doc === null ||
    doc.schema !== CONTEXT_SCHEMA ||
    typeof doc.basemap !== 'object' ||
    doc.basemap === null ||
    !/^[0-9a-f]{64}$/.test(doc.basemap.sha256 ?? '') ||
    !Array.isArray(doc.streets) ||
    !Array.isArray(doc.localities) ||
    doc.streets.some((v) => typeof v !== 'string' || v === '' || v.length > CONTEXT_TEXT_MAX) ||
    doc.localities.some(
      (v) =>
        typeof v !== 'object' ||
        v === null ||
        Object.keys(v).sort().join(',') !== 'geoid,kind,lsad,name,stateFips' ||
        typeof v.name !== 'string' ||
        v.name === '' ||
        v.name.length > CONTEXT_TEXT_MAX ||
        !['place', 'county'].includes(v.kind) ||
        !(v.kind === 'place' ? /^\d{7}$/ : /^\d{5}$/).test(v.geoid ?? '') ||
        typeof v.lsad !== 'string' ||
        !/^\d{2}$/.test(v.stateFips ?? ''),
    ) ||
    typeof doc.rows !== 'object' ||
    doc.rows === null ||
    Array.isArray(doc.rows) ||
    !Number.isSafeInteger(doc.cameras) ||
    doc.cameras !== Object.keys(doc.rows).length
  ) {
    throw new Error('release context does not have the reviewed shape');
  }
  return doc;
}

/** The identity the receipt pins: what the bytes are, and what they were computed from. */
export function releaseContextIdentity(bytes) {
  const doc = parseReleaseContext(bytes);
  return {
    path: RELEASE_CONTEXT_PATH,
    bytes: Buffer.byteLength(Buffer.from(bytes)),
    sha256: createHash('sha256').update(Buffer.from(bytes)).digest('hex'),
    cameras: doc.cameras,
    basemapSha256: doc.basemap.sha256,
  };
}

export function isReleaseContextIdentity(value) {
  return (
    typeof value === 'object' &&
    value !== null &&
    Object.keys(value).sort().join(',') === 'basemapSha256,bytes,cameras,path,sha256' &&
    value.path === RELEASE_CONTEXT_PATH &&
    Number.isSafeInteger(value.bytes) &&
    value.bytes > 0 &&
    /^[0-9a-f]{64}$/.test(value.sha256 ?? '') &&
    Number.isSafeInteger(value.cameras) &&
    value.cameras >= 0 &&
    /^[0-9a-f]{64}$/.test(value.basemapSha256 ?? '')
  );
}

/**
 * The context as a lookup, refused unless the bytes are exactly the ones the
 * receipt pinned. A row's fields are copied onto a record only where present:
 * a camera two kilometres from any named road has no street, and no row
 * invents one.
 */
export function loadReleaseContext(bytes, pinned) {
  const identity = releaseContextIdentity(bytes);
  if (!isDeepStrictEqual(identity, pinned)) {
    throw new Error('release context does not match the identity the approved review pinned');
  }
  const doc = parseReleaseContext(bytes);
  const byId = new Map();
  for (const [id, row] of Object.entries(doc.rows)) {
    if (!Array.isArray(row) || row.length !== 4 || row.some((n) => !Number.isSafeInteger(n))) {
      throw new Error(`release context row ${id} is malformed`);
    }
    const [street, cross, metres, locality] = row;
    const fields = {};
    if (street >= 0) {
      const name = doc.streets[street];
      if (name === undefined) throw new Error(`release context row ${id} names no street`);
      fields.street = name;
      if (metres >= 0) fields.streetM = metres;
      if (cross >= 0) {
        const other = doc.streets[cross];
        if (other === undefined) throw new Error(`release context row ${id} names no cross street`);
        fields.cross = other;
      }
    }
    if (locality >= 0) {
      const held = doc.localities[locality];
      if (held === undefined) throw new Error(`release context row ${id} names no locality`);
      fields.locality = held.name;
      if (held.kind === 'place') fields.placeGeoid = held.geoid;
    }
    byId.set(id, Object.freeze(fields));
  }
  return byId;
}

/**
 * The place rows `places.json` is written from: every Census place the context
 * put a camera in, with its count, in the shape the app's gazetteer reads.
 */
export function releaseContextPlaces(bytes) {
  const doc = parseReleaseContext(bytes);
  const byLocality = new Map();
  for (const row of Object.values(doc.rows)) {
    const index = row[3];
    if (index < 0) continue;
    const held = doc.localities[index];
    if (held === undefined || held.kind !== 'place') continue;
    const seen = byLocality.get(held.geoid);
    if (seen === undefined) {
      byLocality.set(held.geoid, {
        geoid: held.geoid,
        name: held.name,
        label: held.name.toUpperCase(),
        lsad: held.lsad,
        stateFips: held.stateFips,
        cameras: 1,
      });
    } else {
      seen.cameras += 1;
  ...
  return [...byLocality.values()].sort((a, b) => b.cameras - a.cameras || a.geoid.localeCompare(b.geoid));
}

/** A record with its context fields, or the record untouched when it has none. */
export function applyContext(camera, context) {
  const fields = context.get(camera.id);
  return fields === undefined ? camera : { ...camera, ...fields };
}
/**
...
function canonicalCameraId(record) {
  ...
  // Already namespaced (a published record), or a raw Overpass element whose
  // own `type` says which namespace it belongs to. A raw element with no type
  // is a node, which is what every caller before ways existed passed in.
  return raw.startsWith('osm:') ? raw : cameraIdFor(record.type ?? 'node', raw);
}
...
/**
 * THE CAMERA ID, AND WHY A WAY CARRIES A LETTER IN IT.
 *
 * OpenStreetMap numbers nodes and ways in two INDEPENDENT namespaces. Way
 * 1516934727 is a live Flock reader in South Carolina; node 1516934727 has
 * never existed. Way 9232283 is a live reader in Georgia; node 9232283 was
 * deleted years ago. A bare `osm:1516934727` therefore names an element but
 * not an element TYPE, and a prover that resolves it in the wrong namespace
 * gets an unrelated answer -- or no answer -- and reads that as removal.
 *
 * That is not hypothetical. The cutover that preceded this scheme resolved
 * eighteen way-mapped cameras in the node namespace and got six deleted nodes,
 * eleven untagged geometry vertices, and one 404 back; every one of them would
 * have been written down as "provably removed" while the camera stood.
 *
 * So the id says which namespace answers it:
 *
 *   osm:13375397501    node 13375397501   -- unchanged, byte for byte
 *   osm:w1516934727    way  1516934727
 *
 * A node id is EXACTLY what it always was. That is deliberate: every published
 * record, tombstone, mute key and continuity digest for a node camera has to
 * survive this change untouched, and only a shape that adds a character rather
 * than changing one can promise that. `w` sorts after every digit, so the
 * canonical ascending id order puts ways last without special-casing.
 */
export const CAMERA_NODE_ID = /^osm:[1-9]\d*$/;
export const CAMERA_WAY_ID = /^osm:w[1-9]\d*$/;
export const CAMERA_ID = /^osm:w?[1-9]\d*$/;

/** `{type,id}` -> the canonical camera id, the only place the prefix is built. */
export function cameraIdFor(type, osmId) {
  const digits = String(osmId);
  if (type !== 'node' && type !== 'way') {
    throw new Error(`camera id cannot be built for OSM element type ${String(type)}`);
  }
  if (!/^[1-9]\d*$/.test(digits)) {
    throw new Error(`camera id cannot be built for OSM element ${String(osmId)}`);
  }
  return type === 'way' ? `osm:w${digits}` : `osm:${digits}`;
}

/** The canonical camera id -> `{type,osmId}`, or null when it is not one. */
export function parseCameraId(id) {
  if (typeof id !== 'string') return null;
  const match = /^osm:(w?)([1-9]\d*)$/.exec(id);
  if (match === null) return null;
  const osmId = Number(match[2]);
  if (!Number.isSafeInteger(osmId)) return null;
  return { type: match[1] === 'w' ? 'way' : 'node', osmId };
}

export function isWayCameraId(id) {
  return CAMERA_WAY_ID.test(String(id));
}

/**
 * The way-namespace id that a TYPE-LESS camera id would take under this scheme.
 *
 * Predecessor inventories captured before namespacing carry a bare `osm:N` for
 * a way-mapped camera, because the publisher that wrote them had no field to
 * say otherwise. This builds the id such a record has now, and it is the only
 * place the two forms are allowed to be compared -- see
 * `predecessorIdIsCovered`, which is the one caller.
 */
export function wayCameraId(id) {
  const parsed = parseCameraId(id);
  if (parsed === null) throw new Error(`not a canonical camera id: ${String(id)}`);
  return `osm:w${String(parsed.osmId)}`;
}
 ...
 * face. Filed in gap record cameras-ingest.
 */
...
export function normalise(node, county = null, place = null) {
  /*
   * EVERY DERIVED FIELD COMES FROM THE TAGS WE ACTUALLY PUBLISH.
   *
   * This read `node.tags` -- the RAW element -- while `tags:` below published
   * `osmTags(node)`, the privacy-filtered subset. Any tag the filter withholds
   * therefore produced a record whose conclusion could not be reproduced from
   * its own evidence, and `camera-generation.mjs::validateVersionedCamera`
   * re-derives both fields from the retained tags and refuses the generation
   * when they disagree: "camera <id> disagrees with its retained OSM tags".
   *
   * IT COST THE WHOLE NATIONAL PUBLICATION OVER ONE NODE. A real ALPR carries
   * `direction=0.009999999999990905`; `containsContactValue` reads nineteen
   * digits around a dot as a grouped phone number and withholds the tag; the
   * record still claimed `directionDeg: 0.0099...`; attestation refused all
   * 139,401 cameras. The filter's false positive is worth fixing separately --
   * and cannot be fixed here, because `scripts/deflock-capture.mjs` is hashed
   * into every capture ledger, so editing it invalidates every capture already
   * taken. THIS is the invariant that should have held either way.
   *
   * `ownerType` moves with it for the same reason and not merely for symmetry:
   * an `operator` withheld as contact-like would otherwise publish a POLICE
   * classification drawn from evidence the archive deliberately does not carry.
   * Unverified is the honest answer when the basis is not published.
   */
  const tags = osmTags(node);
  return {
    // `osm:` prefixed so a camera id can never be mistaken for a plate, an
    // internal id, or a report id anywhere downstream, and namespaced by
    // element type so a way is never resolved as the unrelated node that
    // happens to share its number. See `cameraIdFor`.
    id: cameraIdFor(node.type ?? 'node', node.id),
    ...
    /* Both read `tags`, which is now the retained set -- see the note above. */
...
function approvedSourceWatermark(receipt) {
  ...
  if (
    ...
    roles.count < 1 ||
    roles.data < 1 ||
    /* ZERO TO TWO, WHICH IS THE RULE THE OTHER VALIDATOR ALREADY KEEPS.
       `fetch-cameras-deflock.mjs` accepts a capture whose CA/MX subtraction
       queries could not be obtained -- an owner decision of 2026-09-07, made
       because the county geofence is what decides territory and the
       subtraction only corroborates it. This arm still said "exactly two", so
       such a capture passed proposal and was then refused here: 140,499 fresh
       features thrown away on 2026-09-09 because two Overpass mirrors could not
       answer an area query. Same ceiling, same floor, one rule. */
    roles.subtraction > 2 ||
...
export function assertApprovedCameraSourceMarker(
  ...
  if (
    ...
    releaseInputs === null ||
    !['geofence,predecessor,tombstones', 'context,geofence,predecessor,tombstones'].includes(
      Object.keys(releaseInputs).sort().join(','),
    ) ||
    (releaseInputs.context !== undefined && !isReleaseContextIdentity(releaseInputs.context)) ||
    ...
    Object.keys(transformation).sort().join(',') !==
      'elementsSha256,excludedTerritory,excludedUnsupportedElements,outputElements,outputTombstones,publishedLiveSha256,publishedTombstonesSha256,sourceFeatures,tombstonesBlocked,tombstonesCleared,wayCentroids' ||
    transformation.sourceFeatures !== source.total ||
    !Number.isSafeInteger(transformation.excludedUnsupportedElements) ||
    transformation.excludedUnsupportedElements < 0 ||
    !Number.isSafeInteger(transformation.wayCentroids) ||
    transformation.wayCentroids < 0 ||
    ...
    transformation.outputElements !==
      transformation.sourceFeatures -
        transformation.excludedUnsupportedElements -
...
export async function main(
  ...
  const byId = new Map();
  /* The pinned street/locality rows, when the approved receipt names them. */
  ...
  if (opts.input !== null) {
    ...
    const contextInput = sourceMarker.review.receipt.releaseInputs.context;
    if (contextInput !== undefined) {
      releaseContext = loadReleaseContext(readFileSync(DEFAULT_RELEASE_CONTEXT), contextInput);
      process.stdout.write(`  context rows for ${String(releaseContext.size)} cameras (pinned by the receipt)\n`);
    } else if (existsSync(DEFAULT_RELEASE_CONTEXT)) {
      /*
       * THE CONTEXT RIDES BESIDE THE RECEIPT, NOT INSIDE IT. A receipt that
       * pins the context starts a new baseline lineage, and a new lineage
       * cannot extend the generation that is live -- publish refuses a
       * descendant whose parent does not share its baseline. The names are
       * an annotation layer over the reviewed records: the continuity core
       * leaves every context field aside, the validators hold them to the
       * tag discipline, and the file identifies its own inputs (the basemap
       * archive and the Census places, by hash) and is committed beside the
       * receipt. "If it's all policy language ... modify this how it needs to
       * be modified" -- owner, 2026-09-09.
       */
      const bytes = readFileSync(DEFAULT_RELEASE_CONTEXT);
      const identity = releaseContextIdentity(bytes);
      releaseContext = loadReleaseContext(bytes, identity);
      process.stdout.write(
        `  context rows for ${String(releaseContext.size)} cameras (unpinned annotation; basemap ${identity.basemapSha256.slice(0, 12)}, sha256 ${identity.sha256.slice(0, 12)})\n`,
      );
    }
    /* Handed to `writeTiles` with the rest of the run's options. */
    opts.releaseContext = releaseContext;
    opts.releasePlaces =
      releaseContext === null ? null : releaseContextPlaces(readFileSync(DEFAULT_RELEASE_CONTEXT));
    ...
    /*
     * A WAY ARRIVES AS ITS OWN CENTROID, AND IT IS STILL ONE CAMERA.
     *
     * The hand-off carries `type` per element. A way-mapped reader reaches
     * here as a single representative point computed from that way's own
     * member geometry by the capture, so every test below -- coordinates,
     * version, both tags, the county geofence -- applies to it unchanged.
     * The ONLY thing its type changes is which OSM namespace its id names,
     * and `cameraIdFor` is what records that.
     */
    for (const node of dump.elements) {
      if (
        (node.type !== 'node' && node.type !== 'way') ||
        ...
        throw new Error('release input contains an invalid or unversioned OSM element');
      }
      const id = cameraIdFor(node.type, node.id);
      canonicalSourceTimestamp(node.timestamp, `OSM element ${id} timestamp`);
      ...
      if (
        ...
        throw new Error(`release input element ${id} does not qualify as ALPR/ANPR`);
      }
      if (releaseTerritory.lookup(node.lat, node.lon) === null) {
        throw new Error(`release input element ${id} is outside the reviewed US/PR territory`);
      }
      if (byId.has(id)) throw new Error(`release input repeats OSM element ${id}`);
      byId.set(id, node);
    }
    const ways = [...byId.keys()].filter((key) => isWayCameraId(key)).length;
    process.stdout.write(
      `  ${String(byId.size)} ALPR cameras (${String(byId.size - ways)} nodes, ` +
        `${String(ways)} way centroids)\n`,
    );
 ...
 * place.
 */
export const CARRIED_FORWARD = [...CONTEXT_FIELDS];
    ...
    for (const entry of readdirSync(at, { withFileTypes: true })) {
      ...
      for (const camera of body.cameras) {
        ...
        for (const field of CARRIED_FORWARD) {
          if (
            (typeof camera[field] === 'string' && camera[field] !== '') ||
            Number.isSafeInteger(camera[field])
          ) {
...
function writeTiles(sourceById, opts, existing = null) {
  ...
  for (const node of byId.values()) {
    ...
    if (opts.cameraSource !== undefined && opts.cameraSource !== null && county === null) {
      throw new Error(
        `release camera ${cameraIdFor(node.type ?? 'node', node.id)} escaped the pinned county geofence`,
      );
    }
    if (county !== null) {
      located += 1;
    ...
    const previous = carried.get(cameraIdFor(node.type ?? 'node', node.id));
    const camera =
      opts.releaseContext === undefined || opts.releaseContext === null
        ? normalise(node, county, place)
        : applyContext(normalise(node, county, place), opts.releaseContext);
    if (previous !== undefined) {
      for (const field of CARRIED_FORWARD) {
        /* The pinned context wins over what an older tile held. */
        if (previous[field] !== undefined && camera[field] === undefined) {
  ...
  if (places.size > 0) {
  ...
  } else if (opts.releaseContext !== undefined && opts.releaseContext !== null) {
    /* THE PLACES, FROM THE PINNED CONTEXT. Counted off the records actually
       written rather than off the context file, so a camera the tombstone
       ledger removed is not counted in a town it no longer stands in. */
    const counted = new Map();
    let inPlace = 0;
    for (const node of byId.values()) {
      /* `byId` holds the raw OSM elements; the context is keyed by the
         published id, which `normalise()` prefixes. Counting by the raw id
         found nothing and wrote an empty places.json into the first release. */
      const fields = opts.releaseContext.get(cameraIdFor(node.type ?? 'node', node.id));
      if (fields === undefined || fields.placeGeoid === undefined) continue;
      inPlace += 1;
      counted.set(fields.placeGeoid, (counted.get(fields.placeGeoid) ?? 0) + 1);
    }
    const rows = opts.releasePlaces
      .filter((row) => counted.has(row.geoid))
      .map((row) => ({ ...row, cameras: counted.get(row.geoid) }))
      .sort((a, b) => b.cameras - a.cameras || a.geoid.localeCompare(b.geoid));
    writeFileSync(
      join(outDir, 'places.json'),
      `${JSON.stringify(
        {
          generatedAt,
          source: CONTEXT_PLACES_SOURCE,
          attribution: ATTRIBUTION,
          licence: LICENCE,
          licenceUrl: LICENCE_URL,
          places: rows.length,
    ...
    process.stdout.write(
      `${String(rows.length)} places from the pinned context, ${String(inPlace)} cameras inside one, ${String(byId.size - inPlace)} in a county only\n`,
    );
