...
import { ATTRIBUTION, CAMERA_ID, LICENCE, LICENCE_URL } from './fetch-cameras.mjs';
...
export function cameraOsmCore(camera) {
  // Either namespace. A way-mapped camera is as much a published record as a
  // node one, and a continuity digest that could not name it would be a digest
  // with a hole in exactly the records hardest to prove.
  if (!object(camera) || !CAMERA_ID.test(camera.id ?? '')) {
