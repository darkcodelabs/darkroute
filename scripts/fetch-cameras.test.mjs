...
const COMMITTED_REVIEW = JSON.parse(REVIEW_BYTES.toString('utf8'));
/*
 * THE COMMITTED RECEIPT IS APPROVED -- promoted by hand on 2026-09-09 -- so the
 * "explicitly unapproved" refusal has to be exercised with a receipt that says
 * so. This is the shape the receipt carried before promotion, verbatim from
 * the receipt as committed before promotion, minus the run-specific reason text.
 */
const UNAPPROVED_REVIEW_BYTES = Buffer.from(
  JSON.stringify({
    ...COMMITTED_REVIEW,
    sourceWatermark: {
      status: 'unapproved',
      reason:
        'fetch run did not retain osm3s.timestamp_osm_base for its constituent Overpass responses',
      minimumOsmBase: null,
      responseLedger: null,
    },
  }),
...
function releaseSourceFixture(elements = [], baselineTombstoneBytes = null) {
  ...
  const excludedUnsupportedElements = 0;
  const transformation = {
    sourceFeatures: total,
    excludedUnsupportedElements,
    excludedTerritory: total - excludedUnsupportedElements - elements.length,
    wayCentroids: elements.filter((element) => element.type === 'way').length,
    tombstonesBlocked: 0,
    tombstonesCleared: 0,
...
describe('the fields a refresh must not destroy', () => {
  it('names street and cross, which this script cannot recompute', () => {
    ...
    // cards, silently.
    assert.deepEqual([...CARRIED_FORWARD].sort(), ['cross', 'locality', 'placeGeoid', 'street', 'streetM']);
  });
...
describe('input snapshot provenance', () => {
  it('requires the exact pinned US/PR adapter hand-off marker', () => {
    ...
    assert.equal(COMMITTED_REVIEW.sourceWatermark.status, 'approved');
    assert.throws(
      () => assertReleaseCameraSource(valid, UNAPPROVED_REVIEW_BYTES),
      /explicitly unapproved.*osm3s/,
    );
    ...
    /* THE COMMITTED RECEIPT'S OWN NUMBERS, pinned. Generation d91bec12, built
       from capture c9f55ffe on 2026-09-09; a receipt edit that changes any of
       these without a new capture is what this catches. */
    assert.deepEqual(COMMITTED_REVIEW.expectedTransformation, {
      sourceFeatures: 140_503,
      excludedUnsupportedElements: 0,
      excludedTerritory: 890,
      wayCentroids: 18,
      tombstonesBlocked: 0,
      tombstonesCleared: 6,
      outputElements: 139_613,
      elementsSha256: 'c43f762187e64cde8dc85bf1b217a6d16a20a259b0090f9c0ed1cf420f8c3f74',
      outputTombstones: 3644,
      publishedLiveSha256: 'd2d5c2eb04a70a04871f49983bebf462c5a216ac0cf29690ddf2644a41065364',
      publishedTombstonesSha256: 'a992f8393d10b3abf58b8cab3b17d0929a30c1fbab389b33930c64bb3238d741',
    });
  });

  it('accepts a capture whose CA/MX subtraction could not be obtained, and still refuses three', () => {
    /*
     * THE OTHER VALIDATOR ALREADY ALLOWED THIS. `fetch-cameras-deflock.mjs`
     * accepts 0..2 subtraction responses -- an owner decision of 2026-09-07,
     * because the county geofence is what decides territory and the CA/MX
     * subtraction only corroborates it. This file still said "exactly two", so
     * on 2026-09-09 a fresh 140,499-feature capture passed proposal and was then
     * refused HERE, over two Overpass mirrors that could not answer an area
     * query. One rule now, with the same ceiling.
     */
    const ledger = REVIEW.sourceWatermark.responseLedger;
    const held = { roleCounts: ledger.roleCounts, responseCount: ledger.responseCount, bundle: ledger.responseBundle.responseCount };
    const withSubtractions = (subtraction) => {
      ledger.roleCounts = { count: 2, data: 2, subtraction };
      ledger.responseCount = 4 + subtraction;
      ledger.responseBundle.responseCount = 4 + subtraction;
      return releaseSourceFixture();
    };
    try {
      for (const subtraction of [0, 1, 2]) {
        const fixture = withSubtractions(subtraction);
        assert.equal(
          assertReleaseCameraSource(fixture.dump, fixture.trustedReviewBytes, TEST_BASELINE_VALIDATION),
          fixture.dump.cameraSource,
          `${String(subtraction)} subtraction responses must be accepted`,
        );
      }
      const three = withSubtractions(3);
      assert.throws(
        () => assertReleaseCameraSource(three.dump, three.trustedReviewBytes, TEST_BASELINE_VALIDATION),
        /approved review receipt/,
      );
    } finally {
      ledger.roleCounts = held.roleCounts;
      ledger.responseCount = held.responseCount;
      ledger.responseBundle.responseCount = held.bundle;
    }
  });
