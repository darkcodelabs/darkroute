...
/**
 * NODE NAMESPACE ONLY, AND THAT IS THE POINT.
 *
 * A backfilled version is proved from a `node` event inside the exact recorded
 * hourly diff. A way-namespaced id (`osm:w<id>`) has no node event to be proved
 * from, so it is refused by name here rather than silently sliced into a
 * number that would then match an unrelated node.
 */
function rawNodeId(id) {
  if (typeof id !== 'string' || !/^osm:[1-9]\d*$/.test(id)) {
    throw new Error(
      `${String(id)} is not a node-namespace camera id; ` +
        'only a node has an hourly diff event that can prove its version',
    );
  }
  return id.slice(4);
}
