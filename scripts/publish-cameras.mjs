...
/**
 * THE R2 CLIENT, AND THE ONE OPTION THAT MADE IT WORK.
 *
 * Exported so a test can assert the headers this configuration actually puts
 * on the wire. That assertion is the whole point of the function existing
 * separately: the bug it guards against is invisible in the source of
 * `putJson` and only shows up in the signed request.
 *
 * `putJson` sends its own `ContentMD5`, deliberately -- the digest is computed
 * here so the bytes that leave and the bytes that land are checked against a
 * value this script chose. From @aws-sdk/client-s3 3.729 onward the SDK ALSO
 * adds `x-amz-checksum-crc32` to every PutObject on its own, because
 * `requestChecksumCalculation` defaults to `WHEN_SUPPORTED`. This project is
 * on 3.1116.0, well past that.
 *
 * S3 tolerates the pair. R2 does not, and answers:
 *
 *   InvalidRequest: You can only specify one non-default checksum at a time
 *
 * a 400 that arrives AFTER the whole archive has validated -- 8,870 tiles and
 * 141,660 cameras, every run, then a hard stop on the first upload. Nothing
 * published between 2026-09-01 and the day this was found.
 *
 * `WHEN_REQUIRED` adds a checksum only when the operation demands one, leaving
 * the explicit ContentMD5 as the single checksum on the wire.
 *
 * Do not "fix" a recurrence by deleting ContentMD5 instead. That would also
 * stop the error, and would silently hand integrity to a default -- which is
 * not a trade this bucket's contents deserve.
 */
export function createCameraR2Client({ account, credentials, requestHandler }) {
  return new S3Client({
    region: 'auto',
...
export async function main(argv) {
  ...
  const client = createCameraR2Client({
    account,
