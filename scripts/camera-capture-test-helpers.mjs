...
export function completeTerritorialCaptureFixture() {
  ...
  const subtractions = {
    CA: { iso: 'CA', responseId: 'subtraction:CA', featureCount: 300, minimum: 300, unavailable: null },
...
/**
 * The same complete capture, with three of its cameras drawn as WAYS.
 *
 * This is not a synthetic shape. Eighteen live Flock, Motorola and LVT readers
 * in the national capture are mapped as ways rather than nodes -- a volunteer
 * tagged the road, gantry or pole line the reader watches instead of dropping
 * a separate pin -- and the release has to carry them or lose them. Each one
 * arrives exactly like this: a tagged `way` plus the untagged member nodes the
 * `>; out skel qt;` half of the data query returns, from which the pinned
 * transform computes one representative point.
 *
 * The count is deliberately unchanged: three qualifying nodes become three
 * qualifying ways, so every count probe, leaf feature count and territorial
 * total in the fixture stays exactly what it was. Only the element TYPE moves.
 */
export const FIXTURE_WAY_CAMERAS = 3;

export function captureWithWayMappedCameras() {
  const base = completeTerritorialCaptureFixture();
  const bodies = decodeResponseBundle(base.artifacts.responseBundle);
  const queryByResponse = new Map();
  for (const node of base.ledger.topology.countNodes) {
  ...
  const targetLeaf = base.ledger.topology.dataLeaves[0];
  const wayIds = [];
  const responses = new Map();
  for (const record of base.ledger.responses) {
    let body = bodies.get(record.id);
    if (record.id === targetLeaf.dataResponseId) {
      const parsed = JSON.parse(body.toString('utf8'));
      const rewritten = [];
      const members = [];
      let converted = 0;
      for (const element of parsed.elements) {
        if (element.type !== 'node' || converted >= FIXTURE_WAY_CAMERAS) {
          rewritten.push(element);
          continue;
        }
        // Two member nodes either side of where the node was, so the mean of
        // the members is the node's own point and the way lands on the same
        // tile the node did.
        const first = 9_000_000 + converted * 2;
        const second = first + 1;
        members.push(
          { type: 'node', id: first, lat: element.lat - 0.001, lon: element.lon - 0.001 },
          { type: 'node', id: second, lat: element.lat + 0.001, lon: element.lon + 0.001 },
        );
        rewritten.push({
          type: 'way',
          id: element.id,
      ...
      if (converted !== FIXTURE_WAY_CAMERAS) {
        throw new Error('way fixture could not convert its cameras');
      }
      parsed.elements = [...rewritten, ...members];
      body = retainedResponseBytes(parsed);
    }
    const query = queryByResponse.get(record.id);
    if (query === undefined || body === undefined) {
  ...
  const validated = validateCaptureArtifacts(built.ledger, {
    ledgerBytes: built.ledgerBytes,
    responseBundle: built.responseBundle,
    rawDataset: built.rawGzip,
    implementationFiles,
