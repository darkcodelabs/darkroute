...
describe('verdict', () => {
  ...
  /*
   * These once used the real allowance for the then-private public repository.
   * It opened, `PRE_RELEASE` was emptied - which is the outcome the mechanism
   * exists to force - and the tests started failing for the one reason a test
   * must never fail: the thing it described came true.
   *
   * So they test the MECHANISM against a fixture. The allowance table is data
   * that is meant to shrink to nothing; the rule that a satisfied allowance is
   * a failure has to keep being checked after it does.
   */
  const HEDGED = [{ prefix: 'https://example.invalid/hedged', why: 'a fixture, not a real gap' }];

  it('passes a declared pre-release gap', () => {
    assert.equal(verdict(HEDGED[0].prefix, 'missing', HEDGED).ok, true);
  });

  it('FAILS a declared gap that has started resolving', () => {
    // The forcing function. The day the gap closes, this gate breaks on purpose.
    const { ok, note } = verdict(HEDGED[0].prefix, 'reachable', HEDGED);
    assert.equal(ok, false);
    assert.match(note, /STALE ALLOWANCE/);
  });

  it('matches an allowance by prefix, so sub-paths inherit it', () => {
    assert.notEqual(allowanceFor(`${HEDGED[0].prefix}/docs/public/AUDITING.md`, HEDGED), null);
    assert.equal(allowanceFor('https://example.invalid/something-else', HEDGED), null);
  });

  it('has no live allowances left, because the repository it hedged is public', () => {
    // The gate's own scoreboard. If something is hedged again, this says so.
    assert.deepEqual(PRE_RELEASE, []);
    assert.equal(allowanceFor(ALLOWED), null);
  });
});
