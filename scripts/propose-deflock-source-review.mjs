...
export async function buildSourceReviewProposal({
  ...
  const review = {
    ...
    releaseInputs: {
      ...
      ...(contextBytes === null ? {} : { context: releaseContextIdentity(contextBytes) }),
...
export function validateApprovedBaselineOffline({
  ...
  releaseGeofenceIdentity(geofenceBytes);
  if (review.releaseInputs.context !== undefined) {
    if (contextBytes === null) throw new Error('approved review pins a camera context that was not supplied');
    if (!isDeepStrictEqual(releaseContextIdentity(contextBytes), review.releaseInputs.context)) {
      throw new Error('camera context does not match the approved review');
    }
  }
  const countyIndex = loadCountiesBytes(geofenceBytes);
  assertTombstoneLedgerAncestry({
...
async function main() {
  ...
  /* The context is not pinned here: see `fetch-cameras.mjs`, which applies it
     beside the receipt so the lineage the receipt names stays the live one. */
  const contextBytes = null;
