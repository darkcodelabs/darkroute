...
for (const width of WIDTHS) {
  ...
  const read = await page.evaluate(() => {
    ...
    // THE ROW THE PICTURE SITS IN. It was `.fwm-intelv1-hero` until brief 4
    // turned the full sheet into a 342px modal: the hero is gone, and the
    // picture is the right-hand column of the modal's one head row.
    const hero = document.querySelector('.fwm-intelv1-head');
  ...
  check('the picture is inside the head row', read.insideHero);
  ...
  /*
   * THE PICTURE TAKES NO GESTURE AT ALL, which is the invariant that survived
   * the redesign.
   *
   * This used to wheel over the picture and assert that the CARD scrolled: the
   * card was a full-height sheet with its own `overflow-y`, and a map that
   * swallowed a drag inside it had broken it. Brief 4's modal is 318px and
   * fits, so there is no scroll left to steal - and the real rule is the one
   * `miniMap.css` states: `interactive: false` plus `pointer-events: none`, so
   * nothing in the figure is a target for anything.
   */
  const inert = await page.evaluate(() => {
    const figure = document.querySelector('.fwm-minimap');
    return figure === null ? null : getComputedStyle(figure).pointerEvents;
  });
  check('the picture takes no gestures', inert === 'none', `pointer-events ${String(inert)}`);
