...
/**
 * The two reasons a one-time cutover may add. `cutover_reconciliation` is a
 * node proved absent, unqualified, or out of territory. `osm_out_of_scope` is
 * an id the node namespace has never held at all, proved in the way namespace
 * instead; the strict baseline is node-only, so a way it cannot carry is out
 * of that baseline's scope by construction.
 */
const CUTOVER_REASONS = new Set(['cutover_reconciliation', 'osm_out_of_scope']);

function isProvedCutoverReplacement(entry, predecessorLive) {
  return CUTOVER_REASONS.has(entry?.reason) && predecessorLive.has(entry.id);
}
...
function versionFetchBatches(entries) {
  ...
  for (const entry of entries) {
    const value = String(parseCameraId(entry.id).osmId);
...
async function fetchExactNodeVersions(entries, fetchImpl) {
  ...
  // The wire answers with element NUMBERS. Our ids also carry a namespace, so
  // a result is filed under the exact id that asked for it rather than under a
  // bare id rebuilt from the number.
  const requested = new Map(
    entries.map((entry) => [String(parseCameraId(entry.id).osmId), entry.id]),
  );
  for (const batch of versionFetchBatches(entries)) {
    ...
    for (const node of body.elements) {
      const id = requested.get(String(node?.id));
      if (
        node?.type !== 'node' ||
        id === undefined ||
        !CAMERA_NODE_ID.test(id) ||
        !expected.has(id) ||
...
function wayVersionFetchBatches(entries) {
  const batches = [];
  let current = [];
  for (const entry of entries) {
    const value = String(parseCameraId(entry.id).osmId);
    const candidate = [...current, value];
    const url = `${OSM_API_ROOT}/ways.json?ways=${candidate.join(',')}`;
    if (url.length > 7_000 && current.length > 0) {
      batches.push(current);
      current = [value];
    } else {
...
async function fetchExactWayVersions(entries, fetchImpl) {
  const found = new Map();
  const expected = new Map(entries.map((entry) => [entry.id, entry.osmVersion]));
  // Same as the node path: an `osm:w<id>` tombstone -- one inherited from a
  // predecessor that already stated its namespace -- must not come back filed
  // under the bare id, which names a different element entirely.
  const requested = new Map(
    entries.map((entry) => [String(parseCameraId(entry.id).osmId), entry.id]),
  );
  for (const batch of wayVersionFetchBatches(entries)) {
    const url = `${OSM_API_ROOT}/ways.json?ways=${batch.join(',')}`;
    const response = await fetchImpl(url, {
      headers: {
        Accept: 'application/json',
        'User-Agent': RECONCILIATION_USER_AGENT,
    ...
    try {
      ...
      throw new Error(`${url}: OSM way response is not JSON`);
    }
    if (!Array.isArray(body?.elements)) throw new Error(`${url}: OSM response has no elements`);
    for (const way of body.elements) {
      const id = requested.get(String(way?.id));
      if (
        way?.type !== 'way' ||
        id === undefined ||
        !CAMERA_ID.test(id) ||
        !expected.has(id) ||
        !Number.isSafeInteger(way.version) ||
        way.version < 1 ||
        way.version !== expected.get(id) ||
        found.has(id)
      ) {
        throw new Error(`${url}: OSM response is not the exact current way version`);
      }
      found.set(id, way);
    }
  }
  if (found.size !== expected.size) {
    const missing = [...expected.keys()].find((id) => !found.has(id));
    throw new Error(`official OSM API omitted current cutover way ${String(missing)}`);
  }
  return found;
}

/**
 * THE TAGS OF ONE OFFICIAL OSM ELEMENT, WHERE HAVING NONE IS AN ANSWER.
 *
 * The OSM API omits `tags` entirely for an element that carries none. Reading
 * an absent key as a malformed response is the exact failure this project has
 * already had once (`fix(node): accept an absence that arrives as undefined`),
 * and here it refused the single most interesting shape in the whole cutover:
 * node 10155751371 is a LIVE, visible node with valid coordinates and no tags
 * at all -- a camera a mapper stripped back to a bare geometry vertex. That is
 * a removal this ledger exists to record, and the run died on it instead.
 *
 * An absent map is no tags. A null or an array is still a malformed response
 * and still refused, because neither is a shape the API produces.
 */
export function officialElementTags(element, label) {
  const tags = element?.tags;
  if (tags === undefined) return {};
  if (typeof tags !== 'object' || tags === null || Array.isArray(tags)) {
    throw new Error(`${label} has a malformed OSM tag map`);
  }
  return tags;
}

/**
 * A way carries no coordinate of its own, so territory is judged at the mean
 * of its member nodes -- the same representative point the source capture
 * publishes for a way-mapped camera.
 */
export async function fetchWayCentroid(id, fetchImpl = fetch) {
  const url = `${OSM_API_ROOT}/way/${String(parseCameraId(id).osmId)}/full.json`;
  const response = await fetchImpl(url, {
    headers: { Accept: 'application/json', 'User-Agent': RECONCILIATION_USER_AGENT },
    redirect: 'error',
  });
  if (!response.ok) throw new Error(`${url}: HTTP ${String(response.status)}`);
  if (response.url !== url) {
  ...
  try {
    ...
    throw new Error(`${url}: OSM way geometry is not JSON`);
  }
  if (!Array.isArray(body?.elements)) throw new Error(`${url}: OSM response has no elements`);
  const seen = new Set();
  let latitude = 0;
  ...
  for (const element of body.elements) {
    ...
    if (
      ...
      throw new Error(`${url}: OSM way member node has invalid geometry`);
    }
    seen.add(element.id);
    latitude += element.lat;
    longitude += element.lon;
  }
  if (seen.size === 0) throw new Error(`${url}: OSM way has no member geometry`);
  return {
    lat: normaliseCoordinate(latitude / seen.size),
    lon: normaliseCoordinate(longitude / seen.size),
  };
}

/**
 * Re-prove one way-namespace cutover tombstone. A way that is still a
 * qualifying camera inside the pinned territory has not been removed from
 * OpenStreetMap at all -- the node-only baseline merely cannot carry it -- so
 * recording it as a removal would publish a deletion that never happened.
 */
async function assertWayIsOutOfBaselineScope(entry, way, countyIndex, fetchImpl) {
  if (way.visible === false) return;
  const tags = officialElementTags(way, `cutover way ${entry.id}`);
  if (!qualifies(tags)) return;
  const centre = await fetchWayCentroid(entry.id, fetchImpl);
  if (countyIndex.lookup(centre.lat, centre.lon) !== null) {
    throw new Error(
      `cutover way ${entry.id} is still a qualifying in-scope camera; it is not removed`,
    );
  }
}
...
export function assertCutoverReconciliationShape({
  ...
  if (
    ...
    baselineLiveIds.some((id) => !CAMERA_ID.test(id ?? '')) ||
  ...
  const predecessorLive = new Set(evidence.liveIds);
  // `predecessorIdIsCovered`, not a bare set membership: a type-less
  // predecessor id whose element is now published under its way-namespace name
  // is still published, and demanding a removal tombstone for it would record
  // a deletion that did not happen. See the note on that function.
  const expectedExtraIds = evidence.liveIds.filter((id) => !predecessorIdIsCovered(id, live));
  ...
  for (const entry of extras) {
    if (
      !CUTOVER_REASONS.has(entry.reason) ||
      !predecessorLive.has(entry.id) ||
      predecessorIdIsCovered(entry.id, live)
    ) {
...
export async function verifyCutoverReconciliation({
  ...
  // Split by namespace before proving. An `osm_out_of_scope` extra was written
  // precisely because the node namespace has never held its id, so asking the
  // node endpoint about it would 404 the whole batch and take the provable
  // node extras down with it.
  const wayExtras = extras.filter((entry) => entry.reason === 'osm_out_of_scope');
  const nodeExtras = extras.filter((entry) => entry.reason !== 'osm_out_of_scope');
  if (wayExtras.length > 0) {
    const ways = await fetchExactWayVersions(wayExtras, fetchImpl);
    for (const entry of wayExtras) {
      const way = ways.get(entry.id);
      if (way === undefined) {
        throw new Error(
          `official OSM API did not return way ${entry.id}v${String(entry.osmVersion)}`,
        );
      }
      const timestamp = canonicalSourceTimestamp(
        way.timestamp,
        `cutover way ${entry.id} timestamp`,
      );
      ...
      await assertWayIsOutOfBaselineScope(entry, way, countyIndex, fetchImpl);
    }
  }
  if (nodeExtras.length === 0) return ledger;
  const nodes = await fetchExactNodeVersions(nodeExtras, fetchImpl);
  for (const entry of nodeExtras) {
    ...
    if (node.visible !== false) {
      if (
        ...
        throw new Error(`cutover node ${entry.id} has invalid current geometry`);
      }
      if (
        qualifies(officialElementTags(node, `cutover node ${entry.id}`)) &&
