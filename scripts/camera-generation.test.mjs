...
describe('deep local camera validation', () => {
  ...
  it('accepts a way-namespaced camera and tombstone under versionsKnown', async () => {
    /*
     * A camera a mapper drew as a WAY is as published as one drawn as a node,
     * and `osm:w<id>` is how the archive says which OpenStreetMap namespace
     * answers for it. A validator that only knew `osm:<id>` would refuse the
     * whole generation rather than the one record, so the eighteen way-mapped
     * readers in the national capture could not be published at all.
     */
    const approved = approvedCameraSourceFixture();
    const root = await mkdtemp(join(tmpdir(), 'camera-generation-way-'));
    try {
      const fixture = await makeCameraFixture(root, {
        id: 'osm:w9232283',
        versionsKnown: true,
        osmVersion: 20,
        tombstones: [{ id: 'osm:w2', reason: 'osm_delete', seq: 9, osmVersion: 2 }],
        cameraSource: approved.marker,
        baseUpstream: approved.minimumOsmBase,
      });
      const generation = await readLocalGeneration(fixture.archive, fixture.stateFile, {
      ...
      const tile = JSON.parse(
        generation.entries.find((entry) => entry.key === fixture.tileKey).body.toString('utf8'),
      );
      assert.equal(tile.cameras[0].id, 'osm:w9232283');
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
