...
/*
 * THE RUN THIS TEST EXISTS FOR. On 2026-09-07 the schedule had been switched
 * off for six days. The first sync afterwards walked all 141 pending diffs
 * correctly, computed 625 tombstones, and was refused by a limit calibrated for
 * ONE HOUR of work - 500, which is about seventy hours of measured churn. The
 * breaker asked the right question with the wrong number: 625 over 141 hours is
 * BELOW the ~990 the documented 0.12%/day rate predicts for that span.
 */
test('the hourly tombstone floor does not move', () => {
  assert.equal(tombstoneCeiling(139_918, 1), 500);
  // A single-diff run is the case 500 was written for, and a routine hour that
  // wants to remove 501 cameras is still a bug.
  assert.equal(tombstoneCeiling(139_918, 0), 500);
});

test('a six-day backlog is allowed the churn six days actually produce', () => {
  const ceiling = tombstoneCeiling(139_918, 141);
  assert.ok(ceiling > 625, `141h ceiling ${ceiling} still refuses the real run`);
  // Derived, not fitted: ~7 removals an hour at the measured rate, times the
  // span, times three because real weeks are lumpier than an annual mean.
  assert.equal(ceiling, 2960);
});

test('a backlog earns absolute room, never the right to empty the archive', () => {
  // The FRACTION check is unchanged and is the real ceiling - it is applied
  // beside this one at the call site. Even a full week of backlog stays well
  // under a twentieth of the live table.
  assert.ok(tombstoneCeiling(139_918, 168) < 139_918 * 0.05);
});

test('a nonsense span does not scale the ceiling', () => {
  assert.equal(tombstoneCeiling(Number.NaN, 141), 500);
  assert.equal(tombstoneCeiling(139_918, Number.NaN), 500);
});
