...
describe('places survive a sync', () => {
  it('recounts the held places document from the records and drops empty rows', async () => {
    const { buildApprovedGazetteers } = await import('./sync-cameras.mjs');
    const { CONTEXT_PLACES_SOURCE } = await import('./fetch-cameras.mjs');
    const countyIndex = { lookup: () => ({ fips: '20091', name: 'Johnson', lsad: 'County', state: 'KS', stateFips: '20' }) };
    const records = [
      { id: 'osm:1', lat: 1, lon: 1, countyFips: '20091', placeGeoid: '2053775' },
      { id: 'osm:2', lat: 1, lon: 1, countyFips: '20091', placeGeoid: '2053775' },
      { id: 'osm:3', lat: 1, lon: 1, countyFips: '20091' },
    ];
    const held = {
      source: CONTEXT_PLACES_SOURCE,
      rows: [
        { geoid: '2053775', name: 'Overland Park', label: 'OVERLAND PARK', lsad: '25', stateFips: '20', cameras: 9 },
        { geoid: '2038900', name: 'Leawood', label: 'LEAWOOD', lsad: '25', stateFips: '20', cameras: 4 },
      ],
    };
    const { places } = buildApprovedGazetteers(records, countyIndex, '2026-09-09T00:00:00.000Z', held);
    assert.equal(places.source, CONTEXT_PLACES_SOURCE);
    assert.equal(places.places, 1);
    assert.equal(places.inPlace, 2);
    assert.equal(places.unincorporated, 1);
    assert.deepEqual(places.rows.map((r) => [r.geoid, r.cameras]), [['2053775', 2]]);
    const none = buildApprovedGazetteers(records, countyIndex, '2026-09-09T00:00:00.000Z', null);
    assert.equal(none.places.places, 0);
  });
});
