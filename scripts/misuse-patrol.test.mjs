...
describe('the GDELT retry', () => {
  ...
  it('a transient GDELT rate limit is retried rather than failing the patrol', async () => {
    ...
    // A RATE LIMIT BACKS OFF FURTHER THAN A DROPPED CONNECTION. Answering a 429
    // two seconds later is asking too often again, which is what the four
    // consecutive scheduled failures from 2026-09-03 actually did.
    assert.deepEqual(waits, [15_000, 60_000]);
  });

  it('a dropped connection keeps the short ramp - only 429 gets the long one', async () => {
    const { fetchWithRetry, gdeltBackoffMs } = await import('./misuse-patrol.mjs');
    assert.equal(gdeltBackoffMs(1, false), 2_000);
    assert.equal(gdeltBackoffMs(1, true), 15_000);
    ...
    await fetchWithRetry({
      ...
      fetchArticles: async () => {
        calls += 1;
        if (calls < 2) throw new Error('GDELT request failed: fetch failed');
        return [];
      },
    });
