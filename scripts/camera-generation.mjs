...
/**
 * THE CONTEXT FIELDS ARE NOT VERSIONED, AND ARE ALLOWED. `street`, `cross`,
 * `streetM` and `locality` come from the pinned release context -- our own
 * basemap and the Census places -- never from OSM, so replication cannot
 * version them and the record schema check leaves them aside. They are still
 * held to the same discipline as a tag: short, non-empty, and free of anything
 * that reads as a contact.
 */
const CONTEXT_TEXT_MAX = 80;
/*
 * A STREET NAME IS NOT A TAG. `containsContactValue` de-obfuscates " dot "
 * and " at " before it looks for a domain, which is right for a free-text
 * tag a mapper typed and wrong for "E Dot Stafford St", a real street in
 * Pecos, Texas, that the first attested release refused. A name that came
 * off the basemap gets the plain test: an @, a URL, or a run of digits long
 * enough to be a number to call.
 */
function looksLikeContact(value) {
  /* A run of seven digits, or three-three-four with separators, is a number to
     call; an @ with no space either side is an address to write to. "2600
     South / 1100 N" -- a Salt Lake grid address -- is eight digits in two runs
     of four, and "Cullen Boulevard @ Elgin St" is how one Houston way is
     named. Both are streets. */
  return /\S@\S|https?:\/\/|www\.|\d{7,}|\d{3}[-. ]\d{3}[-. ]\d{4}/iu.test(value);
}
function validateContextFields(camera, tileKey) {
  for (const key of ['street', 'cross', 'locality']) {
    const value = camera[key];
    if (value === undefined) continue;
    if (typeof value !== 'string' || value === '' || value.length > CONTEXT_TEXT_MAX || looksLikeContact(value)) {
      throw new Error(`${tileKey} camera ${camera.id} has an invalid ${key}`);
    }
  }
  if (camera.streetM !== undefined && (!integer(camera.streetM) || camera.streetM > 100_000)) {
    throw new Error(`${tileKey} camera ${camera.id} has an invalid streetM`);
  }
  if (camera.placeGeoid !== undefined && !/^\d{7}$/.test(camera.placeGeoid)) {
    throw new Error(`${tileKey} camera ${camera.id} has an invalid placeGeoid`);
  }
  if ((camera.cross !== undefined || camera.streetM !== undefined) && camera.street === undefined) {
    throw new Error(`${tileKey} camera ${camera.id} has a cross street or distance with no street`);
  }
}

function validateVersionedCamera(camera, county, tileKey) {
  validateContextFields(camera, tileKey);
  const { street, cross, streetM, locality, placeGeoid, ...versioned } = camera;
  void street;
  void cross;
  void streetM;
  void locality;
  void placeGeoid;
  const keys = Object.keys(versioned).sort();
...
export function validateArchiveBodies(entries, replication, options = {}) {
  ...
  for (const tile of tiles) {
    ...
    for (const camera of doc.cameras) {
      ...
      // `CAMERA_ID`, not a node-only shape: `osm:<id>` for a node and
      // `osm:w<id>` for a way carried at its centroid. What versionsKnown
      // requires is that the id STATES its OSM namespace, which both do.
      if (replication.versionsKnown && !CAMERA_ID.test(camera.id)) {
        throw new Error(
          `versionsKnown is true but live camera ${camera.id} has no canonical OSM element id`,
        );
      }
      if (replication.versionsKnown) {
  ...
  for (const tombstone of tombstones.tombstones) {
    ...
    if (replication.versionsKnown && !CAMERA_ID.test(tombstone.id)) {
      throw new Error(
        `versionsKnown is true but tombstone ${tombstone.id} has no canonical OSM element id`,
      );
    }
  ...
  for (const [key, countField] of [['places.json', 'places'], ['counties.json', 'counties']]) {
    ...
    if (replication.versionsKnown && key === 'places.json') {
      ...
      if (!isTimestamp(doc.generatedAt)) throw new Error('places.json has an invalid generatedAt');
      if (doc.source === CONTEXT_PLACES_SOURCE) {
        /* THE ENABLED FORM: rows from the pinned release context. Every row
           is a Census place with the geoid the app's gazetteer keys on, and
           the counts add up to the archive. */
        if (
          doc.places !== doc.rows.length ||
          !integer(doc.inPlace) ||
          !integer(doc.unincorporated) ||
          doc.inPlace + doc.unincorporated !== cameras ||
          doc.rows.some(
            (row) =>
              !object(row) ||
              Object.keys(row).sort().join(',') !== 'cameras,geoid,label,lsad,name,stateFips' ||
              !/^\d{7}$/.test(row.geoid ?? '') ||
              typeof row.name !== 'string' ||
              row.name === '' ||
              typeof row.label !== 'string' ||
              typeof row.lsad !== 'string' ||
              !/^\d{2}$/.test(row.stateFips ?? '') ||
              !integer(row.cameras) ||
              row.cameras < 1,
          ) ||
          doc.rows.reduce((sum, row) => sum + row.cameras, 0) !== doc.inPlace
        ) {
          throw new Error('places.json does not add up to the archive it describes');
        }
      } else if (
