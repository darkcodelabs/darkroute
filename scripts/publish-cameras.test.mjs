...
/**
 * THE HEADERS, NOT THE SOURCE.
 *
 * This bug is invisible in `putJson`: the code sets exactly one checksum and
 * looks correct. The second one is added by SDK middleware, so the only place
 * the defect is observable is the signed request -- which is why these tests
 * capture the request rather than inspecting the client's options.
 *
 * A request handler that records and throws never opens a socket, so this runs
 * offline and touches no bucket.
 */
describe('the R2 client sends exactly one checksum', () => {
  ...
  async function headersFor(client, requestHandler) {
    const body = Buffer.from('{"probe":true}', 'utf8');
    await assert.rejects(() =>
      client.send(
    ...
    return Object.keys(requestHandler.headers ?? {}).map((key) => key.toLowerCase());
  }

  const CREDENTIALS = { accessKeyId: 'AKIAPROBE', secretAccessKey: 'probe' };

  /** The client this script actually ships. */
  async function shippedHeaders() {
    const requestHandler = new CapturingHandler();
    return headersFor(
      createCameraR2Client({ account: 'probe', credentials: CREDENTIALS, requestHandler }),
      requestHandler,
    );
  }

  /**
   * A client built the DEFAULT way -- no `requestChecksumCalculation` at all.
   * Constructed directly rather than through the factory, because the factory
   * takes no such argument: routing it through there would silently be ignored
   * and the control would pass for the wrong reason. It did exactly that on the
  ...
  async function defaultHeaders() {
    ...
    return headersFor(
      new S3Client({
        region: 'auto',
        endpoint: 'https://probe.r2.cloudflarestorage.com',
  ...
  it('keeps the explicit ContentMD5 this script computes', async () => {
    const headers = await shippedHeaders();
    assert.ok(headers.includes('content-md5'), `expected content-md5 in ${headers.join(', ')}`);
  });

  it('does not let the sdk add a second, default checksum', async () => {
    const headers = await shippedHeaders();
    const extra = headers.filter((key) => key.startsWith('x-amz-checksum-'));
    assert.deepEqual(
      extra,
      [],
      // R2 answers a pair with `InvalidRequest: You can only specify one
      // non-default checksum at a time`, a 400 raised only after the whole
      // archive has validated. Nothing published for six days behind it.
      `the sdk added ${extra.join(', ')} alongside content-md5, which R2 rejects with a 400`,
    );
  });

  it('proves the guard is load-bearing by removing it', async () => {
    // If a default-configured client ever stops adding a checksum, the option
    // in createCameraR2Client has become decoration and the test above is no
    // longer proving anything.
    const headers = await defaultHeaders();
    assert.ok(
      headers.some((key) => key.startsWith('x-amz-checksum-')),
      'expected the sdk default to add a checksum, so the WHEN_REQUIRED assertion means something',
    );
  });
});
