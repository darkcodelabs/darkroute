...
import { loadCounties, loadCountiesBytes } from './counties.mjs';
import {
  FIXTURE_WAY_CAMERAS,
  captureWithWayMappedCameras,
} from './camera-capture-test-helpers.mjs';
...
const handoff = (cameras, total = 1) => {
  const transformation = {
    ...
    wayCentroids: cameras.filter((camera) => isWayCameraId(camera.id)).length,
    tombstonesBlocked: 0,
    tombstonesCleared: 0,
...
describe('source-build provenance', () => {
  ...
  it('refuses the historical receipt whose response watermarks were not retained', () => {
    /* The receipt as it was committed before the 2026-09-09 promotion, kept as
       a fixture: the committed one is approved now and is the direct-capture
       shape this adapter does not read. */
    const blocked = JSON.parse(
      readFileSync(new URL('./fixtures/deflock-us-source-review.unapproved.json', import.meta.url), 'utf8'),
    );
...
/**
 * The eighteen way-mapped readers the national capture actually holds.
 *
 * Ids, centroids and versions are copied from the retained capture, not
 * invented: these are live Flock, Motorola and LVT cameras in fourteen states
 * whose mappers drew the road or gantry rather than dropping a pin. Every one
 * of them was dropped by the node-only transform, and a node-only prover then
 * read each id in the wrong namespace -- six deleted nodes, eleven untagged
 * geometry vertices, one id the node namespace has never held -- and would
 * have published all eighteen as provably removed.
 */
const NATIONAL_WAY_CAMERAS = [
  [9232283, -83.1533247, 34.28493452352941, 20],
  [11008055, -82.01787487878788, 26.652198412121216, 20],
  [112793657, -85.11830929000001, 41.15688463, 31],
  [182478744, -85.0589426, 41.07987825, 10],
  [421764939, -84.01042161780826, 36.016381889041085, 21],
  [544681135, -80.96036081000001, 35.02245744, 16],
  [627049408, -93.4614106, 45.012582800000004, 10],
  [707080825, -93.2390572375, 45.10013427499999, 20],
  [743271520, -95.7757076153846, 29.862917938461543, 5],
  [930508001, -96.08080045, 41.2858939, 2],
  [1040996576, -94.90892775555555, 38.807184033333336, 3],
  [1105411386, -105.83807575, 40.25663555, 4],
  [1113371732, -76.5127886, 42.42591095454546, 3],
  [1123228563, -74.34703135263157, 40.72765409999999, 3],
  [1247537787, -81.05441041666667, 35.22022688333334, 4],
  [1273883625, -84.29125178333334, 33.75728725, 2],
  [1471554229, -80.96313756363635, 35.02567993636364, 2],
  [1516934727, -95.74057245641026, 29.91615561538462, 2],
];

const capturedWayFeature = ([osmId, lon, lat, osmVersion]) => ({
  type: 'Feature',
...
describe('way-mapped cameras', () => {
  it('carries a qualifying way at its centroid under a way-namespaced id', () => {
    const record = capturedFeatureToRecord(capturedWayFeature(NATIONAL_WAY_CAMERAS[0]));
    assert.equal(record.id, 'osm:w9232283');
    assert.deepEqual(parseCameraId(record.id), { type: 'way', osmId: 9232283 });
    assert.equal(record.lon, -83.1533247);
    assert.equal(record.lat, 34.28493452352941);
    assert.equal(record.osmVersion, 20);
    assert.equal(record.tags.manufacturer, 'Flock Safety');
  });

  it('leaves a node record byte-identical to what it always was', () => {
    const feature = {
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [-94.7, 38.9] },
      properties: {
    ...
    assert.deepEqual(capturedFeatureToRecord(feature), {
      id: 'osm:12345',
      lat: 38.9,
      lon: -94.7,
      directionDeg: null,
      osmVersion: 7,
      osmTimestamp: '2026-08-31T13:45:12Z',
      tags: { manufacturer: 'Flock Safety', 'surveillance:type': 'ALPR' },
    });
    assert.equal(isWayCameraId('osm:12345'), false);
  });

  it('refuses an element type the capture cannot place', () => {
    assert.equal(
      capturedFeatureToRecord({
        type: 'Feature',
        geometry: { type: 'Point', coordinates: [-94.7, 38.9] },
        properties: { osmId: 1, osmType: 'relation', osmVersion: 1 },
      }),
      null,
    );
  });

  it('round-trips all eighteen national way ids through the release hand-off', () => {
    const records = NATIONAL_WAY_CAMERAS.map((entry) =>
      capturedFeatureToRecord(capturedWayFeature(entry)),
    );
    const elements = toOverpassElements(records);
    assert.equal(elements.length, NATIONAL_WAY_CAMERAS.length);
    const byId = new Map(elements.map((element) => [element.id, element]));
    for (const [osmId, lon, lat, osmVersion] of NATIONAL_WAY_CAMERAS) {
      const element = byId.get(osmId);
      // The namespace survives the hand-off. Emitting `node` here is exactly
      // the confusion that read eighteen live cameras as removed.
      assert.equal(element.type, 'way');
      assert.equal(element.version, osmVersion);
      assert.equal(element.lon, lon);
      assert.equal(element.lat, lat);
      const published = normalise(element);
      assert.equal(published.id, cameraIdFor('way', osmId));
      assert.deepEqual(parseCameraId(published.id), { type: 'way', osmId });
    }
    // No way id may collide with the node id that shares its number.
    const ids = new Set(records.map((record) => record.id));
    assert.equal(ids.size, NATIONAL_WAY_CAMERAS.length);
    for (const [osmId] of NATIONAL_WAY_CAMERAS) {
      assert.equal(ids.has(`osm:${String(osmId)}`), false);
    }
  });

  it('carries way cameras through the whole reviewed transformation', () => {
    const capture = captureWithWayMappedCameras();
    const countyIndex = loadCountiesBytes(readFileSync(DEFAULT_COUNTY_GEOFENCE));
    const { cameras, transformation } = transformCapturedCollection(
      capture.collection,
      [],
      countyIndex,
    );
    assert.equal(transformation.wayCentroids, FIXTURE_WAY_CAMERAS);
    assert.equal(transformation.excludedUnsupportedElements, 0);
    const live = new Set(cameras.map((camera) => camera.id));
    for (const osmId of capture.wayIds) {
      assert.equal(live.has(cameraIdFor('way', osmId)), true);
      assert.equal(live.has(`osm:${String(osmId)}`), false);
    }
    // A dropped way would show up here as three missing output elements.
    assert.equal(
      transformation.outputElements,
      transformation.sourceFeatures - transformation.excludedTerritory,
    );
  });

  it('excludes and counts an element type it cannot place, without dropping the rest', () => {
    const capture = captureWithWayMappedCameras();
    const countyIndex = loadCountiesBytes(readFileSync(DEFAULT_COUNTY_GEOFENCE));
    const withRelation = {
      ...capture.collection,
      features: [
        ...capture.collection.features,
        {
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [-94.7, 38.9] },
          properties: { osmId: 77, osmType: 'relation', osmVersion: 1 },
        },
      ],
    };
    const { transformation } = transformCapturedCollection(withRelation, [], countyIndex);
    assert.equal(transformation.excludedUnsupportedElements, 1);
    assert.equal(transformation.wayCentroids, FIXTURE_WAY_CAMERAS);
  });
});
