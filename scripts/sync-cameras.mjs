...
/**
 * THE ABSOLUTE LIMIT ASSUMES AN HOURLY RUN, AND A CATCH-UP RUN IS NOT ONE.
 *
 * 500 is about seventy hours of measured churn, which is a generous tripwire
 * for a single hourly diff and a nonsense one for a backlog. On 2026-09-07 the
 * first sync after the schedule had been switched off for six days applied all
 * 141 pending diffs correctly, computed 625 tombstones - LESS than the ~990 the
 * documented 0.12%/day rate predicts over that span - and was refused by a
 * threshold calibrated for one hour of work. The breaker was right to ask; the
 * number it asked with was measuring the wrong thing.
 *
 * So the ceiling scales with the span actually being applied: the hourly floor
 * stays exactly 500, and a run covering N hours is allowed the churn N hours
 * would produce, times three. Three because the daily rate is a mean over a
 * year and real weeks are lumpy - a state doing a bulk retag lands in one day.
 *
 * THE FRACTION CHECK IS UNCHANGED AND IS THE REAL CEILING. No catch-up, however
 * long, may remove more than 1% of the live table in one run. A backlog earns
 * more absolute room; it does not earn the right to empty the archive. The
 * 2026-09-07 run needed 625 against a fraction ceiling of 1,399, so the guard
 * that matters still had it comfortably covered.
 */
const TOMBSTONE_DAILY_CHURN = 0.0012;
const TOMBSTONE_SPAN_SAFETY = 3;

export function tombstoneCeiling(live, diffsApplied) {
  if (!Number.isFinite(live) || !Number.isFinite(diffsApplied) || diffsApplied <= 1) {
    return MAX_TOMBSTONE_ABSOLUTE;
  }
  const perHour = (live * TOMBSTONE_DAILY_CHURN) / 24;
  const spanned = Math.ceil(perHour * diffsApplied * TOMBSTONE_SPAN_SAFETY);
  return Math.max(MAX_TOMBSTONE_ABSOLUTE, spanned);
}
...
export function decide(element, known, inside = insideBoundingFootprint) {
   ...
   *
   * THE ARCHIVE NOW HOLDS WAY-MAPPED CAMERAS, AND THIS STILL IGNORES WAYS.
   * That is safe rather than lucky: a way camera is published as `osm:w<id>`
   * and every lookup on this path builds `osm:<id>`, so a way event can never
   * resolve to one. What it costs is stated where a reader will find it --
   * `docs/public/AGGREGATION-POLICY.md` -- a way-mapped camera is carried at
   * its baseline version and only the next reviewed baseline moves it. Reading
   * a way event here would need the member geometry to recompute a centroid,
   * and an OsmChange carries node references without their coordinates.
   */
...
/**
 * PLACES SURVIVE A SYNC. The release writes `places.json` from the pinned
 * context -- one row per Census place with a camera in it -- and every hourly
 * upsert used to replace it with the "no enrichment" sidecar, because this
 * builder knew counties and nothing else. It now takes the places document it
 * found on disk and recounts it from the records: a row's count is the number
 * of live records carrying its geoid, a row nobody carries any more is
 * dropped, and a camera with no context (one replication added, which the
 * context has never seen) counts as unincorporated until the next context
 * build names its town.
 */
export function buildApprovedGazetteers(records, countyIndex, generatedAt, heldPlaces = null) {
  ...
  return {
    ...
    places: recountPlaces(records, heldPlaces, cameras, generatedAt),
  };
}

function recountPlaces(records, heldPlaces, cameras, generatedAt) {
  const disabled = {
    generatedAt,
  ...
  if (heldPlaces?.source !== CONTEXT_PLACES_SOURCE || !Array.isArray(heldPlaces.rows)) return disabled;
  const counts = new Map();
  let inPlace = 0;
  for (const record of records) {
    if (typeof record.placeGeoid !== 'string') continue;
    inPlace += 1;
    counts.set(record.placeGeoid, (counts.get(record.placeGeoid) ?? 0) + 1);
  }
  const rows = heldPlaces.rows
    .filter((row) => counts.has(row.geoid))
    .map((row) => ({ ...row, cameras: counts.get(row.geoid) }))
    .sort((left, right) => right.cameras - left.cameras || left.geoid.localeCompare(right.geoid));
  const named = rows.reduce((sum, row) => sum + row.cameras, 0);
  if (named !== inPlace) {
    throw new Error(`places.json names ${String(named)} of ${String(inPlace)} records that carry a place`);
  }
  return {
    generatedAt,
    ...
    places: rows.length,
    inPlace,
    unincorporated: cameras - inPlace,
    rows,
  };
}
...
async function main() {
  ...
  /* The span this run actually covered, so the ceiling judges the right amount
     of time. One diff is one hour; see `tombstoneCeiling`. */
  const spanHours = Math.max(1, appliedThrough - state.lastAppliedSeq);
  const tombstoneLimit = tombstoneCeiling(live, spanHours);
  if (wouldTombstone > tombstoneLimit || fraction > MAX_TOMBSTONE_FRACTION) {
    process.stderr.write(
      `\nCIRCUIT BREAKER: ${String(wouldTombstone)} tombstones exceeds the limit ` +
        `(${String(tombstoneLimit)} absolute over ${String(spanHours)}h / ` +
  ...
  for (const [id, up] of upserts) {
     ...
     */
    if (known !== undefined && known.x === x && known.y === y) {
      for (const field of CARRIED_FORWARD) {
        const held = known.camera[field];
        if ((typeof held === 'string' && held !== '') || Number.isSafeInteger(held)) {
          record[field] = held;
        }
      }
    }
  ...
  if (state.versionsKnown === true) {
    let heldPlaces = null;
    try {
      heldPlaces = JSON.parse(readFileSync(join(target, 'places.json'), 'utf8'));
    } catch {
      heldPlaces = null;
    }
