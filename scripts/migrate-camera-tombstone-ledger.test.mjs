...
describe('cutover coverage for a way-mapped predecessor camera', () => {
  /*
   * `osm:9232283` in a legacy tile and `osm:w9232283` in the new generation are
   * the SAME live Flock reader on US 29 in Georgia. Demanding a removal
   * tombstone for the first because the second spells its namespace out would
   * publish a deletion that did not happen -- and `verifyCutoverReconciliation`
   * would then be asked to prove it against a node that OpenStreetMap deleted
   * years ago, which it would happily do.
   */
  ...
  const predecessor = buildPredecessorEvidence({
    ...
    entries: [
      ...
      { key: 'tombstones.json', body: sourceBytes },
      {
        key: '11/1/2.json',
        body: Buffer.from(
          JSON.stringify({ cameras: [{ id: 'osm:9232283' }, { id: 'osm:10' }] }),
        ),
      },
    ],
    deployment,
  ...
  const emptyBytes = Buffer.from(`${JSON.stringify(emptyLedger)}\n`);

  it('needs no cutover tombstone when the way namespace publishes the id', () => {
    const { extras } = assertCutoverReconciliationShape({
      predecessor,
      sourceBytes,
      migratedBytes: emptyBytes,
      migratedLedger: emptyLedger,
      baselineLiveIds: ['osm:10', 'osm:w9232283'],
    });
    assert.deepEqual(extras, []);
  });

  it('still demands one when neither namespace publishes the id', () => {
    assert.throws(
      () =>
        assertCutoverReconciliationShape({
          predecessor,
          ...
          migratedBytes: emptyBytes,
          migratedLedger: emptyLedger,
          baselineLiveIds: ['osm:10'],
        }),
      /does not exactly cover the absent predecessor ids/,
    );
  });

  it('refuses a cutover tombstone for an id the way namespace still publishes', () => {
    const ledger = {
      ...emptyLedger,
      tombstones: [
        { id: 'osm:9232283', reason: 'cutover_reconciliation', seq: 12, osmVersion: 5 },
      ],
    };
    assert.throws(
      () =>
