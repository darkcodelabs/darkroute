...
test('the brand master decodes to its declared dimensions with real transparency', () => {
   ...
   *
   * 1312x1312 is the small mark: painted as an alpha mask at 24px in the dock
   * and 16px in a browser tab, sizes where colour averages to one flat grey -
   * which is why it is white-on-transparent and not the artwork.
   *
   * IT IS DERIVED FROM `drlogo.png`, not a separate drawing. The owner supplies
   * ONE file for the whole brand, so the mark is extracted: the white strokes
   * become the alpha and the colour sparkle field is dropped. Dropping the
   * sparkles is the load-bearing part - the platform tints a mask and paints
   * every opaque pixel, so keeping them renders the notification badge as a
   * filled square in the status bar.
   *
   * 1312 SQUARE is the owner's own canvas -
   * `darkroute_logo_clean_master_1312.png` arrives square, with real
   * transparency and 139k fully-opaque stroke pixels. Nothing is cropped,
   * centred, padded or rescaled. Two earlier attempts did all of those to
   * earlier files and each was a redesign rather than a port: one reframed the
   * artwork onto the mark's bounding box, and one padded a 1312x1199 file to a
   * square because the generator letterboxes a non-square master and
   * letterboxing with transparency put holes in `icon-192` - a launcher icon
   * that shows the user's wallpaper through the logo.
   *
   * The numbers are pinned rather than derived on purpose, and this test has
   * now caught five master replacements: one downscaled to 256px, which would
   * have left every 512px icon upscaled from a thumbnail; one produced a mask
   * with no opaque pixel at all, because it was derived from a file whose own
   * maximum alpha was 254; and one changed the shape of the canvas.
   */
  const master = decodePng(readFileSync(MASTER));
  assert.equal(master.width, 1312);
  assert.equal(master.height, 1312);
  assert.equal(master.data.length, 1312 * 1312 * 4);
