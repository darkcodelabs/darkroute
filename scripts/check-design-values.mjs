...
const SUPPLIED_ASSETS = new Set([
  /*
   * THE CHROME'S LOGO AND WORDMARK, shipped with the design rather than cut
   * from the eye master. The logo is the mark at full pixel spray -- it breaks
   * the search bar's edge on all four sides and its spray is the thing that
   * crosses the hairline, so it is a different drawing from the flat 24px
   * stencil, not a resize of one. The wordmark is type, cropped to "darkroute"
   * with no `.ai`, and nothing derives type from a logo.
   *
   * They live under `public/brand/` and NOT `public/assets/` on purpose:
   * vite-plugin-pwa declares anything whose URL starts with the Vite assetsDir
   * immutable, so a file dropped in `assets/` is precached with
   * `revision: null` and never invalidates on a device that already has it.
   * A rebrand that silently does not reach returning users is the failure that
   * path buys.
   */
  'apps/pwa/public/brand/darkroute-logo.png',
  'apps/pwa/public/brand/darkroute-wordmark.png',
  'apps/pwa/public/assets/node-mesh-eye.png',
  /*
   * THE APP ICON, AS THE PRODUCT'S OWN FACE.
   *
   * `darkroute_icons/darkroute_logo_clean_master_1312.png`, BYTE FOR BYTE.
   * The search bar's mark and the dock's report key draw this one file.
   *
   * NOT REFRAMED, and that is the point. Earlier passes cropped it to the
   * sparkle field, recentered it on the octagon's bounding box and rescaled it -
   * each defensible on its own and all three a redesign, which is not what a
   * port is. The owner's canvas is the canvas. It arrives 1312 square with real
   * alpha and 139k fully-opaque stroke pixels, so there is nothing to correct.
   *
   * NOT DERIVED FROM EITHER MASTER, and the distinction that matters is against
   * `darkroute-icon.png`, which is easy to mistake for this. That file is an
   * older render LETTERBOXED into a square: the eye sits small in the middle
   * with sparkle field padding it out. Cropped to a 24px disc it reads as a
   * dark speckled blob, because the crop lands in the sparkles rather than on
   * the mark. That shipped. The two are not interchangeable and the small one
   * is the one that has to be right.
   *
   * ITS BLACK IS ALPHA, so it composites onto the bar and the dock key instead
   * of sitting on a black disc - which is why nothing that draws it sets a
   * border-radius any more.
   */
  'apps/pwa/public/assets/darkroute-app-icon.png',
  /*
   * THE LIGHT TWIN OF THE APP ICON.
   *
   * Derived from the master above, not supplied separately: only the WHITE
   * STROKES are recoloured to a near-black ink, and the coloured sparkle field
   * is left exactly as drawn. Two files rather than one for the same reason the
   * old search pair had two - this is a PICTURE, not a stencil, so a mask
   * cannot repaint it and inverting the whole image would turn the cyan orange.
   *
   * Swapped by `[data-fwm-mode]` on the three light skins in `searchBar.css`
   * and `reportKey.css`.
   */
  'apps/pwa/public/assets/darkroute-app-icon-light.png',
  /*
   * THE SEARCH MARK, ITS LIGHT TWIN AND THE WIDE LOGO ARE GONE, 2026-09-06.
   *
   * All three drew the PREVIOUS mark. The search pair lost its last caller when
   * the bar's mark became the app icon; `darkroute-wide.png` had none at all.
   * Deleted rather than left in place: a brand asset that nothing references is
   * one somebody wires up a year later, and it would have shipped the old logo
   * onto a surface that had already moved. The bundle carried them until now.
   */
...
function hintFor(tokens, v) {
  ...
  return s ?? 'no token covers this value -- mark the line with a GAP marker before inventing one';
}
