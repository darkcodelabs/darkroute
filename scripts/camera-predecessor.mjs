...
export function validatePredecessorEvidence(evidence) {
  ...
  for (let index = 0; index < evidence.liveIds.length; index += 1) {
    ...
    if (!CAMERA_ID.test(id ?? '') || (index > 0 && evidence.liveIds[index - 1] >= id)) {
...
/**
 * IS THIS PREDECESSOR ID STILL PUBLISHED, UNDER WHATEVER NAME IT NOW HAS?
 *
 * A predecessor id captured before the namespaced scheme is TYPE-LESS. The
 * publisher that wrote `osm:9232283` into a tile had no field for "and this
 * one is a way", so the same element is `osm:w9232283` in a generation built
 * by the current pipeline. Comparing the two forms literally would report a
 * camera that is still on the map, in the same place, at a newer version, as
 * a predecessor id this baseline lost -- and the cutover would then be asked
 * to write a removal for a reader that was never removed.
 *
 * WHY THE ALIAS CANNOT LAUNDER A REAL DISAPPEARANCE. The worry is the
 * coincidence: predecessor `osm:N` was really NODE N, node N is gone, and way
 * N independently happens to be a camera now, so the way answers for the node
 * and hides its loss. That cannot happen here, and the reason is the capture
 * rather than this function. The capture is national and complete over the
 * pinned territory -- every other guard in the release exists to establish
 * exactly that -- so if node N were still a qualifying in-scope camera it
 * would be IN the new live set as `osm:N`, the direct match would fire, and
 * the alias would never be consulted. The alias is only ever reached for an
 * id the node namespace no longer publishes.
 *
 * The alias is one-directional on purpose: a bare id may be answered by a way,
 * a way id is never answered by a bare one. A generation published under this
 * scheme states the namespace, and a stated namespace is not up for
 * reinterpretation.
 */
export function predecessorIdIsCovered(id, live) {
  return live.has(id) || live.has(wayCameraId(id));
}

/** Predecessor ids this live set carries only under their way-namespace name. */
export function wayAliasedPredecessorIds(evidence, liveIds) {
  const predecessor = validatePredecessorEvidence(evidence);
  const live = new Set(liveIds);
  return predecessor.liveIds.filter((id) => !live.has(id) && live.has(wayCameraId(id)));
}
...
export function uncoveredPredecessorIds(evidence, liveIds, tombstones) {
  ...
  return predecessor.liveIds.filter(
    (id) => !predecessorIdIsCovered(id, live) && !removed.has(id),
  );
}
...
export function predecessorIdsMissingFromLiveSet(evidence, liveIds) {
  const predecessor = validatePredecessorEvidence(evidence);
  const live = new Set(liveIds);
  return predecessor.liveIds.filter((id) => !predecessorIdIsCovered(id, live));
}
