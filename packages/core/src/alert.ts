...
/**
 * THE BAND THE DESIGN FILES DRAW, feet.
 * `Flockys Design System.dc.html` § settings: "ALERT AT · 500 FT" between
 * end labels "100" and "1000".
 *
 * These are still the ends of v0's settings slider, and `settings.css` carries
 * one hand-transcribed detent rule per 50 ft step across exactly this band -
 * nineteen of them - so the drawn control and the stylesheet are keyed to this
 * pair. It is no longer what the ENGINE accepts; see the two constants under
 * it, and `apps/pwa/src/features/settings/threshold.ts` for the control that
 * still reads these.
 */
export const ALERT_THRESHOLD_DESIGN_MIN_FT = 100;
export const ALERT_THRESHOLD_DESIGN_MAX_FT = 1000;

/**
 * WHAT THE ENGINE ACCEPTS, feet, which is wider than the band above.
 *
 * OWNER DECISION, 2026-09-06: the warn distance is a slider in the map-view
 * panel running "from 1 miles down to 25ft". A control cannot offer a value the
 * store would throw on, so the gate moved with it rather than the panel
 * clamping into a band the engine would refuse.
 *
 * WHAT THE OLD BOUNDS WERE FOR, because both arguments are still true and
 * somebody will want them when this is next questioned. 100 ft is about what a
 * car covers in a second and a half at 45 mph, so a threshold under it warns
 * about a reader that is already alongside - the alert arrives too late to be
 * a decision. 1000 ft was the ceiling because {@link APPROACHING_OUTER_FT} is
 * 1000: a threshold at the outer edge of the approaching band leaves no
 * approach left to report, so the ramp collapses into one state.
 *
 * Neither is a reason to refuse the setting. A driver who asks to hear about
 * every reader within a mile is asking for a noisier app on purpose, and one
 * who sets 25 ft is asking for a record of what they passed rather than a
 * warning about what is coming. The engine's job is to be right about the
 * number it was given, not to hold an opinion about it.
 */
export const ALERT_THRESHOLD_MIN_FT = 25;
export const ALERT_THRESHOLD_MAX_FT = 5280;

/**
 * THE STOPS THE MAP-VIEW SLIDER OFFERS, feet, low to high.
 *
 * A LIST RATHER THAN A CURVE, and that was the decision worth making. A linear
 * 25..5280 range spends nine tenths of its travel above 500 ft, where nobody
 * needs resolution, and gives the useful low end about three pixels; a
 * logarithmic mapping fixes the travel and hands back values like 37 ft and
 * 412 ft - numbers no driver chose, that the readout then has to round anyway,
 * and that make two phones set "the same" disagree.
 *
 * These are the distances a person says out loud. 1320 and 2640 are the
 * quarter and the half mile, 5280 is the mile the owner asked for, 25 is the
 * floor, and 500 - the design's own default - is a stop rather than something
 * you have to land on. Every value the slider can produce is therefore one the
 * engine accepts, one a test can name, and one {@link snapThresholdFt} leaves
 * alone.
 */
export const ALERT_THRESHOLD_STOPS_FT: readonly number[] = Object.freeze([
  25, 50, 100, 150, 250, 500, 750, 1000, 1320, 2640, 5280,
]);
...
// IS in the design, and each has an entry in gap record core-engine so
// the numbers get a decision rather than inheriting one by default.
// ---------------------------------------------------------------------------

/**
 * Width of the exit band, feet. GAP: alert-hysteresis-band
...
/** Per-camera notification cooldown, ms. GAP: alert-cooldown-window */
export const DEFAULT_NOTIFICATION_COOLDOWN_MS = DEFAULT_MUTE_DURATION_MS;

/** Worst horizontal accuracy that may still alert, metres. GAP: gps-accuracy-gate */
...
/** Stationary this long before alerts are suppressed, ms. GAP: stationary-dwell */
export const DEFAULT_STATIONARY_DWELL_MS = 120_000;

/** Moving this long before alerts come back, ms. GAP: stationary-dwell */
export const DEFAULT_MOVING_DWELL_MS = 5_000;

/** Two cameras closer than this with different ids are one camera, feet. GAP: camera-dedupe-epsilon */
...
/** Smoothing factor for derived speed. GAP: derived-speed-smoothing */
...
/**
 * Threshold bounds are a product decision, not a clamp: the map-view slider
 * goes 25 to 5280 and the bezel steps in 50s inside that, so a value outside it
 * is a caller bug rather than a number to quietly pull into range.
 */
...
/**
 * Snap a threshold to the bezel's 50 ft steps and hold it inside the bounds.
 *
 * AN OFFERED STOP IS RETURNED UNTOUCHED, and that clause is not a nicety. Four
 * of {@link ALERT_THRESHOLD_STOPS_FT} are off the 50 ft grid - 25, 1320, 2640
 * and 5280 - and this function is what the engine loop runs the stored setting
 * through on every reconfigure. Without the check, a driver who chose the half
 * mile would be warned at 2650 ft, and one who chose 25 ft would be warned at
 * 50: the number on the panel and the number in the engine would differ, which
 * is the one failure a threshold control cannot have.
 */
export function snapThresholdFt(thresholdFt: number): number {
  ...
  if (ALERT_THRESHOLD_STOPS_FT.includes(thresholdFt)) return thresholdFt;
