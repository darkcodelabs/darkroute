...
import {
  ALERT_THRESHOLD_DESIGN_MAX_FT,
  ALERT_THRESHOLD_DESIGN_MIN_FT,
  ALERT_THRESHOLD_MAX_FT,
  ALERT_THRESHOLD_MIN_FT,
  ALERT_THRESHOLD_STEP_FT,
  ALERT_THRESHOLD_STOPS_FT,
...
describe('design constants', () => {
  it('matches the values in the design files', () => {
    ...
    expect(ALERT_THRESHOLD_DESIGN_MIN_FT).toBe(100);
    expect(ALERT_THRESHOLD_DESIGN_MAX_FT).toBe(1000);
  ...
  it('accepts a wider band than the design draws, because the owner asked for one', () => {
    // "from 1 miles down to 25ft" - owner, 2026-09-06. The design's own band is
    // asserted above and is still v0's slider; these are the engine's.
    expect(ALERT_THRESHOLD_MIN_FT).toBe(25);
    expect(ALERT_THRESHOLD_MAX_FT).toBe(5280);
    // The band did not shrink at either end while it grew.
    expect(ALERT_THRESHOLD_MIN_FT).toBeLessThan(ALERT_THRESHOLD_DESIGN_MIN_FT);
    expect(ALERT_THRESHOLD_MAX_FT).toBeGreaterThan(ALERT_THRESHOLD_DESIGN_MAX_FT);
  });

  it('offers stops that run the whole band and include the default', () => {
    expect(ALERT_THRESHOLD_STOPS_FT.at(0)).toBe(ALERT_THRESHOLD_MIN_FT);
    expect(ALERT_THRESHOLD_STOPS_FT.at(-1)).toBe(ALERT_THRESHOLD_MAX_FT);
    expect(ALERT_THRESHOLD_STOPS_FT).toContain(DEFAULT_ALERT_THRESHOLD_FT);
    // The quarter and the half mile, which is why they are not round in feet.
    expect(ALERT_THRESHOLD_STOPS_FT).toContain(1320);
    expect(ALERT_THRESHOLD_STOPS_FT).toContain(2640);
    // Low to high, with no repeats: the slider indexes this array.
    const sorted = [...ALERT_THRESHOLD_STOPS_FT].sort((a, b) => a - b);
    expect([...ALERT_THRESHOLD_STOPS_FT]).toEqual(sorted);
    expect(new Set(ALERT_THRESHOLD_STOPS_FT).size).toBe(ALERT_THRESHOLD_STOPS_FT.length);
  });
  ...
  it('leaves an offered stop alone even when it is off the bezel grid', () => {
    // The half mile is 2640 ft, which rounds to 2650 on 50 ft steps. If the
    // grid won, the panel would say "0.5 mi" while the engine warned at 2650.
    for (const stop of ALERT_THRESHOLD_STOPS_FT) {
      expect(snapThresholdFt(stop)).toBe(stop);
    }
  });
});
...
describe('AlertEngine - configuration', () => {
  it('refuses a threshold outside the slider range', () => {
    expect(() => new AlertEngine({ thresholdFt: 24 })).toThrow(RangeError);
    expect(() => new AlertEngine({ thresholdFt: 5281 })).toThrow(RangeError);
    expect(() => new AlertEngine({ thresholdFt: Number.NaN })).toThrow(RangeError);
    // Both ends of what the map-view slider can produce, and the design band
    // inside it, which the watch bezel and v0's slider still cover.
    expect(() => new AlertEngine({ thresholdFt: 25 })).not.toThrow();
    expect(() => new AlertEngine({ thresholdFt: 5280 })).not.toThrow();
  ...
  it('applies a new threshold on the next tick', () => {
    ...
    expect(() => engine.setThresholdFt(5281)).toThrow(RangeError);
  });
