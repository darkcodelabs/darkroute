...
import { onRequestGet, onRequestHead } from './[[path]].ts';
...
describe('HEAD', () => {
  it('answers from the same read as GET: status, generation and cache headers, no body', async () => {
    const entries = new Map([
      ['index.json', flatObject(FLAT_INDEX)],
      [LOGICAL_KEY, flatObject('{"cameras":[]}')],
    ]);
    const get = vi.fn(async (key: string) => entries.get(key) ?? null);
    const next = vi.fn(async () => new Response('<!doctype html>', { headers: { 'content-type': 'text/html' } }));
    const response = (await onRequestHead({
      request: new Request(`https://darkroute.ai/cameras/${LOGICAL_KEY}`, { method: 'HEAD' }),
      env: { CAMERA_TILES: { get } },
      next,
  ...
  it('carries a miss through as the same 404 GET would give', async () => {
    const entries = new Map([['index.json', flatObject(FLAT_INDEX)]]);
    const get = vi.fn(async (key: string) => entries.get(key) ?? null);
    const response = (await onRequestHead({
      request: new Request(`https://darkroute.ai/cameras/${LOGICAL_KEY}`, { method: 'HEAD' }),
      env: { CAMERA_TILES: { get } },
