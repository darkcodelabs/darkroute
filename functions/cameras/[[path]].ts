...
const readTile = async (context: Parameters<PagesFunction<Env>>[0]): Promise<Response> => {
...
/**
 * HEAD, from the same decision GET would make.
 *
 * Pages routes by method, so with only `onRequestGet` exported a HEAD fell past
 * this file to the static layer and came back as the app's HTML with a 200 -
 * `API.md` promised a 405 for unexported methods and the platform did not give
 * one. A HEAD now runs the same read, with the same pointer and the same
 * generation header, and returns the headers alone.
 */
export const onRequestHead: PagesFunction<Env> = async (context) => {
  const full = await onRequestGet(context);
  const headers = new Headers(full.headers);
  headers.delete('content-length');
  return new Response(null, { status: full.status, statusText: full.statusText, headers });
};

/*
 * THE EDGE CACHE, FROM INSIDE THE FUNCTION. A Pages Function's response is
 * DYNAMIC to Cloudflare's cache unless the Function puts it there itself, so
 * every tile read on every drive was an R2 read -- `cf-cache-status: DYNAMIC`,
 * measured 2026-09-09. The zone-level cache rule the blockers list asked for
 * needs a zone token this deployment does not hold; the Cache API needs
 * nothing. The key is the request URL, the lifetime is the `cache-control` the
 * response already carries (an hour, must-revalidate), and a colo that holds
 * the tile answers without touching the bucket. Absent `caches` -- the test
 * runner, a local dev server -- the read is exactly what it was.
 */
export const onRequestGet: PagesFunction<Env> = async (context) => {
  const edge = (globalThis as { caches?: { default?: Cache } }).caches?.default;
  if (edge !== undefined) {
    const held = await edge.match(context.request);
    if (held !== undefined) {
      const headers = new Headers(held.headers);
      headers.set('x-darkroute-edge', 'hit');
      return new Response(held.body, { status: held.status, headers });
    }
  }
  const response = await readTile(context);
  if (edge !== undefined && (response.status === 200 || response.status === 404)) {
    context.waitUntil(edge.put(context.request, response.clone()));
  }
  return response;
};
