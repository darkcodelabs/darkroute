...
copied profile is inert without the user's biometric or PIN. It is stronger and it is not
...
Not building it without a decision. Filed as an open question instead of invented.
...
key reference, trip history, alert history, the photographs attached to reports, and the destination
book - the recent places *and* the saved ones.

The saved places are the one entry on that list somebody will be tempted to spare. Section C of
`DarkRoute Search Entry.html` refuses it in as many words - "Nothing, by design. Saved places are
user-entered data and go with the wipe - if the wipe left anything behind, the promise would be
false" - and the reason is that Home and Work are two addresses a person can retype in ten seconds
and are, together, the single most useful thing on the device to anybody who should not have it.
`removal.test.ts` seeds both lists and asserts both are gone.

Not this path: `Forget all recent places` in SETTINGS is a *partial* clear - it empties the
recents and keeps the saved places - so it goes through `stores/destinations.ts` and must not be
routed here. This function is the wipe. It must not grow a second caller that means "some of it",
and `features/settings/removal.ts` must not grow a `clear()` call of its own.
...
fails it reports that failure instead of claiming a clean wipe.

Surfacing: the removal control needs a home. No SETTINGS screen exists in the design sources
(design gap no-settings-screen-exists), so the candidates are the WATCHLIST screen, which
...
ask their permission before building on their service instead of discovering their position later.
