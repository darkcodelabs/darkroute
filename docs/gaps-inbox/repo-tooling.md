...
- options: (1) drop it as dead design CSS - recommended unless a screen turns up that needs it; (2) add it as the sixth system keyframe if it is meant for the no-fix / stale-data indicator, which is the only behaviour in the brief that reads like a hard blink instead of a pulse.
...
- stand-in: literal `0px` inside the `env()` fallback slot, 13 occurrences in `apps/pwa/src/styles/global.css`, each inside a block comment that explains it is a zero fallback instead of a design value. NOTE FOR THE CHECKER AUTHOR: `scripts/check-design-values.mjs` needs `env(<name>, 0px)` on its allowlist or it will flag all 13.
