...
- decision: the dial is a tap target AND an ARIA slider -- arrow keys step one
  ...
  centre) leaves the bearing untouched instead of snapping to north.
...
- decision: the tile stays drawn and disabled, and the micro line reads
  ...
  note points at where the control actually is instead of restating a refusal
...
- decision: the queue line is the receipt. It is the live region
  ...
  write instead of incremented in memory, and the draft resets so the next
...
- STILL A LITERAL, deliberately: `font-weight: 400 / 600 / 700`. The token set
  ...
  (400 = `normal`, 700 = `bold`), so they read as keywords instead of as a
