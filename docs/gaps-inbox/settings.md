...
so they get their own entries instead of being folded into that one --
...
- Options: (1) keep it -- it is the claim the rest of the screen has to keep;
  (2) `NO ACCOUNT · NO ANALYTICS`, which is quotable from A1 but reads as a claim;
...
- Leaning: (1) now, (2) never -- a disabled row in a settings list reads as a
  broken app instead of as an honest one.
...
- Stand-in: none here. The argument is TRIAGE's, not the no-consumer one: a
  ...
  `SYNC NOW` would be a second inert button instead of one. What IS missing is
...
- Stand-in: `Audio`, derived from B10 and set in section 04's sentence casing
  ...
  constant instead of claiming section 04 as the source.
...
- Stand-in: `SettingsScreen` asks `isWatchSurface(onSurface)` instead, and
  ...
  six rows inert when locked instead of five. `SettingsScreen.test.tsx` covers
...
- Options: (1) keep; (2) `DELETE LOCAL DATA`, plainer and less in-voice;
  (3) `FORGET THIS DEVICE`, which reads as unpairing instead of erasing.
...
- Status: OPEN -- engineering, surfaced instead of hidden.
