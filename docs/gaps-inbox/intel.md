...
- stand-in: FOUR of the six unsourced values -- `MOUNT`, `EFF ATLAS`,
  ...
  which is a claim instead of an absence, and it is the most consequential
  fact on the card.
...
- source: `services/adapters/share.ts` states it outright -- "`url` is supplied
  ...
  out without a link instead of with the wrong one." There is no configured
...
- consequence: a shared card is text, not a link. Also deliberate: the body
  ...
  phone is and has been instead of about public infrastructure.
...
- why: hue-driven, the key would be GREEN (`--fwm-alert-clear`) on a clear
  ...
  camera instead of as a button. It would also put the key's black label on
