...
are recorded below instead of counted as fidelity: `TAKE DETOUR` (no route
...
1. the underlying reason the wake word cannot run ("must render this reason instead of
   a broken promise");
...
one it should be promoted to a shared component instead of copied.
...
it was made here by default instead of drawn. Options a designer might prefer:
...
the notice strip says why in one line instead of letting it happen silently.
...
**The design's own show answer is refused.** Screen 04 renders
...
route answer, and `AskScreen`'s `resolve` prop is the join a real route answerer
...
the join either way, but the answer changes how much of the classifier survives.
...
2. The design's own show suggestion was answered "plate lookup is switched
...
device actually holds instead of being three fixed strings. That is a design
...
plate. The file's own doc block claims refusals "name the underlying reason".
...
multiples of tokens and are derived instead of invented:
