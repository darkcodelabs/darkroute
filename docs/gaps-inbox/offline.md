...
> calls it "OFFLINE, as a list of what still works instead of an apology" -
...
Four elements are RADAR's, imported instead of rebuilt: `OfflineStrip`
...
- stand-in: `capabilities.ts` resolves all five from live facts. `mesh` reads
  ...
  rows go `NO` when IndexedDB is missing, and `cached-alerts` also goes
  `NO` when the tile cache is empty -- "running on cache" with no
...
- stand-in: a literal comma every three digits, written out instead of taken
...
- stand-in: `min-width: 2ch` on `.fwm-offline-cap-mark`. `ch` is one character
  of the row's own monospace face, so the slot tracks the type instead of a
