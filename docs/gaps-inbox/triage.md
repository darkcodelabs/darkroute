...
- Stand-in: the drive unit is the recorded trip. One open trip means one
  ...
  The ROW captions use the whole log instead of the trip -- `11 on your usual
...
- Options: (1) confirm the stand-in; (2) count cached cameras in the tiles
  instead, which measures the cache instead of the driver; (3) build a route
...
- Options: (1) leave verbatim; (2) render the sentence conditionally on the
  flag, which puts two versions of a structural promise in the codebase.
...
- Stand-in: `var(--fwm-text-readout)`, the nearer of the two, with the
  rendered line-height carried as a ratio instead of as a raw length. The
...
- Stand-in: `var(--fwm-text-micro)` (11px). The half pixel is almost
  certainly an artefact of the design document instead of an intent.
...
- Stand-in: the BUTTON is `var(--fwm-touch-min)` tall and
  ...
  knob's 3px inset is derived from the track and knob instead of stated, so it
