...
Why extend instead of renumber. `3px` sits *between* `--fwm-radius-1`
...
matching how `--fwm-radius-full` is already named by role instead of index.
...
- Not a design gap. `scripts/check-design-values.mjs` is not owned by this pass,
  so this is reported instead of fixed.
