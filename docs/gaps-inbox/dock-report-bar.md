...
> a floating pill instead of a pinned panel, with REPORT lifted out into its
...
Cross-references instead of new entries, because the decision is already
...
- options:
  1. keep `--fwm-surface-2`, and document that a divider inside a filled row is
     a surface step instead of a line.
...
- stand-in: `PIN_CONFIRM_DWELL_MS = HOLD_TO_DROP_MS` (1000 ms) -- the one
  published dock timing, reused instead of a second number invented next to
