...
one that looked most useful at first glance.
...
threshold, and the alert only appears when it fires instead of being on the
dash continuously.
...
replace instead of stack) - that replace-don't-stack behaviour is exactly
...
The join already exists on the phone side, and this is the important finding:
...
because it was about the hardware instead of about the screens. The watch is a
...
another phone on the dash instead of a neat watch-sized instrument - and
...
- What happens when the phone is not present? A watch alone has no camera data.
  The likely answer is "it says so and does nothing", matching how the
