...
> the size half is not, and is now unfixable by design instead of by omission.
...
written against the payload instead of against the belief that the field was
...
the honest readings of states the engine can produce
...
three more instead of hiding rows: `SYNCING`/`--fwm-accent-scan`,
