...
need a written decision instead of a default nobody chose.
...
- options:
  ...
  2. Bound by bytes instead of rows, using `navigator.storage.estimate()` and a
...
- options:
  ...
  2. Make the watchlist limit a product decision instead of a storage one -- a
...
- stand-in: `MAX_REPORT_PHOTOS = 50`. At `MAX_BYTES` (600 KB) per prepared JPEG
  ...
  the driver should be told about instead of one the app absorbs.
...
- options:
  1. Keep 50 rows.
  2. Bound by bytes instead of rows -- truer to the real constraint, since the
...
- options:
  ...
  3. Make it connectivity-driven instead of time-driven: retry on the
