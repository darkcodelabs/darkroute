...
behavioural timings and tolerances instead of rendered lengths, so no
`var(--fwm-*)` token covers them and `scripts/check-design-values.mjs` cannot
see them -- which is exactly why they need a written decision instead of a
...
- options:
  ...
  2. Median of the last N deltas -- more sturdy to a single bad fix, more lag.
