...
Two of those deserve a sentence instead of a table row.
...
old ones, so the meter's beat pattern is longer instead of doubled.
...
Why this counts for more than it did in v1. In v1 an inert `TRY` chip dimmed its
...
all of them, and it should come from the design instead of from five features
...
1\. The plate never leaves the device. No request carries it, in any form --
   ...
   `no Flock system is queried`. Both are structural claims.
