...
- `docs/gaps-inbox/log.md#spacing-scale-misses-5-14-and-18` -- B6's 14px body
  ...
  `var(--fwm-space-4)`.
- `docs/gaps-inbox/log.md#trip-lifecycle-has-no-owner` -- **THE STRUCTURAL
...
- Amendment (audit round 2): the layer has a second scope that bands a
  COUNT instead of a rate (`heat-scope-falls-back-when-no-trip-is-open`), and a
...
- Problem: `ShareCard` receives no configured public origin, and
  ...
  without a link instead of with the wrong one." the environment template declares no
...
- Stand-in: the same call `B2 · DEAD DROP` made for `EXPORT JSON`
  ...
  handler; a build with none renders the key disabled instead of live-looking
...
- Amendment (audit round 2): the screen is used zero-prop, so
  ...
  key is now drawn as a filled block instead of erased (see
...
- Stand-in: ground `var(--fwm-bg-sunken)`, hatch alternating
  ...
  is a ratio instead of a colour and so needs no colour token.
...
- Known limit, filed instead of hidden: it answers for the CENTRE, not for
...
- Problem: `what-the-csv-may-contain` refuses coordinates, bearings and
  ...
  1. ROW ORDER. `camerasInZone()` returns NEAREST FIRST and the rows were
     ...
     file instead of by any column in it.
