...
- stand-in: `radiusForDistanceFt()` interpolates piecewise-linearly between the
  drawn rings instead of inventing a curve, so a camera always lands where the
...
- **why this is not fixed here.** Both real fixes are one line in a file this
  ...
  gaming the checker instead of fixing anything.
...
- note: filed instead of fixed because `stores/index.ts` is a shared boundary
