...
belongs to the shell instead of to this screen; `Flockys Watch.dc.html` --
...
- Stand-in: the toggle scopes the two surfaces that carry no scope label of
  ...
  to now while it is open); with no trip started, `TRIP` is empty
...
- Stand-in: level 0 renders at the 1px hairline instead of at zero height,
...
- Status: OPEN -- LOG loads its own half; the rest of the join is unowned.
...
- Stand-in: the nearest step below in each case (`--fwm-space-1`,
  ...
  takes the same `--fwm-space-4` (16px) as the today card's 18px instead of a
