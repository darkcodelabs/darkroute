...
where it cannot the value is withheld instead of borrowed:
...
- `more` renders the unbuilt placeholder under v0. Nothing navigates there
  under v0, so it is unreachable instead of broken - but it is the one id in
