...
In one sentence: a reachable feed is not permission to publish it, two nearby
...
The canonical camera id is `osm:<node-id>` for a camera mapped as an OSM node
and `osm:w<way-id>` for one mapped as an OSM way. It is stable because it comes
from the upstream record, not from coordinates or a name.

OSM nodes, ways, and relations have separate id spaces, and the letter is what
keeps them apart. Way 1516934727 is a live reader in South Carolina; node
1516934727 has never existed. Way 9232283 is a live reader in Georgia; node
9232283 was deleted years ago. An id that did not state its namespace could be
resolved against the wrong one, and an unrelated answer -- or no answer -- would
then read as proof that the camera was removed.

The published camera layer accepts nodes and ways. A way-mapped camera is
published as one point at the centroid of that way's own member geometry, which
is the representative point the source capture computes; a relation is not
accepted, because the capture retains no geometry from which to derive one.
Way-mapped cameras exist because a mapper tagged the road, gantry or pole line
the reader watches instead of placing a separate node, and they are a small
minority of the layer.

Because the scheduled OSM replication path is node-only (see *Removals and
tombstones*), a way-mapped camera is carried at its baseline version and is not
advanced by the hourly sync. It changes only at the next reviewed baseline.
...
The scheduled sync reads `node` elements only. An OSM changefile carries nodes,
ways and relations, and the three id spaces overlap -- a deleted building can
share its number with a live camera -- so acting on a way or relation event
would tombstone an unrelated record. The consequence is stated plainly above: a
way-mapped camera is not maintained by the hourly path, and its absence from
that path is not a removal signal.
