...
| The undeployed curation gateway and platform | A tested Pages-compatible report handler exists as operator code that is not distributed, but no route under `functions/` exposes it and the PWA has no caller. It is **not** `POST /api/v1/submit` (§1.3), which is deployed and opens pull requests. The curation tooling operates a separately deployed platform. Neither is distributed here and neither is a live app endpoint |
...
`/api/v1/openapi.json`. Two administrative Functions manage the Cloudflare
Access tester allowlist on the private dev host (§1.2). None accepts a report
or a driver position, and **the archive itself is never written through any of
them**. Tile addresses are computed on the device and a z11 tile is roughly
15 km across. Reports, plates, watchlists and alert history never leave the
shipped client because it has no report transport.
...
shadows the matching asset path, with one deliberate exception: unknown paths
under `/api/v1/` are answered `404` as JSON by the middleware and a catch-all,
...
(`packages/api-client/src/index.ts`) and is imported by nothing; the
machine-readable contract is the served `openapi.json`, not a package.
...
| `/api/v1/monitoring` | `GET`, `HEAD` | `functions/api/v1/monitoring.ts` | Public equipment inventory; optional `kind` and `bbox` |
| `/api/v1/monitoring/image` | `GET`, `HEAD` | `functions/api/v1/monitoring/image.ts` | Selected camera ID only; source allowlist; no-store |
| `/records/road-monitoring.json` | `GET`, `HEAD` | `functions/records/road-monitoring.json.ts` | Shared live inventory with packaged fallback before initial publication |
...
| `/api/v1/doc/{name}`                  | `GET`, `HEAD` | `functions/api/v1/doc/[name].ts`                           | Public; serves the public mirror's `docs/public/*.md`                              |
| `/api/v1/submit`                      | `POST`        | `functions/api/v1/submit.ts`                               | Public; opens a pull request, writes nothing to the archive; 6/hour/address |
| `/api/v1/photo` (also `/api/v1/photo/`) | `PUT`       | `functions/api/v1/photo/[[key]].ts`                        | Public; stores one image, keyed by its bytes |
| `/api/v1/photo/{key}`                 | `GET`, `HEAD` | `functions/api/v1/photo/[[key]].ts`                        | Public; content-addressed, immutable |
| `/api/v1/*`, anything else | any | `functions/api/v1/_middleware.ts`, `functions/api/v1/[[path]].ts` | `404` JSON listing what exists; wrong method on a real path is `405` + `allow` |
| `/api/admin/me`, `/api/admin/testers` | see §1.2 | the admin Functions (not distributed)                                 | Cloudflare Access; not distributed |

Methods, measured. Cloudflare Pages routes strictly by method and falls
through to the static layer when no handler matches. That gave two different
answers for "a method the file does not export": a `POST` to `/cameras/*` is
`405` from the static layer, but a `HEAD` used to fall through to the app's
HTML with a `200`, on `/cameras/*` and on every `/api/v1/*` route, while
`allow:` advertised HEAD. So neither is left to the platform any more. The
camera route exports `onRequestHead`, and under `/api/v1/` the middleware
answers every method itself: HEAD from a same-origin GET of the same URL,
unknown paths `404`, wrong methods `405` with `allow`. `grep -rn 405 functions/`
now finds the middleware and the catch-all. Verify it against a deployment:
`curl -sI https://darkroute.ai/api/v1/stats` must print
`content-type: application/json; charset=utf-8`, and
`curl -s https://darkroute.ai/api/v1/<anything not in the table>` must be JSON, not HTML.
...
and noncanonical integer spellings return `400` before R2 data is read. `HEAD`
...
### 1.3 `/api/v1/*`, the public API

Files: `functions/api/v1/_middleware.ts` (the front door), `_routes.ts`
(the route table), `[[path]].ts` (the catch-all), and one file per endpoint.
Contract: `GET /api/v1/openapi.json`, OpenAPI 3.1, produced by
`functions/api/v1/openapi.json.ts` from the handlers' own exported constants  -
the span cap, the tile cap, the owner types, the media types, the document
names, and held to the route table and to every handler's error codes by
`functions/api/v1/openapi.json.test.ts`. This section is the prose; the spec is
the schema. Where they disagree, the spec is generated from the code and this
file is the bug.

Hosts. The canonical origin is `https://darkroute.ai`. `https://api.darkroute.ai`
is the developer console (§1.4), whose Functions proxy `/api/v1/*` and the
shorter `/v1/*` to the canonical origin unchanged, so the spec is valid against
either host. Answers through the alias carry `x-darkroute-upstream: https://darkroute.ai`.

#### The front door, `_middleware.ts`

Runs before any handler, for every `/api/v1/*` request. Nothing under
`/cameras/*` or `/api/admin/*` passes through it.

| Concern | Behaviour |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CORS | `access-control-allow-origin: *`, `access-control-allow-methods: GET, HEAD, POST, PUT, OPTIONS`, `access-control-allow-headers: content-type`, `access-control-max-age: 86400`. `OPTIONS` is `204`. No credentials, ever |
| Methods | From `_routes.ts`. A real path asked with a method it does not take is `405` JSON `method_not_allowed` with `allow`. A path that is not a route, with any method, is `404` JSON `not_found` whose `available` lists every route as `METHOD /path`. The app's HTML is never the answer |
| HEAD | Answered by a same-origin `GET` of the same URL: its status and headers, no body, `content-length` removed. The HEAD is not counted against the limit; the GET it makes is |
| Rate limit | 60 requests per 60 s per `CF-Connecting-IP`, **per isolate**, 10,000 addresses tracked. `ratelimit-limit`, `ratelimit-remaining`, `ratelimit-reset` on every pass-through; `429` JSON `rate_limited` with `retry-after` and `retryAfterSeconds` when spent. A speed bump that stops one loop on one machine, the file says so, and so does this. The per-request caps below are the real protection |
| Errors | Every refusal is `application/json; charset=utf-8`, `cache-control: no-store`, body `{ "error": <stable code>, "detail": <a sentence> }`. Codes are enumerated per response in the spec |
| Caching | A `200` from the shared `json()` helper is `cache-control: public, max-age=300` unless the handler says otherwise. Edge status is `DYNAMIC`: Function bodies are not cached at the edge, only by clients |
...
| `GET /api/v1/monitoring` | optional `kind` and `bbox=west,south,east,north` | `{ schema, generatedAt, sources, records, query, count, total }`; source counts describe the unfiltered inventory | `400 bad_kind · bad_bbox`; `503 monitoring_unavailable · monitoring_malformed` | 300 s |
| `GET /api/v1/monitoring/image` | known inventory `id` | Publisher image, bounded to 5 MiB; redirects and unapproved source URLs refused | `400` invalid ID; `404` unknown camera or no image; `502` invalid image; `503` unavailable inventory/image or refused source | no-store |
...
| `GET /api/v1/doc/{name}`                  | `name` ∈ `data-contracts, taxonomy, legal, transparency, architecture, auditing, threat-model, data-provenance, api, security, terms`                                                                                                                                                                                                                                                      | `text/markdown; charset=utf-8`, the file from the **public mirror** `darkcodelabs/darkroute`, branch `main`, `docs/public/`, fetched server-side (edge-cached 900 s) and untouched. `x-darkroute-source` names the raw URL. Edits to this file reach the endpoint only once the public seed is pushed | `404 unknown_document` (lists the names); `502 document_unavailable`                                                                                                                                                  | 900 s |
| `POST /api/v1/submit`                     | JSON: `kind` ∈ `correction, new-camera, removed` **required**; `cameraId` ≤ 64 chars of `[A-Za-z0-9:_-]`, required unless `new-camera`; `lat` −85..85 and `lon` −180..180, required for `new-camera`; `field` ∈ `operator, brand, ownerType, direction, position, mount, gone, other`; `wrong`, `right` ≤ 120; `note` ≤ 600; `photoKey` a key `/photo` issued; `contact` ≤ 120, optional | `{ ok: true, url, branch, duplicate, note }`. Creates branch `submission/<kind>-<sha12>`, file `submissions/<kind>/<id>.json`, and a PR titled `[unreviewed] …` on `SUBMISSIONS_REPO` (default `darkcodelabs/darkroute`). Same content twice → one branch, `duplicate: true`                                    | `400 bad_json · bad_submission`; `429 rate_limited` (its own limiter: **6 per hour per address**, `retry-after`); `502 repository_unavailable · branch_failed · write_failed · pull_request_failed`; `503 submissions_unconfigured` | `no-store`                                   |
| `PUT /api/v1/photo` (also `/api/v1/photo/`) | Body is the image. `content-type` ∈ `image/jpeg, image/png, image/webp` (bare media type; parameters ignored). 1 byte to **6 MiB**. Key = first 32 hex of SHA-256(bytes) + extension; nothing else is recorded. **EXIF is not stripped here**, the app strips it on the device before upload | `{ ok: true, key, bytes }`                                                                                                                                                                                                                                                                                       | `400 empty`; `413 too_large`; `415 unsupported_type`; `503 photos_unconfigured`                                                                                                                                      | `no-store`                                   |
| `GET /api/v1/photo/{key}`                 | `key` must match `^[0-9a-f]{32}\.(jpg\|png\|webp)$`, else `404` with no object read | The bytes under their stored `content-type`, with `content-security-policy: default-src 'none'; sandbox` and `x-content-type-options: nosniff`, a stranger's bytes are served as an inert object | `404 not_found`; `503 photos_unconfigured`                                                                                                                                                                            | `public, max-age=31536000, immutable`        |

Three things a caller should know about the data behind it.

1. Reads go through the app's own route. `cameras.ts` issues same-origin
   subrequests to `/cameras/11/{x}/{y}.json` (sequentially, never `Promise.all`)
   and `stats.ts`/`abuse.ts` read `/cameras/index.json` and
   `/records/counties.json` the same way, so the API inherits the tile cache and
   the generation protocol and **can never serve a generation the app would not**.
   A tile `404` is counted in `emptyTiles`; any other failure is `503`, never a
   partial answer.
2. An empty answer means no camera is mapped there. Not that none is
   present. `stats.caveat` says so in the payload, so a consumer who never reads
   this file still receives the warning.
3. The write routes never touch the archive. A submission is a pull request
   a person merges or does not; a photo is an object a pull request links to.
   The published data changes only when a generation is published (§4.3).

Cross-origin reads. `/api/v1/*` carries `access-control-allow-origin: *`;
...
```bash
ORIGIN=https://darkroute.ai
curl -s $ORIGIN/api/v1/stats | jq '{cameras, generation, generatedAt}'
curl -sI $ORIGIN/api/v1/stats | grep -i '^content-type'        # application/json - HEAD is real
curl -s -o /dev/null -w '%{http_code}\n' "$ORIGIN/api/v1/nonexistent"   # 404, and the body is JSON
curl -s -X DELETE -o /dev/null -D - https://darkroute.ai/api/v1/stats | grep -i '^allow'   # GET, HEAD, OPTIONS
curl -s https://api.darkroute.ai/v1/openapi.json | jq '.servers[].url'     # both hosts, via the proxy
curl -s https://darkroute.ai/api/v1/openapi.json | jq '.paths | keys'       # exactly the table above
```

The unit tests under `functions/api/v1/*.test.ts` and `apps/desktop/functions/*.test.ts`
hold every row above against the code: `pnpm run test:functions` and
`pnpm --filter @fwm/desktop test:unit`.

### 1.4 `api.darkroute.ai`, the developer console and its proxy

Project: a second Cloudflare Pages project, built from `apps/desktop/`
...
the method, `accept`, and `CF-Connecting-IP` (so the canonical rate limiter
buckets the real caller, not the proxy); for `POST`, `PUT` and `PATCH` the body
streams through with its `content-type` and `content-length`. Nothing else the
caller sent is forwarded. Redirects are not followed (`redirect: 'manual'`).
The upstream's headers come back untouched, the rate-limit ones included  -
plus `access-control-allow-origin: *` and `x-darkroute-upstream`. It
...
canonical origin cross-origin; its "try it" box fetches relative `/api/v1/...`
paths, which the proxy makes real.

No workflow in this repository deploys the console; it is deployed by hand
from `apps/desktop/` -- `pnpm build`, then `wrangler pages deploy dist
--project-name darkroute-console` -- so that its `functions/` directory is
picked up alongside the bundle.
...
degrade those three silently instead of raising the sign-in banner. That is
...
> `tiles.darkroute.ai` host. A PMTiles range request is a byte offset instead of a `z/x/y` path, so
...
### 2.6 the four requests the client makes on purpose

This section used to open "the shipped client has no upload path". Four
endpoints have shipped since, and the sentence is corrected instead of left
standing. Every one of them is behind a press, and none of them runs on a timer,
a queue drain, app start, or a keystroke.

| Route | Carries | Fired by |
| --- | --- | --- |
| `POST /api/v1/submit` | a correction a driver typed, plus any contact they chose to give | the send key on the correction sheet |
| `PUT /api/v1/photo` | a photograph, re-encoded through a canvas so no EXIF block: GPS included, survives; refused unless `metadataStripped === true` | attaching a photo to that correction |
| `GET /api/v1/place?q=` | a place name the driver typed, and a 2°-wide box around them as a preference | the "look this up as a place" key, under the on-device results |
| `GET /api/v1/route?from=&to=&avoid=` | the driver's position, their destination, and up to 60 reader coordinates to route around | the avoid key on the destination card |

The last two are proxies on purpose. Calling Nominatim and Valhalla from the
phone would hand a third party the driver's IP address alongside the name of
where they are going, the exact pair an ALPR network exists to assemble.
Through `functions/api/v1/place.ts` and `functions/api/v1/route.ts` those
services receive a request from one server. Places are edge-cached by query
alone, so two drivers asking the same thing are one upstream request; **a route
is `no-store` and is never cached**, because unlike a place name it is one
driver's origin and destination.

**The server's own bounds on the two writes**, so a reader of this section does
not have to open §1.3: a photo is 1 byte to 6 MiB and one of `image/jpeg`,
`image/png`, `image/webp`, keyed by its bytes; a submission's every field is
bounded and validated before anything is created, and one address may file six
an hour. Neither touches the archive, a submission is a pull request.

Search itself is none of these. Typing matches against the cameras already
on the phone; `features/drive/SearchBar.test.tsx` spies on the global `fetch`
and asserts `not.toHaveBeenCalled` through a fully typed query.

Which readers are on a route is computed on the device
(`services/route/corridor.ts`), so the server is never told which cameras a
driver cared about.

### 2.7 What the client still never sends
...
- Analytics, crash reporting, tag managers, ad SDKs. None. No beacon, no
  error sink, no tag manager, in any build. Every request the client does make
  goes through an injected `fetchImpl` whose default is `globalThis.fetch`, so
...
**Circuit breakers**, this script will refuse to write instead of publish a
hole:
...
sync fails before publication instead of seeding a knowingly behind pointer.
...
if it exists, the script throws instead of overwrite (`:233-243`).
...
It runs in GitHub Actions instead of as a Worker Cron Trigger on purpose: a
...
exactly `refs/heads/main`. Scheduled runs also require
...
The gate reads the live `workbox.globPatterns` value in `apps/pwa/vite.config.ts`.
...
| Name | Kind | Used by |
| ------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `CAMERA_TILES`      | R2 bucket binding | `functions/cameras/[[path]].ts`; read indirectly by `/api/v1/cameras` and `/api/v1/stats`                        |
| `SUBMISSION_PHOTOS` | R2 bucket binding | `functions/api/v1/photo/[[key]].ts`. Absent → `503 photos_unconfigured`, nothing stored |
| `SUBMISSIONS_TOKEN` | secret: a fine-grained GitHub token with contents and pull-requests write on the target repo | `functions/api/v1/submit.ts`. Absent → `503 submissions_unconfigured`, nothing queued |
| `SUBMISSIONS_REPO`  | plain text, optional | `functions/api/v1/submit.ts`, default `darkcodelabs/darkroute`                                                   |
| `NOMINATIM_URL`     | plain text, optional | `functions/api/v1/place.ts`, default `https://nominatim.openstreetmap.org/search`                                |
| `VALHALLA_URL`      | plain text, optional | `functions/api/v1/route.ts`, default `https://valhalla1.openstreetmap.de/route`                                  |

Each of these is named in a source file that ships in the public tree, and each
route says plainly when it is missing instead of pretending to have done
something. The developer console project (§1.4) needs no bindings and no
variables: it proxies.
...
| -------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tile bodies, `index.json`, `tombstones.json` | **Scheduled hourly** (§4.2, `cron: '10 * * * *'`), but the schedule is an intention and `GET /api/v1/stats` is the fact: read `generatedAt` and `upstream`. On 2026-09-09 the live generation was dated 2026-09-01 because every `Camera sync` run that day failed in `scripts/hydrate-cameras.mjs` (no generation pointer in the bucket) | Expect an immediate cached response followed by service-worker revalidation; cached entries expire after 7 days. Never infer freshness from the cron |
| The public API's response shapes (§1.3) | With the app. `openapi.json` `info.version` moves when a shape changes (`1.1.0` as of this revision) | Pin the fields you use and validate them. Fields are added; existing ones are not removed or renamed without a version bump |
...
- **`api.darkroute.ai` as a second implementation.** It is a proxy (§1.4). If
  an answer there ever differs from `darkroute.ai/api/v1`, the canonical origin
  is right and the proxy is broken; `x-darkroute-upstream` on every proxied
  answer says where to look. The per-isolate rate limit is shared, not doubled.
...
rg -n 'https?://[^"'"'"'`) ]*' apps/pwa/src --glob '!*.test.*'
 ...
 # Every route the site serves from code - the apex, then the console host.
find functions apps/desktop/functions -name '*.ts' -not -name '*.test.ts'

 # The route table the middleware gates by and the spec is tested against.
cat functions/api/v1/_routes.ts

 # The public API, against the live origin (see also §1.3).
curl -s https://darkroute.ai/api/v1/openapi.json | jq '.paths | keys'
curl -sI https://darkroute.ai/api/v1/stats | grep -i '^content-type'
