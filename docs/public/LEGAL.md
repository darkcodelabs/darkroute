...
That framing is not spin, it is the structural fact for everything below.
...
**In brief: there is strong support for yes, and no case squarely on point.
...
is about consequences instead of argument.

### What follows

The right posture is not "we are safe". It is: the activity has strong
constitutional support, the realistic threat is cost instead of loss, the
...
Two consequences a reader should be told instead of left to discover:
...
- `@meshtastic/js@2.6.0-0` is a prerelease and is **marked deprecated on npm**
  ...
  in under a minute and it should be acknowledged instead of discovered.
...
| §4.5(c) | Internal use within an organisation is not public use | A internal build is out of scope. A public beta is not obviously internal: see the flag below |
...
§4.5(c) if the beta is internal or by §4.7(b) parallel distribution if
...
counts as "internal" under §4.5(c) is unclear and should not be
assumed.**
...
for exactly this purpose, named so they can be copied instead of described:
...
The definition worth quoting to anyone who asks what turns on it: reproducible
