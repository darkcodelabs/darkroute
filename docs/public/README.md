...
| If you are… | Read |
|---|---|
| somebody who wants the whole product explained once, with diagrams | [`WHAT-DARKROUTE-IS.md`](./WHAT-DARKROUTE-IS.md), the long-form writeup |
...
### [`WHAT-DARKROUTE-IS.md`](./WHAT-DARKROUTE-IS.md), the whole product, once

The long-form writeup: what the app is and is not, the architecture as
diagrams, the camera archive and the provenance mechanism that has never run in
production, navigation mode, the LoRa link, the UI's locked geometry, every
outbound request, the quirks, and the commands to check all of it. Every
technical claim cites the file that makes it true, and
`scripts/writeup-currency.test.mjs` fails CI when a cited path stops existing.
Section 10 says which section to update when which code changes.
...
that leaves the device**, deliberately placed near the top instead of at the
...
Surveillance as a join instead of a field map. Eleven working recipes for
...
claim on the record instead of an absence of one.

### [`LEGAL.md`](./LEGAL.md), the legal position

What is settled, what is unsettled, and the things this project will
...
| [`/the design-gap ledger](../../the design-gap ledger) | Decisions deliberately not made yet, referenced by `GAP:` markers in the source |
