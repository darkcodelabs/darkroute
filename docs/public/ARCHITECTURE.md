...
| The data pipeline | `scripts/*.mjs`                               | Node scripts, run by a human or by GitHub Actions. `fetch-cameras.mjs` bootstraps; `hydrate-cameras.mjs` restores one R2 generation and its state; `sync-cameras.mjs` applies replication diffs; `publish-cameras.mjs` atomically activates the new generation. |
...
| The notices archive | `transparency/`                               | Every legal demand received, published in full, modelled on `github/dmca`. Count is currently 0, and the count is a statement instead of an absence. |
...
redirect is a failed request instead of an empty square. The production camera
...
and "we could not ask" remains structural in a car.
...
| 15 | `POST /api/v1/submit`                                | a correction a driver typed, and any contact they chose to give | one press on the correction sheet; opens a public pull request and writes nothing to the archive | `services/share/shareCorrection.ts:160`, `functions/api/v1/submit.ts`                                                  |
| 16 | `PUT /api/v1/photo`                                  | a photograph, re-encoded through a canvas so no EXIF block: GPS included, exists in it | refused unless `metadataStripped === true`; the strip happens on the device before the bytes reach the service | `services/share/shareCorrection.ts:125`, `features/report/preparePhoto.ts`                                             |
| 17 | `GET /api/v1/place?q=`                               | **a place name you typed**, and a 2°-wide box around you as a preference | **one press**, on a key that says what it sends. Never fires while typing, on blur, or because the local search came back empty | `features/drive/SearchBar.tsx`, `services/route/planRoute.ts`                                                          |
| 18 | `GET /api/v1/route?from=&to=&avoid=`                 | **your position AND your destination in one request**: the most sensitive pair this app computes | **one press**, on the destination card. No timer, no replan on movement, no route planned on your behalf | `features/drive/DriveScreen.tsx`, `services/route/planRoute.ts`                                                        |

### Search runs on the phone; routing does not, and the difference is a press

Typing sends nothing. The search bar matches against the cameras already
cached on the device, through the same `searchCameras` LOOKUP uses, street,
cross street, operator, id and the landmark index, so "home depot" finds the
readers around one. There is no autocomplete request, no geocoder call, no
search-as-you-type. `features/drive/SearchBar.test.tsx` spies on the global
`fetch` and asserts `not.toHaveBeenCalled` through a fully typed query, because
an autocomplete is a keystroke logger you have agreed to, and a comment does not
stop one being added later.

**The phone holds cameras, not addresses**, so "take me to 1420 Baltimore Ave"
is a question it cannot answer. Row 17 is the answer, and it is a **key somebody
presses**, under the local results, labelled with what it sends and where.

Both requests are proxied on purpose, and the reason is the whole product.
The obvious build has the phone call Nominatim and Valhalla directly. That hands
a third party the driver's IP address alongside the name of where they are
going, which is precisely the pair an ALPR network exists to assemble. Through
`functions/api/v1/place.ts` and `functions/api/v1/route.ts` those services see
one server asking about a place; they do not see a person. The cost is that we
see the query instead, which is why neither Function logs one, why places are
cached at the edge by query alone, so two drivers asking the same thing are one
upstream request and neither is distinguishable in it, and why a **route is
never cached at all**.

Which readers are on your route is computed on the device. The route shape
comes back from the server; the cameras were already here.
`services/route/corridor.ts` pairs them locally, so the server is never told
which readers a driver cares about and never sees the answer.

The maps-app handoff for routing is gone. "Route around all 9" used to open
Google Maps with the readers as via-points. Two things were wrong with that: it
posted the driver's origin and destination to a maps company, and a via-point
says "go through here" when what a driver wants is "do not go through there".
Each reader is now an `exclude_polygons` entry the router may not enter, and the
line is drawn on DarkRoute's own map.
...
the same diffs instead of skipping past them.
...
session lives at module scope instead of in a component, so walking to RADAR
...
2. IndexedDB: database `fwm`, thirteen stores (`services/db/schema.ts:58`),
   ...
   migration creates makes `openFwmDb()` refuse at startup instead of fail at
   the first read.
...
- The basemap is cross-origin. PMTiles archives at `tiles.darkroute.ai`
  ...
  offline map coverage depends on the browser's own HTTP cache instead of on
...
Several capabilities are absent on purpose, and the absences are structural
rather than incidental. Where the refusal is enforced by a test instead of by a
