...
Absence of a camera on the map means nothing at all. Of the 139,918 cameras in
the archive, 116,152, 83%, carry no `operator` tag at all and are classed
`unverified` (`scripts/fetch-cameras.mjs:481-490`; most of them do carry a
`manufacturer` tag, 97,284 of the unverified ones say Flock Safety, and
113,562 of the whole archive do). A blank stretch of
road is a statement about the data and never a statement about the road. Drive
as though there is a reader there.
...
Right now the published archive holds 139,918 cameras across 8,803 tiles, built
on 1 September 2026. That makes it eight days old as this is written, because the
hourly sync is broken. Not "was", is. Of its last twenty scheduled runs,
twelve failed and eight were skipped, and none succeeded; counting the manual
re-runs as well, nineteen of the last twenty failed. The last success was
2026-09-01T18:25Z. The app prints the archive's age next to the camera count for
exactly this reason (`apps/pwa/src/services/cameras/useCatalogueUpstream.ts:49-50`:
LIVE under six hours, BEHIND to forty-eight, STALE after). I would rather show
you a stale number than a confident one.
...
| Where the cameras come from | OpenStreetMap, ODbL. 139,918 cameras, 8,803 z11 tiles, upstream watermark `2026-09-01T18:00:00Z` (`apps/pwa/public/cameras/index.json`) |
...
| Where they live on the phone | IndexedDB, up to 512 tiles, hard expiry 30 days, stale after 24 h (`apps/pwa/src/services/db/policy.ts:34-52`); service worker cache `fwm-camera-tiles-v2` for 7 days (`apps/pwa/vite.config.ts:715`) |
...
| The provenance mechanism | sha256-pinned generations behind a compare-and-set pointer: built, gated, tested, **never run in production** (§3) |
...
flowchart TD
  ...
  subgraph PIPE["Operator pipeline, scripts/*.mjs"]
    ...
    REV["propose-deflock-source-review.mjs<br/>receipt, always unapproved"]
...
The whole archive does reach the phone in one piece as `overview.json`, 139,918
flat coordinates, about 4 MB, for the national map and the POI export.
...
- **The edge is not currently caching the tiles.** The Function sets
  `cache-control: public, max-age=3600, must-revalidate`
  (`functions/cameras/[[path]].ts:60`), but three consecutive live requests on
  2026-09-09 answered `cf-cache-status: DYNAMIC`. The cost argument in the
  Function's header is not being realised; the privacy argument does not depend
  on it.

### 3.2 the generation pointer, and why it has never been used

**The archive is meant to be content-addressed, and in production it is not.**
This paragraph is a correction instead of a deletion, because what it used to
say is the most instructive thing in the document.
...
write of the pointer (`scripts/publish-cameras.mjs:1113-1114`: `IfMatch` on the
current pointer's ETag, or `IfNoneMatch: '*'` for the first pointer ever
written) so a half-published generation cannot be pointed at. A generation must carry at
...
generation. An archive swapped underneath a device becomes a *detectable event*
...
curl -sD- https://darkroute.ai/cameras/index.json -o idx.json | grep -i camera-generation
 # x-darkroute-camera-generation: 0e1b7974e8086ef621c0a3f574962cd9d06108e29b442200c1e8007b2b0c6bdd
sha256sum idx.json
 # 0e1b7974e8086ef621c0a3f574962cd9d06108e29b442200c1e8007b2b0c6bdd  idx.json
wc -c idx.json
 # 397 idx.json
```

**The generation id is the hash of a 397-byte index file, and nothing else.** It
pins that one object. The other 8,800-odd are not covered by it. That is the
serving code's documented fallback for "no generation has ever been published"
(`functions/cameras/[[path]].ts:221`, `readFlatGeneration`): `readPointer`
returns null, the flat root answers, and the header is `sha256(index.json)`. The
Function re-reads the pointer and index on every request, deliberately
unmemoised, so the moment a pointer is published every tile switches. No
generation ever has been. A client comparing that header against its cached
tiles learns nothing about the tiles.
...
sequenceDiagram
  ...
  Note over P,R2: designed. Has never completed in production.
  D->>F: GET /cameras/11/x/y.json
  F->>R2: GET __camera/current.json
  R2-->>F: absent
  F->>R2: GET index.json (flat root)
  R2-->>F: 397 bytes
  F-->>D: tile + x-darkroute-camera-generation: sha256(index.json)
```

The same correction applies upstream. The design is a **retained-response
ledger**: every raw Overpass response kept and hashed, the ledger recording
which response authorised which tile, so the dataset can be rebuilt from the
exact bytes upstream returned (`scripts/capture-deflock-source.mjs`). The code
is in the repository. The artifacts are not:

```bash
git log --all --oneline -- scripts/data/deflock-us-overpass-response-ledger.json
git log --all --oneline -- scripts/data/deflock-us-overpass-responses.bundle.gz
git log --all --oneline -- scripts/data/deflock-us-source.geojson.gz
 # no output: zero commits, ever, for all three
```

And the archive that is actually being served says so about itself, in its own
body (`apps/pwa/public/cameras/index.json`, same bytes live):
`"source": "OpenStreetMap (ODbL), via the DeFlock hourly build + our own archive"`.
Derived from DeFlock's build, not from a first-party capture whose bytes were
retained.

### 3.3 the review receipt, and why nothing can fake it

A capture becomes a trust root only through a review receipt, and
`scripts/propose-deflock-source-review.mjs` writes every receipt as `unapproved`
by construction. The receipt in the tree (`scripts/data/deflock-us-source-review.json`)
is schema `darkroute-deflock-source-review/v2` with `sourceWatermark.status:
"unapproved"`, `responseLedger: null` and `replicationFloor: null`, and the
reason is written into the receipt itself: the fetch run did not retain
`osm3s.timestamp_osm_base` for its constituent Overpass responses, so nothing
can authorise a replication floor. Only a human edit of one field can change
that, and no script will. An earlier version of this text said the validator
rejects v2 outright; it does not. It rejects *this* receipt, for the quoted
reason (`docs/camera-sync-runbook.md`, `validateSourceReview` in
`scripts/fetch-cameras-deflock.mjs`).

The validator threw away three finished national captures before the bugs
turned out to be in the validator instead of the captures (fixed 2026-09-09,
"accept a capture with 0..2 subtraction responses"). A validator that has never
approved a generation is a gate nothing has passed, which is a different thing
from a chain of custody. Fail-closed is the right default and it is not the same
as provenance.

### 3.4 Hourly replication, and why it is failing

`.github/workflows/camera-sync.yml` runs at ten past every hour (`:36-39`), gated
on the repository variable `FWM_CAMERA_SYNC_ENABLED == 'true'` and on the
`main` branch of the private repository (`:64-68`). It does not poll Overpass  -
`scripts/sync-cameras.mjs` refuses to, because Overpass's usage policy names
country-scale scheduled polling as prohibited, it consumes OSM's hourly
replication diffs, driven by DarkRoute's own id set instead of by tags, because
a `<delete>` in OsmChange carries no tags and a tag filter would see zero camera
removals forever (`sync-cameras.mjs`, header). Removals become tombstones;
`scripts/attest-camera-continuity.mjs` builds the proof a versioned generation
must carry.

Of the last twenty scheduled runs on 2026-09-09, twelve failed and eight were
skipped (a skip is the `if:` gate above evaluating false); of the last twenty
runs of any kind, manual re-runs included, nineteen failed and one was skipped.
Every failure is the same line, from the step "Hydrate the published camera
archive":

```text
camera generation pointer is absent; hydration has no canonical snapshot
```

`scripts/hydrate-cameras.mjs:689-690` throws it because there is no pointer to
pin, because no generation has been published, because the receipt is
unapproved. The sync is not broken by accident; it is refusing to replicate on
top of an archive it cannot name. `scripts/camera-sync-state.json` still records
`lastAppliedSeq 122459`, `lastAppliedTimestamp 2026-09-01T18:00:00Z` and
`versionsKnown: false`.
...
Measured over `apps/pwa/public/cameras/11/` on 2026-09-09 (the local tree and
the live archive are the same generation):

| Class | Cameras | Share |
|---|---:|---:|
| `unverified` | 116,152 | 83.0% |
| `police` | 14,868 | 10.6% |
| `private` | 6,497 | 4.6% |
| `inter_agency` | 2,181 | 1.6% |
| `hoa` | 220 | 0.2% |

`operator` is present on 17.0% of records; `manufacturer` on about 95% (Flock
Safety 113,562; Motorola 7,061; Genetec 3,520; Axis 1,798; Leonardo 1,177). So
the archive knows *what* most of these cameras are and does not know *whose*
they are, and the class labels say so on their face: `UNVERIFIED REPORTS`
(`apps/pwa/src/features/triage/triage.ts:70-76`).
...
Every tile in the tree carries `attribution`, `licence` and `licenceUrl`
(`scripts/backfill-camera-licence-url.mjs`; required by the generation validator,
`camera-generation.mjs:525`). The *served* tiles carry `attribution` and
`licence` only, one more sign that the live archive is not a published
generation, and a licence obligation the next publish closes.
...
`--fwm-glass-rim`, two inset highlights instead of a drop shadow; the border
...
- **The map does not follow the chrome theme.** Cameras, routes and every alert
  hue were tuned against one ground (`apps/pwa/src/stores/settings.ts:545`).
...
git log -1 --format='%H %ci %s'
 ...
 # 1. The live generation header is the hash of the 397-byte index, and nothing else
curl -sD- https://darkroute.ai/cameras/index.json -o idx.json | grep -i 'camera-generation\|cf-cache-status'
sha256sum idx.json && wc -c idx.json

 # 2. The retained-response ledger has never been committed
git log --all --oneline -- scripts/data/deflock-us-overpass-response-ledger.json \
  scripts/data/deflock-us-overpass-responses.bundle.gz scripts/data/deflock-us-source.geojson.gz

 # 3. The review receipt is unapproved, and says why
node -e "const r=require('./scripts/data/deflock-us-source-review.json');console.log(r.schema,r.sourceWatermark)"

 # 4. The hourly sync's last twenty runs, scheduled only and then all kinds (private repository; needs access)
gh run list --workflow camera-sync.yml -R the private working repository --event schedule --limit 20
gh run list --workflow camera-sync.yml -R the private working repository --limit 20
gh run list --workflow camera-sync.yml -R the private working repository --status success --limit 1
...
matrix takes when it is marketing. So it was audited against source rather than
...
- **The comparison file** (private) went on claiming the
  ...
  staleness warnings now, and `docs/STALENESS.md` exists because the first did
...
- Mobile and trailer-mounted units move. A static archive is structurally
  behind them.
- The archive is eight days stale as of 2026-09-09, and the sync that should fix
  it has not succeeded since 2026-09-01T18:25Z, for the reason in §3.4.
- The generation-pinning and retained-ledger mechanisms have never run in
  production. The live archive is DeFlock-build-derived and its generation id
  pins one 397-byte file.
- The served tiles lack `licenceUrl`; the tree's tiles have it.
- `continuity.json` answers 503 on the live host; the client never requests it
  (`apps/pwa/src/services/cameras/generation.ts:22-27`), so there is no user
  impact, and it is a required sidecar the next publish must carry.
