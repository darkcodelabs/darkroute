...
named so you can check the claim instead of trust it.
...
| `subject_position`        | `{lat, lon} \| null`                          | yes | The camera's position, when established. `null` until somebody establishes it, and `null` is the correct answer, never a copy of `observer_position`. |
| `subject_position_source` | `"projected" \| "placed" \| "record" \| null` | yes | `null` exactly when `subject_position` is `null`. |
| `synthetic`               | `boolean`                                     | no | `true` when the fix came from the scripted demo drive instead of a radio. Signed, see below. |
| `gps_accuracy_m`          | `number \| null`                              | yes | Metres. Spelled to match `GPS_ACCURACY_FIELD` in `chain.ts`, so `EvidenceRecord.gpsAccuracyM` is a _projection_ of this number instead of a second copy that could drift. |
...
It is domain-separated instead of all-zeros so that a genesis link can never
...
`404` is what the client reads as "empty" rather than as an error
...
`coords` is `[lat, lon, lat, lon, …]`, length `2 × count`. Flat instead of nested
...
silently", and it is the intended readout instead of an accounting bug.
...
also has to model _"we have stopped trying"_, which is what `dead_letter`
...
This is the subtlest contract in the codebase and it deserves a plain statement.
...
which is why the sheet can state as fact instead of as hope that the location
...
purpose** (`overrideFrequency`). `setLora` throws instead of guessing if the node
...
`presetCode()` reverse the same table instead of maintaining a second one, because
...
This is documented instead of silently corrected because this document is meant
...
Every field the driver left blank is absent instead of defaulted. A tag nobody
...
`unsure` maps to nothing because a driver at speed may not know, and
...
came from the ground instead of from imagery.
...
coordinates are skipped instead of ranked as `NaN`.
...
`editorUrl` returns `null` for a non-finite position instead of linking to null
island.

## 7. Building DarkRoute data into other data

The published files are deliberately boring JSON so that turning them into
something else is a short script instead of a project. Full mappings, GeoJSON,
...
Stated here instead of left for a reader to discover, because a contract document
