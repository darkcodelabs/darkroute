...
This is the structural claim and the one you cannot check from inside the app.
...
disclosive instead of only the ones that are not.
...
real, they are in the code, and they are documented instead of patched quietly
...
Every camera has an OSM node id, so this claim is auditable; it is
...
sum the batches instead of asking for one enormous call.
...
[`DATA-CONTRACTS.md` §1 and §2](./DATA-CONTRACTS.md#1-fwm-canonical-jsonv1-canonicalisation), canonical JSON in six rules, two hashes, one signature.
...
263 lines guarding the claim that the NODE screen listens instead of transmits.
...
| a store taking zustand's default storage | the default port is explicitly **non-durable** and says so, so a store that was never given a durable port reports that instead of pretending it saved |
...
with no IndexedDB claiming a successful wipe instead of saying nothing was ever
stored.
