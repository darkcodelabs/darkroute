...
If you are here to audit instead of to contribute, you want
...
- Deliberately unimplemented paths use a `GAP:` marker pointing at the note that
  ...
  ```text
  GAP: dead-drop / an-export-carries-bodies-not-rows
  ```

  A marker names a real decision, in words a reader can act on. If you add a
  marker, record the decision in the same commit.
...
When the code departs from the design, say so and why, in a GAP marker at
the line that departs.
...
the enforcement so you can check instead of remember.
...
durable port says so instead of pretending it saved.
