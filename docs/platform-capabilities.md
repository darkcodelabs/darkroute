...
`NO GPS · last fix 40s ago` state can be drawn from the platform's own timestamp instead of from
...
The shell is not on Google Play and has no `ACCESS_BACKGROUND_LOCATION`
permission, so it does not promise alerts after the app is fully backgrounded
or killed.

It **does** have notification delegation, which this document previously said
it did not. Nothing in `apps/android/` asks for it; it comes down the class
chain. `AndroidManifest.xml` exports `.ExtraFeaturesService` under the intent
filter `android.support.customtabs.trusted.TRUSTED_WEB_ACTIVITY_SERVICE`; that
class `extends DelegationService`; and in `androidbrowserhelper:2.7.3`,
`DelegationService extends androidx.browser.trusted.TrustedWebActivityService`, the class that implements `onNotifyNotificationWithChannel`, with a
constructor that registers `NotificationDelegationExtraCommandHandler`.
`apps/android/README.md` carries the `javap` command that shows both.

In practice: a card for this origin is posted by the `ai.darkroute.app`
package instead of by Chrome, so the shade attributes it to DarkRoute and
draws DarkRoute's app name and icon; the posted card belongs to this package's
`NotificationManager`, so it survives the app being backgrounded; and the
haptic pattern carried on it is run under this package's
`android.permission.VIBRATE`, which the manifest now declares.

That changes who owns a card, not what causes one. Something still has to call
`show()`, and that something is the page. The killed-app sentence above rests
on that, not on delegation being missing.
...
Notifications (`services/adapters/notifications.ts`) implement section 06's rule as far as the
platform allows: a channel label per state, and **one shared tag** for every camera alert so a
replacement replaces instead of stacks. Abuse-area, watchlist and navigation get their own tags
precisely so they cannot evict a live camera alert. `clear` and `approaching` post silently and
`in_range` and `multiple` may make noise; watchlist is **always** silent, and abuse-area is
deliberately not - a driver who asked to be told they are entering a jurisdiction with a record is
not told by a silent card.

The channel is a **JS-side label only**. `data.channel` was documented for years as the thing an
Android notification channel keys off and never was; the next section is the ceiling that explains
why, and it is the reason `silent`, `renotify`, `vibrate` and the tag are the whole lever set.
...
## The ceiling on a delegated notification

Written down so the next person does not have to rediscover it by disassembling a jar. Delegation
(limit 5) changes *who posts* the card. It does not widen *what the card can say*, and the boundary
is sharper than it looks. Each fact below is labelled by how it was established.

- **Chrome builds the `Notification`; androidx only re-posts it.**
  *Platform-knowledge, read off `androidx.browser:browser:1.10.0` in the Gradle cache.* The object
  arrives already built and parcelled over the AIDL.
  `TrustedWebActivityServiceConnection` carries exactly four keys - `KEY_PLATFORM_TAG`,
  `KEY_PLATFORM_ID`, `KEY_NOTIFICATION`, `KEY_CHANNEL_NAME` - and nothing else.
  `TrustedWebActivityService.onNotifyNotificationWithChannel` turns the name Chrome sent into an id
  (`name.toLowerCase(ROOT).replace(' ', '_') + "_channel_id"`), creates that channel at
  `IMPORTANCE_DEFAULT` if it does not exist, copies the notification onto it, and calls
  `NotificationManager.notify`.

- **Therefore a web page cannot name an Android channel.**
  *Verified in repo.* `NotificationChannel` in `services/adapters/notifications.ts` is a TypeScript
  label. `data.channel` was documented for years as "what an Android notification channel keys off"
  and never did anything of the sort: `notification.data` is an opaque structured-clone value handed
  back on `notificationclick`, and nothing reads it on the way out. Every DarkRoute card lands on the
  one channel Chrome names, so importance, sound and the user's per-channel switch in Android
  settings are **shared across all of them** - mute the abuse card and you have muted the camera
  alert.

- **On Android 8+ the channel owns vibration.**
  *Platform-knowledge.* `NotificationOptions.vibrate` is a request, not a guarantee; the channel's
  own pattern wins once the user or the OS has settled it. What is guaranteed is the difference
  between asking and not asking - and asking at all requires the posting package to hold
  `android.permission.VIBRATE`, which `apps/android/app/src/main/AndroidManifest.xml` now declares.

- **One lever needs no code: the status-bar stencil.**
  *Verified in repo (absent) plus platform-knowledge.* `TrustedWebActivityService` reads
  `android.support.customtabs.trusted.SMALL_ICON` meta-data off the service and hands the resource id
  back to Chrome. The manifest declares none, so `onGetSmallIconId()` returns `SMALL_ICON_NOT_SET`
  (-1) and Chrome's own choice stands. One drawable and one `meta-data` element would change that.

- **Everything else means native code.**
  *Platform-knowledge.* Per-state importance, a `WearableExtender`, an ongoing navigation
  notification that cannot be swiped away, or a custom `RemoteViews` layout are all on the far side
  of one method: override `onNotifyNotificationWithChannel` in `ExtraFeaturesService` and rebuild the
  card with `NotificationCompat` before posting it. There is no web-side spelling of any of them.

  **That is the fork in the road, not a bug.** `ExtraFeaturesService.java` says "There is
  deliberately no other native code", and gives the reason in the same comment: every rule about what
  the app may store, show or send lives in the web app, where it is testable, and duplicating one
  here creates a second place for the answer to be different. Taking the fork means accepting that
  cost knowingly, for a named capability. Nobody should arrive at it by accident while trying to make
  one card louder.
...
capability-level notes instead of a complete screen-reachability inventory.
...
| `cameraCapture.ts` | One still photo via the OS camera app (`<input capture="environment">`) | **None** - the OS camera app owns the decision, so `permission()`/`request()` are absent | `no document` / `this browser does not support file inputs`. Browsers without the `cancel` event never resolve on back-out; the promise stays pending until `abort()` - stated instead of guessed | Report sheet `ADD A PHOTO` tile (`ReportViewV1`), **live since 2026-08-31** - `ReportScreen` calls `capture({ facing: 'environment' })` and hands the result straight to `preparePhoto()`. The intel-card `DROP PHOTO OF CAMERA` key is still inert: a photograph is owned by a report id, and an intel card has no report to hang one on | Implemented + Mocked. **`CapturedPhoto.metadataStripped` is always `false`** - EXIF GPS is not removed here, so any upload path must treat such a photo as unsendable. Still true and still the right contract: the stripping is `features/report/preparePhoto.ts`, one layer up, which re-encodes through a canvas and returns a `PreparedPhoto` with `metadataStripped: true`. Only prepared bytes are ever stored |
