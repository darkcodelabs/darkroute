...
export default defineConfig({
  ...
  build: {
    ...
    /*
     * DO NOT PRELOAD THE MAP ENGINE.
     *
     * `React.lazy` defers when a chunk EXECUTES. It does not stop the browser
     * fetching it, and Vite emits `<link rel="modulepreload">` for every chunk
     * it can reach - so making `MapCanvas` and `MiniMap` lazy moved 917 KB off
     * the parse path and left it exactly where it was on the DOWNLOAD path.
     * Measured: maplibre still arrived before the onboarding headline painted.
     *
     * A preload is a hint that a chunk is needed NOW. For the map engine that
     * is false on onboarding, SETTINGS, LOOK UP and every other screen without
     * a map, and it competes for bandwidth with the shell on exactly the
     * connection where that matters. DRIVE fetches it when it mounts, which is
     * a request that begins a few hundred milliseconds later and no longer
     * blocks anything.
     *
     * `deps` is filtered rather than `modulePreload` being switched off: every
     * OTHER chunk still gets its hint, which is what the feature is for.
     */
    modulePreload: {
      resolveDependencies(_url: string, deps: string[]): string[] {
    ...
    rollupOptions: {
      output: {
        /*
         * MAPLIBRE IS ITS OWN CHUNK.
         *
         * The entry bundle was 1.62 MB because the whole WebGL map engine was
         * inlined into it, so every visitor downloaded, parsed and compiled it
         * before the first pixel of UI - including on ONBOARDING, which is the
         * first screen a new install sees and draws no map at all.
         *
         * Splitting it buys three things and costs nothing:
         *
         *   it downloads in PARALLEL with the shell rather than in front of it
         *   it is cached across deploys, because it changes when the library
         *     changes and not when our code does - today every app edit
         *     invalidated 1.6 MB for every returning visitor
         *   `MapCanvas` can defer it, which is the half that actually keeps it
         *     off the boot path
         *
         * A function rather than an object map: the object form matches on
         * chunk NAME and this needs to match on module path, since maplibre
         * arrives through several importers.
         */
        manualChunks(id: string): string | undefined {
          if (id.includes('node_modules/maplibre-gl')) return 'maplibre';
          /*
           * `pmtiles` gets its OWN chunk, not maplibre's.
           *
           * Grouping them looked tidy and undid the whole split: DRIVE's speed
           * lookup imports `pmtiles` on the boot path, so the shared chunk was
           * reachable from the entry and the browser fetched all 917 KB of map
           * engine with it - measured, after both components were already
           * lazy. One eager 200 KB dependency dragged the 700 KB one behind it.
           */
