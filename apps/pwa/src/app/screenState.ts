 ...
 * picture. See gap record radar-sweep-merge.
 *
...
export interface ScreenState {
  ...
  /**
   * THE SCREEN THIS ONE WAS OPENED FROM, or null on a cold entry.
   *
   * `BackKey` used to send every screen behind MORE to MORE, whatever route
   * had actually been taken to it - so DRIVE's gear opened SETTINGS and the
   * back arrow put the driver on a hub they had never seen. This is the one
   * fact that was missing, kept where the rest of the navigation is rather
   * than in a second model beside it.
   *
   * ONE STEP, not a stack. A stack would have to be unwound correctly on every
   * dock press, deep link and alert interrupt, and getting that wrong is how a
   * back arrow starts walking somebody through screens they never opened.
   * `BackKey` still declares a parent, and that parent is what this falls back
   * to. This only overrides it when the app knows better.
   *
   * IT RIDES IN `history.state` WITH THE REST OF THE ENTRY. It did not, and a
   * back gesture left it stale: MORE -> DOCS -> DRIVE -> back landed on DOCS
   * still remembering DRIVE as the screen it had been "opened from", so its
   * arrow pointed at the screen just left - or, when the stale value was DOCS
   * itself, at nothing. A screen id from the closed list is the one thing this
   * module is allowed to write there, so the entry carries it and popstate
   * reads it back.
   */
...
/**
 * Whether an overlay is still part of the navigation - presented, or held
 * aside by a live alert. Distinct from `topOverlay()`, which answers "what is
 * drawn": a surface that owns its own history entry needs to know whether that
 * entry is still there, and an alert moving it aside is not the entry going.
 */
export function hasOverlay(id: string, current: ScreenState = state): boolean {
  return (
    current.overlays.some((overlay) => overlay.id === id) ||
    current.savedOverlays.some((overlay) => overlay.id === id)
  );
}
...
interface HistoryEntry {
  ...
  readonly overlays: readonly Overlay[];
  readonly depth: number;
  /** See `ScreenState.from`. Null on an entry written before it was recorded. */
  readonly from: ScreenId | null;
}
...
function readHistoryEntry(raw: unknown): HistoryEntry | null {
  ...
  const from = isScreenId(entry['from']) ? entry['from'] : null;
  return { screen: entry['screen'], overlays, depth, from };
}
...
function onPopState(): void {
  ...
  // The entry's own `from`, not the one the screen just left had. A back
  // gesture is a return trip, and the screen it lands on must remember where
  // IT was opened from - not where the driver has just been.
...
export interface OpenScreenOptions {
  ...
  /**
   * This navigation IS a back key. Clears `from` instead of setting it.
   *
   * Without it, backing out of SETTINGS to DRIVE would leave DRIVE remembering
   * SETTINGS, and DRIVE's own back arrow would point at the screen the driver
   * had just left - a loop. Declared by the caller rather than guessed from the
   * ids, because "am I going backwards" is a fact about intent that the two
   * screen names cannot answer.
   */
...
export function openScreen(screen: ScreenId, options: OpenScreenOptions = {}): void {
  ...
  setState({
    ...
    // Going back does not leave a trail to follow back to.
    from: options.back === true ? null : state.screen,
  });
