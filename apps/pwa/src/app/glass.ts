...
/* =============================================================================
 * FLAT - the material's RELIEF, and a FIFTH axis rather than a value on any of
 * the four above.
 * =============================================================================
 * WHAT `glass=off` DOES NOT ALREADY GIVE YOU. `off` reads as "no glass", so it
 * is the first place a fifth step would go, and it is the wrong one: `off` sets
 * `--fwm-glass-filter: none` and stops. Read the tokens - the specular rim, the
 * sheen, the liquid edge, the cast shadow under the two DRIVE cards and the
 * meniscus on the report key all survive it, because not one of them is a
 * backdrop filter. They are paint. `off` exists to stop a per-frame read-back
 * over a live map, and paint costs no read-back, so it was never asked to
 * remove any of it. A panel with the blur off is still a lit, bevelled,
 * floating object, and that is the thing this control is about.
 *
 * WHY NOT A THIRD `liquid` VALUE, which is the closest call. `liquid=off`
 * already removes two of the four - the sheen and the lit edge - so
 * `flat | off | on` would read as a tidy relief ladder. It fails on two counts.
 * The smaller one is that the other two are not liquid's to remove:
 * `--fwm-glass-rim` is declared at `:root` and is on every glass surface
 * whatever liquid says, and `--fwm-card-shadow` belongs to the cards.
 *
 * The one that decides it is that a value CANNOT REMEMBER ANOTHER VALUE. Flat
 * is a modifier over whatever the rest of the material says; the driver who
 * turns it on has not stopped wanting liquid glass, and the driver who turns it
 * off again expects the sheen they chose to come back. Folded into `liquid`,
 * picking flat would overwrite that preference and turning it off would guess -
 * which is precisely the fight `data-fwm-clear` and `data-fwm-tone` were split
 * apart to prevent.
 *
 * NOT A `tone` VALUE EITHER: tone decides which CHANNEL the alpha lands on, and
 * flat has nothing to say about colour - a flat panel is still clear or tinted.
 * Not a `clear` value: that one is alpha, and a flat panel is as see-through as
 * the driver asked for.
 *
 * WHAT FLAT IS NOT ALLOWED TO TOUCH: the blur, the alpha, the channel, and the
 * radius. A flat panel can still be heavily frosted; v1 is rounded in every
 * theme and stays rounded here (see the note in `tokens.css`). What is left is
 * the owner's own description of it - a plain filled rectangle with a hairline
 * round it.
 * ========================================================================== */

export const FLAT_ATTRIBUTE = 'data-fwm-flat';

/**
 * Two steps, spelled like the liquid switch because it is the same shape of
 * question: a look you are either in or not.
 *
 * `on` is first now, and the order carries meaning: the fallback for anything
 * unrecognised is the first entry, and it should be the shipping look rather
 * than one nobody chose. Flat IS the shipping look - see `DEFAULT_FLAT`.
 */
export const FWM_FLAT_LEVELS = ['on', 'off'] as const;

export type FwmFlat = (typeof FWM_FLAT_LEVELS)[number];

/**
 * ON, by owner decision 2026-09-06.
 *
 * This shipped as `off` on the argument that the lit material is what the
 * product looks like and a stored blob written before the axis existed must not
 * silently strip the rim off an install that already had the app. The first
 * half was the owner's call to make and they have made it the other way; the
 * second half is a real concern and it is why `resolveFlat` still lands an
 * unreadable value on the DEFAULT rather than on `off` specifically - an
 * install that predates this axis now gets the flat look, deliberately, the
 * same as a fresh one. Nobody ends up on a look that depends on when they
 * installed.
 *
 * The lit material is one tap away and every token it needs is still here.
 */
export const DEFAULT_FLAT: FwmFlat = 'on';

export const FLAT_LABELS: Readonly<Record<FwmFlat, string>> = Object.freeze({
  off: 'Off',
  on: 'On',
});

export const FLAT_NOTES: Readonly<Record<FwmFlat, string>> = Object.freeze({
  off: 'panels are lit and raised: a specular rim at the edge, and a cast shadow under the cards that float over the map.',
  on: 'no rim, no sheen, no cast shadow, no gradient - a plain filled panel with a hairline round it. the blur and the tint are still whatever you set them to.',
});

export function isFlat(value: unknown): value is FwmFlat {
  return typeof value === 'string' && (FWM_FLAT_LEVELS as readonly string[]).includes(value);
}

export function resolveFlat(value: unknown): FwmFlat {
  return isFlat(value) ? value : DEFAULT_FLAT;
}

/** Written for every value, for the same specificity reason as `applyGlass`. */
export function applyFlat(value: FwmFlat, root?: HTMLElement): void {
  const element = root ?? globalThis.document?.documentElement;
  element?.setAttribute(FLAT_ATTRIBUTE, value);
}
