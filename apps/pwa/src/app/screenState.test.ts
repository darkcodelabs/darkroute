...
describe('the way back, across a back gesture', () => {
  it('lands on a screen remembering where IT was opened from, not where the driver just was', () => {
    // MORE -> DOCS -> DRIVE, then the browser's back key. DOCS is on screen
    // again and its arrow has to point at MORE, which is where DOCS was opened
    // from - not at DRIVE, the screen the gesture just left.
    const port = createFakeHistory('?screen=more');
    initScreenState({ port });
    openScreen('docs');
    openScreen('radar');
    expect(getScreenState().from, 'DRIVE was opened from DOCS').toBe('docs');

    port.history.back();

    expect(getScreenState().screen).toBe('docs');
    expect(getScreenState().from, 'DOCS was opened from MORE').toBe('more');
  });

  it('forgets a way back when the entry it lands on never recorded one', () => {
    // The entry point. A cold start has nothing it was opened from, and a back
    // gesture that lands on it must say so, so BackKey falls back to the
    // declared parent rather than a stale id.
    const port = createFakeHistory();
    initScreenState({ port });
    openScreen('settings');
    expect(getScreenState().from).toBe('radar');

    port.history.back();

    expect(getScreenState().screen).toBe('radar');
    expect(getScreenState().from).toBeNull();
  });

  it('never puts the way back in the URL', () => {
    // It rides in history.state with the rest of the entry, and history.state
    // is not copied, synced or pasted. The URL contract stays one parameter.
    const port = createFakeHistory();
    initScreenState({ port });
    openScreen('more');
    openScreen('docs');
    expect(port.location.search).toBe('?screen=docs');
    expect(port.location.search).not.toContain('more');
  });
});

describe('hasOverlay', () => {
  it('is true while the entry is presented and false once it is closed', () => {
    initScreenState({ port: createFakeHistory() });
    expect(hasOverlay('doc:terms')).toBe(false);
    openOverlay({ id: 'doc:terms', kind: 'modal' });
    expect(hasOverlay('doc:terms')).toBe(true);
    closeOverlay('doc:terms');
    expect(hasOverlay('doc:terms')).toBe(false);
  });

  it('is still true while an alert holds the entry aside', () => {
    // An alert moving a document aside is not the document closing. A reader
    // that took the saved stack for "gone" would shut itself under every alert
    // and the driver would come back to a map with the document missing.
    initScreenState({ port: createFakeHistory() });
    openOverlay({ id: 'doc:terms', kind: 'modal' });
    interruptForAlert();
    expect(topOverlay(), 'nothing is drawn during an alert').toBeNull();
    expect(hasOverlay('doc:terms')).toBe(true);
    restoreAfterAlert();
    expect(hasOverlay('doc:terms')).toBe(true);
  });

  it('is false once a back gesture has popped the entry', () => {
    const port = createFakeHistory();
    initScreenState({ port });
    openOverlay({ id: 'doc:terms', kind: 'modal' });
    port.history.back();
