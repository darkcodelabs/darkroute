...
describe('the shell', () => {
  ...
  it('lands on the map, rather than a dead end, when a screen is not built', () => {
    renderShell();
    act(() => {
      openScreen('mesh');
    });
    expect(screen.queryByText('screen not built')).not.toBeInTheDocument();
    expect(screen.getAllByTestId('screen-radar')).toHaveLength(1);
    expect(globalThis.location.search).not.toContain('screen=mesh');
  });
});
