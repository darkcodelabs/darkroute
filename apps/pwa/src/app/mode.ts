 ...
 *   block (see design gap night-watch-has-no-block). The attribute is still
...
export const FWM_MODES = [
   ...
   * GAP: settings / e-ink-mode-is-not-drawn
   */
  'e-ink',
  /**
 ...
 */
/*
 * `slate` by owner decision, 2026-09-05. It has now been `night-watch`, then
 * `slate` for one day, then `neon-grid` for part of one, back to `night-watch`,
 * and now `slate` again.
 *
 ...
 * the value.
 *
 * ONE THING THIS CHANGE COSTS. `night-watch` has NO override block in
 * tokens.css, because `:root` IS Night Watch - see
 * design gap night-watch-has-no-block. While it was the default, a mode
 * that failed to load landed on the unstyled base and still looked correct.
 * `slate` has a real block, so that free safety net is gone: a stylesheet that
 * fails to apply now shows Night Watch while the app believes it is Slate.
 * Worth knowing, not worth refusing the change over - the owner picks the
 * default and nothing in the code depends on the value.
 *
 ...
 */
export const DEFAULT_MODE: FwmMode = 'slate';
