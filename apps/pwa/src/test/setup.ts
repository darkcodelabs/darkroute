...
afterEach(() => {
  ...
  vi.restoreAllMocks()
  /**
   * THE DESTINATION IS MODULE STATE, so it survives `cleanup()`.
   *
   * A file that set one left it set for every file that ran after it in the
   * same worker, and DRIVE renders a whole extra card when a destination
   * exists - so a test about the closest-camera card started failing purely
   * because of what a test in another file had done. That is the class of
   * failure that gets called flaky and is not.
   *
   * Reset HERE rather than in each DRIVE test's own `beforeEach`, because the
   * next slice added will have the same problem and nobody will remember.
   */
