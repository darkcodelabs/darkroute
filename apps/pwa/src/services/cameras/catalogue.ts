...
export interface CameraCatalogue {
  /**
   * Be told when a read lands, so a screen can render the answer rather than
   * the absence of it. Returns its own unsubscribe.
   */
...
export function createCatalogue(options: CatalogueOptions = {}): CameraCatalogue {
  ...
  /**
   * WHO TO TELL WHEN THE COUNT ARRIVES.
   *
   * `load` was fire-and-forget and nothing subscribed, so a screen that called
   * `total()` during its first render got `null` - correctly, the read had not
   * landed - and then never re-rendered when it did. DRIVE's header drew the
   * lone figure "1k" on every cold start, which this module's own docstring
   * calls out as the thing to avoid: a bare number could mean the network
   * knows about 987 cameras or that this phone holds 987 of a much larger set,
   * and those are very different claims.
   *
   * A Set of thunks rather than an event emitter, because the only consumer is
   * `useSyncExternalStore`, which wants exactly this shape.
   */
  ...
  const load = (wanted: string): Promise<void> => {
    ...
    loading = (async (): Promise<void> => {
      try {
        ...
        // Announced on success AND on failure: "we tried and there is still no
        // number" is a state a caller may want to draw differently from "we
        // have not tried yet", and it can only tell them apart if it is told.
        announce();
  ...
  return {
    /**
     * Called whenever the count changes or a load settles.
     *
     * Returns its own unsubscribe, which is the contract
