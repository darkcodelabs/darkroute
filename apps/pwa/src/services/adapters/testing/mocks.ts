 ...
 * real `assertCanBuzz` guard against the real `SOURCE_PATTERNS` table, and
 * `notifications.show` runs the real `composeNotification`, so a test that
 * drives a mock is testing the same decisions the device would make.
 */
...
export function createMockVibration(): MockVibrationAdapter {
  ...
  /*
   * THE REAL TABLE, not a camera-only copy of it. This used to be
   * `CAMERA_ALERT_PATTERNS[state]`, which was correct while cameras were the
   * only thing allowed to buzz. Abuse-area entry and turn cues have their own
   * signatures now, and a mock that answered every source with a camera
   * pattern would let a test prove the exact thing the source table exists to
   * forbid: something that is not a camera feeling like one.
   */
  const patternFor = realPatternFor;
  ...
  return {
    ...
    buzz(request: VibrationRequest): VibrationResult {
      ...
      if (!base.mock.started()) {
        ...
        // names. A mock that refuses with a different sentence is a mock a test
      ...
      // `state` is present for camera alerts and absent for everything else,
      // and under exactOptionalPropertyTypes an absent key and a key holding
      // undefined are different things. Only add it when there is one.
      base.mock.emit({
