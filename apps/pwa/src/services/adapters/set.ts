...
import { wrapWithDemoDrive } from './demoGeolocation.ts';
...
export function createPlatformAdapters(): AdapterSet {
  return {
    /* Wrapped, not replaced. Demo drive is off until somebody turns it on, and
       while it is off this is the real adapter's behaviour unchanged. See
       `demoGeolocation.ts` for why the switch cannot be a boot-time choice. */
    geolocation: wrapWithDemoDrive(createGeolocationAdapter()),
