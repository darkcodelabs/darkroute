 ...
 *
 * `doc-snippet` is a fenced code block out of a document in
 * `docs/public/`, copied from the reader's own Copy affordance. It carries no
 * driver data by construction: the reader fetches through
 * `functions/api/v1/doc/[name].ts`, which serves eleven allowlisted files out
 * of the public repository and nothing else, so the text on the clipboard is
 * the same text anybody can read on GitHub without this app.
 */
