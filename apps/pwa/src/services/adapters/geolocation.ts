 ...
 *
 * There is no further web API to reach for, and this paragraph used to say that
 * slightly wrong, which matters because somebody acted on it. It said Android's
 * `PRIORITY_HIGH_ACCURACY` is a native call the web cannot make. It is not:
 * `enableHighAccuracy: true` IS that constant. Chromium's
 * `LocationProviderGmsCore.java` maps the flag straight onto the fused provider
 *
 *     priority = highAccuracy ? PRIORITY_HIGH_ACCURACY : PRIORITY_BALANCED_POWER_ACCURACY
 *     interval = highAccuracy ? 500ms : 1000ms
 *
 * so this app already asks for the best fix the platform offers, at 500 ms.
 * What is genuinely out of reach is anything FINER than that one boolean: no
 * rate control, no duty cycle, no `kCLLocationAccuracyBestForNavigation`. A
 * Trusted Web Activity is still a WebView using this exact API, so wrapping the
 * PWA unlocks none of it. Anything better needs a native location service
 * handing fixes to the web layer, which is a real option and a much larger one.
 *
 * AND THE LOW-POWER LEVER IS A TRAP, so it is written down rather than left for
 * someone to rediscover as a good idea. `enableHighAccuracy: false` is not a
 * gentler version of the same watch: on Android it is throttled to roughly ONE
 * FIX PER FIFTEEN MINUTES by default, and on iOS it returns
 * `kCLLocationAccuracyHundredMeters` - which is exactly the fixes
 * `DEFAULT_GPS_ACCURACY_LIMIT_M = 50` exists to discard. Worse, the accuracy
 * hint is OR'd across every notifier in the page, so one low-accuracy client
 * coarsens the provider for all of them.
 *
 ...
 *
 *   It also costs nothing per fix. Blink consults `maximumAge` once, in
 *   `StartRequest`, for a notifier's FIRST callback; once the watch is live,
 *   positions arrive through `QueryNextPosition` and it is never read again.
 *   There is no running trade here to reconsider.
 *
 * WHAT THIS COSTS, MEASURED RATHER THAN FEARED
 *   Continuous GNSS is 0.23%-0.69% of a battery-hour (Android's own power
 *   profile puts tracking at 10-30 mA and acquisition at 50 mA). The share a
 *   stationary low-power mode could recover is 0.01%-0.03% - below the 0.06%
 *   this project used to walk away from the alert-engine finding, and about
 *   0.2%-0.5% of what the SCREEN spends in the same hour.
 *
 *   The watch also costs exactly zero while backgrounded, with no app code
 *   involved: Blink stops the provider on page hide through three independent
 *   guards, and clears the cached position so the next observer cannot be
 *   handed a stale one.
 */
