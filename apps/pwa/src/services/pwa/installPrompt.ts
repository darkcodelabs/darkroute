...
export function createInstallPromptController(
  ...
  let running = false;
  /**
   * TRUE ONCE `start()` HAS READ WHAT IS REMEMBERED.
   *
   * Without this the module had a race that reported as "I press KEEP USING THE
   * WEBSITE and it comes back every refresh". `start()` counts the launch,
   * awaits a write, and only THEN reads the dismissal - so between those two
   * awaits `sessions` is already past the gate while `dismissed` is still
   * false. `beforeinstallprompt` fires on its own schedule, and when it landed
   * in that window it published `canPrompt: true`, App opened the sheet, and
   * the refusal arrived a tick later with nothing left to stop.
   *
   * It is also why the card that appeared showed the "add it from your browser
   * menu" branch: by the time it rendered, the real status had resolved to
   * blocked, so `installable` was false. The manual copy on a phone whose
   * browser HAD offered an install was the visible half of this bug.
   *
   * Nothing may be offered before what the driver already said is known.
   */
  let loaded = false;

  const blockReason = (): InstallBlockReason => {
    if (installed) return 'already-installed';
    // BEFORE EVERY OTHER GATE. A refusal that has not been read yet is not the
    // same as no refusal, and treating it as one is what nagged people who had
    // already answered. See `loaded`.
    if (!loaded) return 'first-session';
  ...
  return {
    ...
    async start() {
      ...
      // LAST. Everything remembered is now in hand, so a published status can
      // be trusted - see `loaded`.
      loaded = true;
      publish();
      return snapshot();
    },
