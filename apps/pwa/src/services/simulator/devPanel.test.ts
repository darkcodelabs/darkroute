...
describe('the controls', () => {
  ...
  it('moves the threshold, and refuses one off the slider', () => {
    ...
    // 24 and 5281, because the band widened to 25 ft .. 1 mile with the
    // map-view slider. 25 and 5000 are both settings a driver can now reach,
    // and a simulator that refused them would refuse to reproduce a real drive.
    expect(() => panel.setThresholdFt(24)).toThrow(RangeError);
    expect(() => panel.setThresholdFt(5281)).toThrow(RangeError);
    panel.dispose();
  });
});
