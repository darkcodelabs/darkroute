 ...
 * entry in gap record db-storage where a different number is a
 ...
 *
 * GAP: pending-action-queue-ceiling
 */
 ...
 *
 * GAP: db-storage / report-photo-ceiling
 */
...
export const STORE_CAPS: Readonly<Record<StoreName, number | null>> = {
  ...
  // ONE ROW, AND IT IS BOUNDED BY A RULE RATHER THAN BY A CEILING. The book is
  // a single record holding at most a handful of saved places and nine recents,
  // and the nine is section C's own number enforced at the WRITE - the tenth
  // trip evicts the oldest by last-used inside `rememberOnStart()`. A cap here
  // would be a second, weaker copy of that rule counting the wrong thing: this
  // layer counts ROWS, and there is exactly one no matter how full the book is.
  destinations: null,
};
 ...
 * cache every time.
 *
 * GAP: storage-pressure-threshold
 */
