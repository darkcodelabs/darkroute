...
import { createDestinationsRepository } from './repositories/destinations.ts';
...
export interface Repositories {
  ...
  readonly destinations: ReturnType<typeof createDestinationsRepository>;
}
...
export function createRepositories(
  ...
  return {
    ...
    destinations: createDestinationsRepository(db, deps),
  };
}
...
export interface ClearLocalDataReport {
  ...
  /**
   * Places the user NAMED - Home, Work, the starred ones - removed.
   *
   * A separate number from the recents on purpose. Section C promises that
   * saved places go too ("Nothing, by design ... if the wipe left anything
   * behind, the promise would be false"), and that promise is the one a person
   * is least likely to believe, because every other product on their phone
   * keeps the addresses they typed. Folding the two into one figure would
   * report the fact without letting them check it.
   */
  readonly savedPlacesRemoved: number;
  /** Recent destinations removed. At most nine - section C's own ceiling. */
  readonly recentPlacesRemoved: number;
 ...
 * match index, trips, alerts, the photographs attached to reports, and the
 * destination book - saved places included.
 *
 * WHY THE SAVED PLACES GO WHEN THE SIGNED REPORTS STAY. They look like the same
 * class of thing: both are data the user entered on purpose. The difference is
 * what deleting them costs. A report is a link in a hash chain and removing one
 * breaks verification for every record after it, so shredding it destroys
 * something that cannot be reconstructed and was never only the user's. Home
 * and Work are two addresses somebody can type again in ten seconds, and they
 * are a list of where that person lives and works - which is the single most
 * useful thing on the device to anybody who should not have it. Section C is
 * explicit and this path obeys it: "Nothing, by design."
 *
...
export async function clearLocalData(db: FwmDatabase): Promise<ClearLocalDataReport> {
  ...
  // Counted by ITS OWN clear rather than beside the others above, because the
  // number owed here is places and not rows - the whole book is one row, and
  // "1 removed" is not something a person can check against what they saw in
  // the panel.
  const places = await repos.destinations.clear();
  ...
  return {
    ...
    savedPlacesRemoved: places.saved,
    recentPlacesRemoved: places.recents,
...
export { createDestinationsRepository, isPlaceBook } from './repositories/destinations.ts';
export type { DestinationsRepository, PlaceCounts } from './repositories/destinations.ts';
