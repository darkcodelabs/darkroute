 ...
 *     `alerts` keeps distance and heading, never a latitude. TWO stores hold a
 *     coordinate, and both hold one the user TYPED rather than one the phone
 *     measured: a report they deliberately filed, because a camera report
 *     without a position is not a report, and `destinations` - the places they
 *     chose to drive to, because you cannot route back to somewhere you cannot
 *     locate. Neither is a sample of where the vehicle has been. The
 *     distinction is the whole of the rule: a position sample is a track, and a
 *     destination somebody named is an address book. `destinations` is
 *     additionally the ONLY store in this file that is emptied completely by
 *     `clearLocalData()` including the rows the user curated by hand - see
 *     `DestinationBookRecord`.
...
export interface CameraRecord {
  ...
  /**
   * The street the camera stands on, and the nearest DIFFERENTLY named road
   * when one is close enough to call an intersection.
   *
   * From the pinned release context (`scripts/build-camera-context.mjs`):
   * every camera snapped to the nearest named road of our own basemap, the
   * archive the map draws roads from, so the name on the row is the road under
   * the marker. Abbreviated the way a sign is: "W 95th St". Absent only when no
   * named road lies within two kilometres.
   */
  readonly street?: string;
  readonly cross?: string;
  /**
   * Metres from the camera to `street`. Most are under 40; a camera hundreds of
   * metres from its nearest named road is OFF that road, and a row says so
   * rather than putting it at an intersection it is not at.
   */
  readonly streetM?: number;
  /**
   * The city or town, from the Census PLACE polygons, or "<Name> County" when
   * the camera is in no incorporated place. Never absent on a released record.
   */
...
/**
 * THE PLACE BOOK, AS ONE ROW.
 *
 * The saved places and the recents together, serialised whole under a single
 * key, and the shape stored is `PlaceBook` from `features/search/places.ts` -
 * which is where every rule about it lives. Nothing is re-declared here: this
 * file is storage, and a second copy of `SavedPlace` would be a second place
 * for the four fields section C names to drift.
 *
 * WHY ONE ROW AND NOT A ROW PER PLACE. Section C: "A recent promoted to saved
 * leaves the recents list - never in both." With a row per place that invariant
 * is two writes, and a process killed between them leaves a place in both lists
 * - which is not a state any reader here could tell from a legitimate one. With
 * the book as the unit, the invariant is a property of a single `put` and the
 * bad state is unrepresentable. The book is at most a handful of saved places
 * and nine recents, so there is no size argument on the other side.
 *
 * WHY IT IS NOT IN `storeBlobs`. That store is the zustand persist port's, and
 * everything in it goes through `assertPersistSafe`, whose walker refuses
 * plate-shaped values and says nothing about coordinates. More to the point,
 * `storeBlobs` is `untouched` by the removal path - preferences are not personal
 * data - and this book must be `removed`. Putting it there would have made
 * "Wipe everything on this phone takes both" false by inheritance.
 */
export interface DestinationBookRecord {
  readonly key: typeof DESTINATION_BOOK_KEY;
  /**
   * `PlaceBook` from `features/search/places.ts`.
   *
   * Typed `unknown` for the same reason `SettingsRecord.value` is: IndexedDB
   * returns whatever was put in, and a row that has survived a browser upgrade
   * is not proof of its own shape. `destinationsRepository` validates on read
   * and is the only place the type is decided.
   */
  readonly book: unknown;
  readonly updatedAt: number;
}

/** The single destination-book row. There is only ever one. */
export const DESTINATION_BOOK_KEY = 'book';
...
export interface FwmDB extends DBSchema {
  ...
  // No indexes, and one row. An index over this store would be an ordering of
  // the places somebody drives to, which is the record this application exists
  // to keep from being assembled.
  destinations: {
    key: string;
    value: DestinationBookRecord;
  };
}
