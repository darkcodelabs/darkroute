...
/**
 * v6 - the destination book, which is the first coordinate this application
 * writes to disk that the phone did not have to be told to measure.
 *
 * `features/search/places.ts` has held the whole of section C - four fields and
 * no fifth, written on route start only, nine kept, evicted by last-used,
 * ninety-day expiry, promotion leaving recents, and a wipe that takes the saved
 * places too - since the search panel shipped, and none of it reached disk. The
 * book lived for a session, which lost a person's history on every reload and
 * made the panel's whole premise ("most trips are somewhere you have already
 * been") false in practice.
 *
 * ONE ROW, KEYED `book`. See `DestinationBookRecord` for why the pair of lists
 * is a single record rather than a row per place: the "never in both" invariant
 * becomes a property of one `put` instead of a window between two.
 *
 * NO INDEX, matching `plateVault`, `reportPhotos` and `session`. An index here
 * would be an ordering of where somebody goes.
 *
 * No backfill, because there is nothing on any device to backfill FROM - the
 * previous implementation kept the book in RAM. A phone upgrading to this
 * version starts with no row, which `destinationsRepository.read()` returns as
 * the empty book, which is the first-run state the panel already draws.
 */
const migration6: Migration = {
  version: 6,
  describe: 'add destinations, the durable place book behind the search panel',
  up({ db }) {
    if (!db.objectStoreNames.contains('destinations')) {
      db.createObjectStore('destinations', { keyPath: 'key' });
    }
  },
};
