...
export interface Correction {
  ...
  /**
   * THE RECORD AS THE CARD SHOWED IT -- street, town, owner, maker, facing --
   * so the review reads the claim against what it corrects. Public archive
   * fields only; a driver's own position never goes in here, and `lat`/`lon`
   * above are the CAMERA's, which are already public.
   */
  readonly archive?: Readonly<Record<string, string>> | undefined;
}
...
export async function shareCorrection(
  ...
  const response = await fetchImpl(SUBMIT_URL, {
    ...
    body: JSON.stringify({
      ...
      archive: correction.archive ?? null,
    }),
  });
