 ...
 *   index, the key reference, trip history, alert history, the photographs
 *   attached to reports, and the destination book - the recent places AND the
 *   saved ones.
 *
...
export function describeForgetReport(report: ForgetLocalIdentityReport): string[] {
  const lines = [
    ...
    /*
     * BOTH NUMBERS, ON ONE LINE, AND THE SAVED ONES NAMED OUT LOUD.
     *
     * `Forget all recent places` in SETTINGS keeps the saved places, and this
     * does not. Somebody who has used the smaller control will reasonably
     * assume the bigger one behaves the same way and stops at the recents, so
     * the line says the second number instead of leaving them to notice that
     * Home is gone the next time they open the panel. The clause is the same
     * job the photograph clause does two lines down: correct the misreading the
     * sentence itself creates.
     */
    `${String(report.recentPlacesRemoved)} recent place${report.recentPlacesRemoved === 1 ? '' : 's'} deleted, ` +
      `and the ${String(report.savedPlacesRemoved)} place${report.savedPlacesRemoved === 1 ? '' : 's'} you saved - ` +
      'a wipe that kept your addresses would not be a wipe.',
  ]
