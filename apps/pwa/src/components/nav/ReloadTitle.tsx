...
export interface ReloadTitleProps {
  ...
  /**
   * ARTWORK THE CALLER DRAWS ITSELF, in place of the words and in place of
   * `asMark`'s own stencil.
   *
   * DRIVE's chrome is the one caller that needs this. `asMark` paints
   * `/assets/darkroute-wordmark.png` as a mask at `--fwm-text-title`, which is
   * the heading the status pill drew; the chrome brief draws a DIFFERENT file
   * at a measured 86x15 -- `features/chrome/topBar.css` sizes it and inverts
   * it on the light skins -- and a second stencil rule here would be this
   * component holding an opinion about one screen's brand asset.
   *
   * WHAT DOES NOT MOVE IS THE GUARANTEE. The `<h1>`, the button, the
   * `data-fwm-reload-title` hook and `reloadTitleLabel(title)` are the same on
   * this path as on the other two, so the artwork is `aria-hidden` decoration
   * inside a control that still announces "darkroute.ai - reload this page".
   * `title` stays REQUIRED for exactly that reason.
   */
...
export function ReloadTitle({
  ...
  return (
    <h1 className={className}>
      ...
      >
        {art ?? (asMark ? <span className="fwm-reloadtitle-mark" aria-hidden="true" /> : title)}
      </button>
    </h1>
  );
}
