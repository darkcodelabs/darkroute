...
/**
 * SCREENS THAT DRAW THE TITLE THROUGH A COMPONENT OF THEIR OWN.
 *
 * The guarantee this file exists to hold is that every screen has a way to
 * reload - in a PWA running without browser chrome, the wordmark is the only
 * one. It does not require that the JSX sit in the screen's own file.
 *
 * DRIVE's top row became the search bar, and the wordmark went with it. Then
 * the chrome brief replaced that bar with `features/chrome/TopBar.tsx` and the
 * wordmark went with it again -- drawn as the brief's own 86x15 artwork, handed
 * to `ReloadTitle` through `art` rather than restated by it. Naming the file
 * the row delegates to keeps the guarantee checked rather than excused: this
 * test still fails if the reload control disappears, it just looks where DRIVE
 * actually draws it now.
 */
const DELEGATES: Readonly<Record<string, string>> = {
  radar: 'features/chrome/TopBar.tsx',
};
...
describe('the reload title', () => {
  ...
  it('is drawn by every v1 page that is not an overlay or a takeover', () => {
    for (const [screen, file] of Object.entries(TITLE_FILE)) {
      ...
      const drawnIn = DELEGATES[screen] ?? file;
      const code = codeOf(drawnIn);
      expect(code, `${screen.toUpperCase()} (${drawnIn}) draws no ReloadTitle`).toContain(
        '<ReloadTitle',
      );
      expect(
        code,
