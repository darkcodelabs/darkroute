...
/* The list's own predicate and day boundary, so the hero cannot disagree with
   the rows about what a pass is or when today started. */
import { isCameraPass } from '../features/log/exposure.ts';
import { localDayStart } from '../services/history/passSummary.ts';
...
export function createHistoryStore() {
  ...
  return create<HistoryStore>()((set, get) => ({
    ...
    hydrate(entries, allTimePasses, sinceMs) {
      ...
      const kept = [...entries].slice(0, DEFAULT_MAX_HISTORY_ENTRIES);

      /*
       * AND REBUILD TODAY FROM WHAT WAS RESTORED.
       *
       * THE BUG THIS CLOSES: EXPOSURE read `0 plate reads today` above a TODAY
       * list holding two passes from this afternoon. Both numbers were honest
       * about their own source and the sources had different lifetimes.
       *
       * `today.passes` is only ever incremented by `notePass`, which is a
       * SESSION counter living in RAM. The list is derived from `entries`,
       * which hydrate restores from IndexedDB. So every reload restored the
       * passes and reset the count of them to zero, and the screen contradicted
       * itself until the driver was read again.
       *
       * Rebuilt with `isCameraPass` and `localDayStart` - the same predicate the
       * list filters on and the same boundary it uses - so the hero and the
       * rows cannot disagree about what "today" means or what counts as a pass.
       * Deriving it is also why this cannot drift again: there is now one
       * definition, read twice, rather than two counters kept in step by hand.
       */
      const dayStart = localDayStart(Date.now());
      const todayPasses = kept.filter(
        (entry) => isCameraPass(entry) && entry.atMs >= dayStart,
      );
      const uniqueToday: string[] = [];
      for (const entry of todayPasses) {
        const id = entry.cameraId;
        if (id !== null && !uniqueToday.includes(id)) uniqueToday.push(id);
      }
      ...
      set({
        ...
        today: {
          dayStartMs: dayStart,
          passes: todayPasses.length,
          uniqueCameraIds: uniqueToday,
        },
