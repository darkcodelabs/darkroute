...
describe('assertPersistSafe', () => {
  ...
  it('allows the id shape the catalogue ACTUALLY issues, not just the design files', () => {
    ...
    /*
     * And the way form. A camera a mapper drew as an OSM WAY rather than a
     * node ships as `osm:w<digits>`, because OSM numbers the two independently
     * and `osm:1516934727` is a camera in the way namespace and nothing at all
     * in the node one. Refusing the `w` would have made those cameras
     * unmutable, in the same silent everything-resets way.
     */
    expect(() => assertPersistSafe({ mutedCameras: { 'osm:w1516934727': 1 } })).not.toThrow();
    expect(() => assertPersistSafe({ mutedCameras: { 'osm:w1': 1 } })).not.toThrow();
    ...
    for (const key of ['osm:', 'osm:abc', 'osm:12ab', 'notosm:123', 'osm 123', 'osm:w', 'osm:ww1']) {
