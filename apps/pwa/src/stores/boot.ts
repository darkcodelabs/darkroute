...
export async function bootStores(options: BootStoresOptions = {}): Promise<BootResult> {
  ...
  /*
   * THE PLACE BOOK, BACK OFF DISK - and STRICTLY AFTER the settings rehydrate
   * above, which is the one ordering constraint in this function.
   *
   * `destinationsActions.hydrate()` sweeps recents older than the `Keep for`
   * window and DELETES what it sweeps. That window is a stored preference whose
   * four values include `never`, so running the sweep while the preference was
   * still in flight would delete, at the ninety-day default, exactly the rows a
   * person chose `never` to protect. The slice refuses to sweep destructively
   * if it is called early, and this call site is why it never has to.
   *
   * Awaited rather than raced against the deadline, for `hydrateHistory`'s
   * reason: a boot that gives up on the book opens the panel to a cursor and to
   * a first-run card, telling somebody with twelve places that they have none.
   * A failure is survivable because `hydrate()` returns false rather than
   * throwing.
   */
