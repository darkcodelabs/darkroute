 ...
 * `apps/pwa/public/cameras/**` is `osm:<digits>` or `osm:w<digits>`, because
 * they are OpenStreetMap elements: 1163 of 1163 across a sample of the tiles,
 * with no other prefix. The `w` marks a camera a mapper drew as a WAY rather
 * than a node - the road or gantry the reader sits on - published at that
 * way's centroid. OSM numbers nodes and ways independently, so the letter is
 * what keeps `osm:1516934727` and `osm:w1516934727` from being the same key
 ...
 */
const CAMERA_ID_RE = /^(?:[A-Z]{2,6}-\d{2,8}|osm:w?\d{1,20})$/;
