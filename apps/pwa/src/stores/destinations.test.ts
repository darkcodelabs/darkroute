...
describe('WHEN IT IS WRITTEN - on route start, not on search and not on tap', () => {
  it('has exactly one function that adds a place, and it is named for the moment', () => {
    ...
    expect(adders).toEqual(['rememberRouteStart', 'savePlace', 'saveNewPlace']);
    // `savePlace` promotes something already in the book; it adds no place the
    // user did not already drive to. `saveNewPlace` is section C's other
    // deliberate write -- SET HOME / SET WORK on first run -- and it writes a
    // SAVED place from a press on that key, never a recent: a destination
    // looked at and abandoned still leaves no trace.
  });
