...
/**
 * THE FULL-SCREEN ROW, and its two sentences.
 *
 * The OFF sentence names what a driver keeps rather than what they lose,
 * because OFF is the default and a default has to read like a decision. The ON
 * sentence names the cost, because turning it on is the choice that has one.
 */
export const FULLSCREEN_LABEL = 'Full screen';
export const FULLSCREEN_OFF = 'off - your clock, battery and signal stay on screen';
export const FULLSCREEN_ON = 'on - the map takes the whole screen from the next launch';
...
export function OnboardingViewV1({
  ...
  const immersive = useImmersiveOnLaunch();
  ...
  return (
    <section className="fwm-onboardingv1" data-testid="onboarding">
      ...
      {/* FULL SCREEN, ASKED FOR RATHER THAN TAKEN.
          The app used to grab the whole screen on the first touch of every
          session, which took the clock, the battery and the signal off a phone
          somebody was about to drive with. It was the most common complaint
          about the app. It is a choice now, it is OFF unless this is pressed,
          and it is on the first screen so nobody has to go looking for it.

          Deliberately NOT in the permissions list above: those rows are the
          operating system's answers and this one is entirely ours, and mixing
          them would suggest a driver has to grant something for it. */}
      <button
        type="button"
        className="fwm-onboardingv1-permission fwm-onboardingv1-fullscreen"
        data-testid="onboarding-fullscreen"
        role="switch"
        aria-checked={immersive}
        onClick={() => {
          useSettingsStore.getState().setImmersiveOnLaunch(!immersive);
        }}
      >
        <span className="fwm-onboardingv1-rule" aria-hidden="true" />
        <span className="fwm-onboardingv1-where">
          <span className="fwm-onboardingv1-label">{FULLSCREEN_LABEL}</span>
          <span className="fwm-onboardingv1-sub fwm-data">
            {immersive ? FULLSCREEN_ON : FULLSCREEN_OFF}
          </span>
        </span>
        <span className="fwm-onboardingv1-track" aria-hidden="true">
          <span className="fwm-onboardingv1-knob" />
