...
export const RADIOS_NONE_NOTE =
  'A Meshtastic radio over Bluetooth. It needs firmware on it already — flash it with ' +
  'Meshtastic’s own installer, we do not build any.';

/**
 * THE LINE THAT REPLACES TWO EMPTY SCREENS.
 *
 * Brief 4 removes Chat and Config while unpaired - "they are a locked line
 * inside Radios rather than two 700 px voids" - and this is that line. It sits
 * on the pairing card rather than beside the segmented control on purpose: the
 * thing that unlocks them is the button directly above it, so the explanation
 * belongs to the button and not to the tabs it is about.
 */
export const RADIOS_LOCKED_NOTE =
  'Chat and Config unlock when a node is paired. Until then they are locked here rather than ' +
  'shown as two empty screens.';
...
export function MeshRadios(): ReactElement {
  ...
  return (
    <div className="fwm-radios">
      {/* --- the pairing card ---------------------------------------------
          Section C's card, and the spec's own frame 5: the state as a tracked
          badge beside the name, the Meshtastic sentence, the one key, and the
          line that says where Chat and Config went. `IDLE` is not copy - it is
          `mesh.status` upper-cased, so the badge cannot claim a state the
          session is not in. */}
      <div className="fwm-radios-card">
        ...
        {/* Only while there is nothing paired. Once a node is on the cable the
            two tabs are real and a sentence about them being locked would be
            the screen contradicting the control above it. */}
        {connected ? null : (
          <p className="fwm-radios-locked">{RADIOS_LOCKED_NOTE}</p>
        )}
      </div>
