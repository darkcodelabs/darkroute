...
describe('privacy', () => {
  it('offers no maps key on an iPhone in the shipped v1 card', () => {
    ...
    // BY ITS REGION, NOT BY A TITLE ROW. Brief 4's modal has no "Camera"
    // heading: its head row is the operator tag, the cross street and the
    // one-line readout, because a 342px card over a live map cannot spend a
    // 56px track on the word for what the driver just tapped. The heading is
    // now the street; what this test is about is the key that is NOT there.
    expect(screen.getByRole('region', { name: 'camera' })).toBeInTheDocument();
