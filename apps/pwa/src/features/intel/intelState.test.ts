...
// ---------------------------------------------------------------------------
// The modal's one line
// ---------------------------------------------------------------------------

/**
 * `mi NE · faces 135° · signal mast`.
 *
 * Brief 4 collapses three facts onto one line beside the distance numeral, and
 * these are the two decisions in that: the OSM values are normalised to the
 * spec's sentence case rather than to the tiles' caps, and a clause nobody
 * wrote down is dropped rather than printed as an em dash inside a sentence.
 * The second one is only defensible because the deep sheet still draws MOUNT
 * and FACING as tiles with the dash in them, which is asserted in
 * `components/IntelDetailsV1.test.tsx`.
 */
describe('the summary line the modal draws', () => {
  it('reads the unit, the bearing, the facing and the mount in that order', () => {
    const line = intelSummary(
      intelReadout(assessment({ distanceFt: 12_672 })),
      intelTiles(record({ tags: { 'camera:mount': 'traffic_signals' } })),
    );

    expect(line).toBe('mi SW · faces 223° · traffic signals');
  });

  it('lower-cases the mapper own casing, the way the tiles upper-case it', () => {
    // `pole`, `Pole` and `POLE` are one answer and must not read as three. The
    // tiles normalise upwards because that card is set in the data face; this
    // line normalises downwards because the spec draws it in sentence case.
    // What must not happen either way is the mapper's own casing reaching a
    // screen, and it does not.
    const line = intelSummary(
      intelReadout(assessment()),
      intelTiles(record({ tags: { support: 'POLE' } })),
    );

    expect(line).toContain('pole');
  });

  it('drops a clause nobody wrote down rather than printing a dash mid-sentence', () => {
    // No mount tag, no bearing: two thirds of the line have no answer, so the
    // line is what is left of it rather than `ft SW · faces — · —`.
    const line = intelSummary(
      intelReadout(assessment()),
      intelTiles(record({ directionDeg: null })),
    );

    expect(line).toBe('ft SW');
    expect(line).not.toContain(NO_VALUE);
  });

  it('still names the unit when there is no fix at all', () => {
    // The value beside it is an em dash. `ft` under a dash still says what the
    // dash would have been measured in, which is more than a blank does.
    expect(intelSummary(intelReadout(null), intelTiles(null))).toBe('ft');
  });
});
