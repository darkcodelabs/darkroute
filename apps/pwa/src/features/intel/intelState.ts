 ...
 * GAP: intel / identity-line-copy-is-authored
 */
 ...
 *   GAP: intel / header-cardinal-is-ambiguous
 *
 ...
 * GAP: intel / owner-tile-has-one-drawn-value
 */
/**
...
export const OWNER_LABEL: Readonly<Record<CameraOwnerType, string>> = Object.freeze({
  ...
  hoa: 'HOA',
  private: 'PRIVATE',
  /* "What's unverified about this?" -- owner, 2026-09-09. Nothing about the
     camera: the class means the mapper recorded no operator. Say that. */
  unverified: 'OPERATOR NOT RECORDED',
});
...
/**
 * The separator every short fact in this product is joined with, written once.
 * `formatCoveredDirections` above builds the same glyph from an escape; this is
 * the same character, spelled the way the copy constants in `components/` are.
 */
const SUMMARY_JOIN = ' · ';

/**
 * THE MODAL'S ONE LINE OF FACTS -- `mi NE · faces 135° · signal mast`.
 *
 * =============================================================================
 * WHY THIS IS A FUNCTION AND NOT THREE SPANS IN THE VIEW
 * =============================================================================
 * Brief 4 collapses "how far", "which way it looks" and "what it is bolted to"
 * onto one line beside the distance numeral, and the three answers come from
 * three different places -- the engine's assessment, `direction`, and
 * `camera:mount`/`support`. Composing them in the view would put a second
 * reading of those tags in a component, which is the thing this module exists
 * to prevent: the deep sheet still draws MOUNT and FACING as their own tiles,
 * and two renderings of one tag is how a card ends up contradicting itself.
 *
 * =============================================================================
 * IT IS SENTENCE CASE, AND THAT IS NOT A LOSS OF THE TILES' NORMALISATION
 * =============================================================================
 * `tagValue` upper-cases every OSM value on the way in, because mappers type
 * `pole`, `Pole` and `POLE` and those are one answer that must not read as
 * three. The spec draws this line in sentence case, so it lower-cases instead
 * -- the SAME normalisation, applied the other way. What must not happen is the
 * mapper's own casing reaching the screen, and it does not.
 *
 * The degree value keeps its own casing: `135°` has none, and a cardinal like
 * `NE` is an abbreviation the whole product sets in caps.
 *
 * =============================================================================
 * AN UNKNOWN CLAUSE IS DROPPED HERE, AND STATED ONE LEVEL DEEPER
 * =============================================================================
 * This card's rule is that an unknown is SAID rather than omitted -- a missing
 * row reads as "this camera does not do that". The rule is kept, and it is kept
 * where there is room for it: the deep sheet draws MOUNT and FACING as tiles
 * with an em dash in them exactly as before. Printing `faces —` in a summary
 * line would not state an absence, it would state noise, and this line is a
 * summary rather than the record.
 */
export function intelSummary(readout: IntelReadout, tiles: readonly IntelTile[]): string {
  const facing = tiles.find((tile) => tile.label === 'FACING');
  const mount = tiles.find((tile) => tile.label === 'MOUNT');
  const parts: string[] = [
    // The unit always prints. The VALUE beside it may be an em dash -- no fix,
    // no distance -- and `ft` under a dash still says what the dash would have
    // been measured in, which is more than a blank does.
    [readout.unit.toLowerCase(), readout.cardinal]
      .filter((part): part is string => part !== null)
      .join(' '),
  ];
  if (facing?.known === true) parts.push(`faces ${facing.value}`);
  if (mount?.known === true) parts.push(mount.value.toLowerCase());
  return parts.join(SUMMARY_JOIN);
}
 ...
 * GAP: intel / your-reads-tone-at-zero-not-drawn
 */
...
export interface IntelInput {
   ...
   * GAP: intel / mute-this-one-is-a-ten-minute-timer
   */
...
export interface IntelViewModel {
  ...
  /**
   * THE CROSS STREET ON ITS OWN, WHICH THE MODAL LEADS WITH.
   *
   * `identity.subline` already carries this, and carries it BEHIND the
   * hardware name -- `intelIdentity` leads with `manufacturer`, because the
   * full card's job is to say what the thing on the pole is. The modal's job is
   * different and the brief says so: "you tapped it BECAUSE of where it is", so
   * `METCALF AVE @ W 111TH ST` is its headline and everything else moves one
   * level deeper.
   *
   * Not a new source. It is `streetLine(record)` with the same basemap fallback
   * the identity line already gets, resolved once here so the two lines cannot
   * disagree about which road this is. Null when the record snapped to nothing
   * and the map could not be asked, which is when the modal falls back to
   * {@link identity}'s title.
   */
  readonly street: string | null;
  readonly readout: IntelReadout;
  /**
   * `mi NE · faces 135° · signal mast` -- the modal's one line of facts.
   *
   * Composed from the readout and the tiles rather than from the record, so
   * there is exactly one reading of the OSM tags in this file and the line can
   * never contradict the MOUNT and FACING tiles the deep sheet still draws.
   * See {@link intelSummary} for what it does with a value nobody wrote down.
   */
...
export function intelModel(input: IntelInput): IntelViewModel {
  return {
    ...
    /*
     * ONE RESOLUTION OF "WHICH ROAD", read by both the identity line and the
     * modal's headline. The record's own street wins over the basemap's,
     * because the baked value carries a cross street and a nearest-road lookup
     * cannot: see `streetLine` and `IntelInput.streetFallback`.
     */
    street: streetLine(input.record) ?? input.streetFallback ?? null,
    readout: intelReadout(input.assessment),
    summary: intelSummary(intelReadout(input.assessment), intelTiles(input.record)),
 ...
 * GAP: intel / mute-this-one-is-a-ten-minute-timer
 * GAP: intel / no-action-feedback-is-drawn
 */
