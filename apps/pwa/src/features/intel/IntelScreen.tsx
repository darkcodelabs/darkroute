...
export function IntelScreen({
  ...
  return (
    <View
      ...
      /*
       * GO TO IT, ON THIS APP'S OWN MAP.
       *
       * This used to hand the camera's coordinates to whatever map application
       * the phone has registered - which meant the one screen in the product
       * about a surveillance camera ended by telling a maps company you were
       * interested in it.
       *
       * DarkRoute routes for itself now, so it sets the camera as a destination
       * and closes the card back onto the map. No handoff, and no
       * `canUseGeoHandoff` gate either: the reason that gate existed was that
       * iOS has no `geo:` handler, and there is nothing to hand off any more.
       *
       * A DESTINATION IS NOT A ROUTE. This draws the card; the route is the
       * card's own press. The app does not plan a drive on somebody's behalf.
       */
      onNavigate={
        selected === null
          ? undefined
          : () => {
              routeActions.setDestination({
                name: titleOf(selected),
                detail: placeOf(selected),
                lat: selected.lat,
                lon: selected.lon,
              });
              /* AND PLAN. Setting a destination drew nothing on its own; the
                 planner lived in a closure on DRIVE this screen could not
                 reach, so the key closed the card and the map sat still. The
                 press is the authorisation; the store does the rest. */
