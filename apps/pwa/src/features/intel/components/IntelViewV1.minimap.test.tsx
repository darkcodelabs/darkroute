...
/**
 * THE PICTURE ARRIVES IN ITS OWN CHUNK NOW.
 *
 * `MiniMap` builds a MapLibre instance, so importing it statically kept the
 * whole 898 KB map engine on the boot path of every screen. It is behind
 * `React.lazy` via `LazyMiniMap`, which means the first render is the Suspense
 * fallback and the real figure appears a microtask later.
 *
 * So these await it. `findBy*` rather than `getBy*` is the whole change - what
 * each test asserts is unchanged, because what the component draws is
 * unchanged.
 */
async function mount(model: Parameters<typeof IntelViewV1>[0]['model']): Promise<HTMLElement> {
  const { container } = render(<IntelViewV1 model={model} />);
  // The real figure, not the fallback: the fallback is `aria-hidden` and draws
  // no mark, so waiting for the mark is waiting for the chunk.
  await screen.findByRole('img');
  return container;
}

describe('the mini map on the hero', () => {
  it('draws a picture of the camera beside the line that describes it', async () => {
    const container = await mount(model());
    const head = container.querySelector('.fwm-intelv1-head');
    expect(head).not.toBeNull();
    // Beside, not under: the ident column and the 62px picture are siblings in
    // the modal's one head row, which is what the spec's frame draws.
    expect(head?.querySelector('.fwm-intelv1-readout')).not.toBeNull();
    expect(head?.querySelector('.fwm-intelv1-inset .fwm-minimap')).not.toBeNull();
  });

  it('marks the camera and the way its lens points', async () => {
    const container = await mount(model({ directionDeg: 90 }));
  ...
  it('draws no cone for a camera whose facing nobody wrote down', async () => {
    // `null` facing never means "not facing you". The picture says nothing
    // rather than pointing the lens somewhere it cannot justify.
    const container = await mount(model({ directionDeg: null }));
    expect(container.querySelectorAll('.fwm-minimap-cone')).toHaveLength(0);
    expect(container.querySelector('.fwm-minimap-dot')).not.toBeNull();
  });

  it('draws a cone for every direction on a record that carries several', async () => {
    const container = await mount(model({ directionDeg: 90, tags: { direction: '90;270' } }));
