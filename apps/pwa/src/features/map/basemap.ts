 ...
 *
 * `pois` WAS REMOVED FROM THIS LIST AND IS BACK, 2026-09-07. The removal was
 * correct and its reasoning was too narrow. It read: "the black flavour draws
 * boundaries, buildings, earth, landuse, places, roads and water -- no `pois`
 * layer exists in it to filter, so the entry matched nothing". True of BLACK,
 * and of white and grayscale, all of which emit 69 layers. It is false of DARK
 * and LIGHT, which emit 71: the two extra are `pois` and `landcover`.
 *
 * That stopped being a hypothetical when `flavors.ts` began deriving the two
 * custom flavours from `dark` and `light`, exactly as the brief asked. Without
 * this entry, slate and refinement each ship a symbol layer pointing at a
 * source layer the archive does not contain -- drawing nothing, evaluated
 * every frame, which is the precise thing the paragraph above says a style
 * must not do.
 *
 * `landcover` is NOT here and must not be, though it arrives by the same door.
 * READ OFF THE SHIPPED ARCHIVE'S OWN METADATA rather than assumed - a range
 * request for the PMTiles header of `basemap-us-20260901-full-us.pmtiles`
 * lists its `vector_layers` as exactly:
 *
 *     boundaries, earth, landcover, places, roads, water
 *
 * which confirms both halves at once: the three names above really are absent,
 * and `landcover` really is present. It is the only ground a driver sees when
 * they pinch out past z7 - `MIN_ZOOM` is 3 - so the flavours set its colours
 * rather than dropping it.
 */
export const OMITTED_SOURCE_LAYERS: readonly string[] = ['buildings', 'landuse', 'pois'];
 ...
 *
 * FIVE SHEETS EXIST, AND THERE ARE SEVEN FLAVOURS. The two this project draws
 * itself have no artwork of their own and are not getting any: a sprite sheet
 * is raster, so a custom one would be a new asset to regenerate on every
 * upstream bump. `spriteFlavor` in `flavors.ts` maps each onto the stock sheet
 * it can actually be read against, and the argument there is mechanical rather
 * than a preference - one flavour key paints both the road name and the number
 * inside the shield.
 */
