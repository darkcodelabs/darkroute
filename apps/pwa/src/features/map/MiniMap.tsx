...
// TYPES ONLY. The instance itself comes from `createMap`, which is where the
// MapLibre 6 constructor's silent no-GPU shell is caught -- see `mapStyle.ts`.
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        //
        // Reached through `createMap`, which is what still throws here: MapLibre
        // 6's own constructor returns a painterless shell instead. See the note
        // on `createMap` in `mapStyle.ts`.
        setGround('bare');
        return;
      }
