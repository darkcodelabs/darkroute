...
/**
 * THE ENVELOPE MUST ACCEPT AN ABSENCE THAT ARRIVES AS `undefined`.
 *
 * This is a compile-time test wearing a runtime test's clothes. The failure it
 * guards against is invisible at runtime and fatal at build time: with
 * `exactOptionalPropertyTypes` on, `rxRssi?: number | null` refuses an
 * explicitly-passed `undefined`, and the radio library passes its fields
 * positionally, so absence arrives explicitly.
 *
 * `@jsr/meshtastic__protobufs` 2.8.0 changed those fields from `| null` to
 * `| undefined`, and the radio package depends on it by range, so any lockfile
 * regeneration adopts it. Ten dependabot pull requests -- react types, eslint,
 * tailwind, none of them touching this file or this dependency -- all failed CI
 * on exactly this assignment. Without this test the next person to widen the
 * type back would not find out until ten unrelated PRs went red again.
 */
describe('PacketEnvelope tolerates both shapes of absence', () => {
  it('accepts undefined for every optional field, as the radio actually sends it', () => {
    const fromRadio: PacketEnvelope = {
      from: 0x433a1b2c,
      rxSnr: undefined,
      rxRssi: undefined,
      hopStart: undefined,
    ...
    const state = ingestPacket(EMPTY_INSTRUMENT, fromRadio, T0);
    expect(state.totalCount).toBe(1);
    expect(state.heard[0]?.rssi ?? null).toBeNull();
  });

  it('still accepts null, which is what the previous protobuf release sent', () => {
    const legacy: PacketEnvelope = {
      from: 0x433a1b2c,
      rxSnr: null,
      rxRssi: null,
      hopStart: null,
    ...
    expect(ingestPacket(EMPTY_INSTRUMENT, legacy, T0).totalCount).toBe(1);
  });
});
