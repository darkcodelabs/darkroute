 ...
 *
 * EVERY OPTIONAL FIELD SPELLS `| undefined` OUT LOUD, and that is not noise.
 * `exactOptionalPropertyTypes` is on, so `rxRssi?: number | null` means "may be
 * absent, or a number, or null" and specifically NOT "may be set to undefined".
 * The radio library hands us its fields positionally, so absent arrives as an
 * explicit `undefined` rather than a missing key, and the narrower type rejects
 * it.
 *
 * That is not hypothetical. `@jsr/meshtastic__protobufs` changed `rxRssi` from
 * `number | null` to `number | undefined` in 2.8.0, and because the radio
 * package depends on it by range rather than by pin, ANY `pnpm install` that
 * regenerates the lockfile picks the new one up. Ten open dependabot pull
 * requests -- every one of them claiming to bump something unrelated, a react
 * type, a lint rule, tailwind -- all failed CI on this exact line, because each
 * had quietly re-resolved that transitive dependency.
 *
 * Accepting both shapes is the honest fix: the field means "the radio did not
 * tell us", and null and undefined are the same absence wearing different
 * clothes. Pinning the transitive package would have hidden it until the next
 * time something forced the bump.
 */
export interface PacketEnvelope {
  readonly from: number;
  readonly rxSnr?: number | null | undefined;
  readonly rxRssi?: number | null | undefined;
  readonly hopStart?: number | null | undefined;
  readonly hopLimit?: number | null | undefined;
  readonly viaMqtt?: boolean | undefined;
