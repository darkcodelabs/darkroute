 ...
 *
 * `Wipe everything on this phone` is brief 4's own row label, and the swap from
 * "Delete everything on this device" is a copy change the spec makes rather
 * than a styling one: THIS PHONE is the group, and every other row in it says
 * "phone" too. The word is still terminal and still names the whole store --
 * the two properties the v0 strings were chosen for. `REMOVAL_WARNING` and
 * `REMOVAL_CANCEL_LABEL` are untouched and still v0's, imported rather than
 * retyped, because the words shown at the moment of an irreversible press are
 * not a styling decision.
 */
export const REMOVAL_V1_LABELS: Readonly<Record<RemovalPhase, string>> = Object.freeze({
  idle: 'Wipe everything on this phone',
  armed: 'Tap again to confirm',
  working: 'Wiping…',
  done: 'Wipe everything on this phone',
  unavailable: 'Wipe everything on this phone',
});
