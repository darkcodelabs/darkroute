 ...
 * this build; the greps are in gap record settings. Rendering a
 ...
 * GAP: settings / four-stored-preferences-have-no-consumer
 *
 * =============================================================================
 * EVERYTHING ELSE THE BRIEF'S SCREEN 6 LISTS, AND WHY IT IS NOT HERE
 * =============================================================================
 * design gap no-settings-screen-exists records the brief as: "alert
 ...
 * GAP: settings / audio-toggle-label-is-not-drawn
 */
export const AUDIO_LABEL = 'Audio';

/**
 * What the two toggles do, in one line.
 * GAP: settings / alerts-section-caption-is-not-drawn
 */
...
export interface SettingsViewModel {
  ...
  /** The chosen map ground, or `null` for the theme's own. Six-digit hex. */
  readonly mapEarth: string | null;
  /** Whether the position on the map is placed by hand. */
  readonly demoDrive: boolean;
  /** Which basemap cartography the map draws. */
  readonly mapView: FwmMapView;
  /*
   * `mapTilt` WAS HERE. The angle is a row in DRIVE's MAP VIEW panel now, by
   * owner decision - it reads and writes the store directly, one press from
   * the map it tilts. Neither view on this screen drew it any more, and a
   * model field no view reads is a value the container computes for nobody.
   */
...
export interface SettingsViewHandlers {
  ...
  readonly onFlatPick?: ((flat: FwmFlat) => void) | undefined;
  readonly onMapEarthPick?: ((colour: string | null) => void) | undefined;
  readonly onDemoDriveToggle?: ((on: boolean) => void) | undefined;
  readonly onMapViewPick?: ((view: FwmMapView) => void) | undefined;
  /* `onMapTiltPick` WAS HERE, with `mapTilt` above. See that note. */
