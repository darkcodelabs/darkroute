 ...
 * removal path.
 *
 * SOURCE: no panel. design gap no-settings-screen-exists records that
 ...
 * GAP: settings / stored-mode-is-not-applied-at-boot
 *
 ...
 * else. On success this screen also calls `historyActions.reset()` and
 * `destinationsActions.forgotten()`, because the
...
export function SettingsScreen({
  ...
  const tone = useTone();
  const flat = useFlat();
  const mapEarth = useSettingsStore((state) => state.mapEarth);
  const demoDrive = useSettingsStore((state) => state.demoDrive);
    ...
    void removalPort().then(
      (outcome) => {
        ...
        if (outcome.status === 'removed') {
          // The durable copy is gone; drop the in-memory mirror with it. Both
          // mirrors: the LOG timeline, and the place book - a wipe that leaves
          // Home in the search panel until the next reload has not removed it
          // in any sense the driver would recognise, and section C's promise is
          // that the saved places went too.
  ...
  /* Same route again. Flat is the fifth axis, not a level of any of the four
     above it, so it gets its own action rather than a branch inside one. */
  const onFlatPick = useCallback((next: FwmFlat): void => {
    useSettingsStore.getState().setFlat(next);
  }, []);


  /* The ground colour, and the way back to the theme's own. Both go through the
     one store action so `applyMapEarth` runs on either. */
  const onMapEarthPick = useCallback((next: string | null): void => {
    useSettingsStore.getState().setMapEarth(next);
  }, []);

  /* DEMO DRIVE STARTS WHERE YOU ARE. Turning it on with no position would put
     the pad on a map showing nothing, so it seeds from the last real fix and
     falls back to the map's current centre. `DriveScreen` owns that seed - this
     only records the preference. */
  const onDemoDriveToggle = useCallback((on: boolean): void => {
  ...
  /*
   * `onMapTiltPick` WAS HERE, and the store action it called is very much
   * alive - DRIVE's MAP VIEW panel cycles the angle through it. What is gone
   * is the SETTINGS route to it: v1's map-angle card moved to that panel by
   * owner decision and v0's view never drew one, so this container was
   * building a callback and threading it into a view that had stopped reading
   * it. `model.mapTilt` went with it for the same reason.
   */
  ...
  return (
    <View
      ...
      onFlatPick={onFlatPick}
      onMapEarthPick={onMapEarthPick}
      onDemoDriveToggle={onDemoDriveToggle}
