 ...
 * =============================================================================
 * 100, 1000 and 50 are `@fwm/core`'s `ALERT_THRESHOLD_DESIGN_MIN_FT`,
 * `ALERT_THRESHOLD_DESIGN_MAX_FT` and `ALERT_THRESHOLD_STEP_FT`, reached
 * through `stores/fwmCore.ts` -- the one wiring point the stores already use. A
 * second copy here would be a second place for the bezel and the slider to
 * disagree about what a detent is, and the copy that drifts is always the one
 * nobody remembered.
 *
 * THE _DESIGN_ PAIR, NOT THE ENGINE'S OWN BOUNDS, and that distinction is new.
 * The engine now accepts 25 ft to a mile, because the map-view panel's slider
 * offers that range by owner decision (see `ALERT_THRESHOLD_MIN_FT` in
 * `packages/core/src/alert.ts`). THIS control did not widen with it: its
 * picture is nineteen hand-transcribed detent rules in `settings.css`, and
 * deriving its stops from the engine's bounds would silently ask for a hundred
 * and six of them - a slider whose every value fell through to the `0`
 * fallback and pinned the knob to the floor. So the two ends stay pointed at
 * the band the design file actually draws, and a threshold set outside it from
 * the map panel arrives here as an uncovered value, which is exactly the case
 * {@link hasDetentRule} was written for: the number is shown, the position is
 * withheld.
 *
 * =============================================================================
 ...
 * GAP: settings / threshold-detent-rules-are-not-machine-checked
 */
...
export const THRESHOLD_MIN_LABEL = String(ALERT_THRESHOLD_DESIGN_MIN_FT);
export const THRESHOLD_MAX_LABEL = String(ALERT_THRESHOLD_DESIGN_MAX_FT);
...
function buildStops(): number[] {
  const stops: number[] = [];
  for (
    let ft = ALERT_THRESHOLD_DESIGN_MIN_FT;
    ft <= ALERT_THRESHOLD_DESIGN_MAX_FT;
    ft += ALERT_THRESHOLD_STEP_FT
  ) {
    stops.push(ft);
  }
  return stops;
