...
describe('the picker offers section 05, in section 05 order', () => {
  ...
  it('names each one as its card names it', () => {
    ...
    //
    // AND ONLY WHEN THE DEFAULT IS ONE OF THE SIX. This is a RETIRED picker
    // offering `V0_MODES`, and the default has twice now been a mode outside
    // that list - `slate` today. Asserting the badge unconditionally made the
    // name test fail for a reason that has nothing to do with names, which is
    // the failure its own comment above says it exists to avoid. The rule the
    // badge actually follows is asserted in full by the test above.
    if (byMode.has(DEFAULT_MODE)) {
