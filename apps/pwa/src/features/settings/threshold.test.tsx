...
describe('the stops are the bezel detents', () => {
  it('runs the design system slider end to end in bezel steps', () => {
    // THE DESIGN'S BAND, not the engine's, and the two are no longer the same
    // number. The engine accepts 25 ft to a mile because the map-view panel
    // offers it; this control still draws the 100..1000 the design file draws,
    // because `settings.css` has a detent rule for each of those and nothing
    // else. See the header of `threshold.ts`.
    expect(THRESHOLD_STOPS[0]).toBe(ALERT_THRESHOLD_DESIGN_MIN_FT);
    expect(THRESHOLD_STOPS.at(-1)).toBe(ALERT_THRESHOLD_DESIGN_MAX_FT);
...
describe('snapping', () => {
  ...
  it('says out loud that the engine ends are now outside this control', () => {
    // Not a defect, and worth an assertion so nobody "fixes" it: the snapper is
    // the ENGINE's and clamps to 25..5280, while this slider draws 100..1000.
    // Both ends of the engine's band therefore land on a value this stylesheet
    // has no detent rule for, and `ThresholdControl` withholds the knob rather
    // than drawing it at the floor. The measured number is still shown.
    expect(hasDetentRule(ALERT_THRESHOLD_MIN_FT)).toBe(false);
    expect(hasDetentRule(ALERT_THRESHOLD_MAX_FT)).toBe(false);
  });
});
