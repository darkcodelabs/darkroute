...
export const DOC_ENTRIES: readonly DocEntry[] = [
  ...
  {
    file: 'WHAT-DARKROUTE-IS.md',
    title: 'What DarkRoute is',
    sub: 'the whole thing, from the code that runs it, with diagrams',
  },
  {
  ...
  },
  {
    file: 'TERMS.md',
    title: 'Terms of use',
    sub: 'what you are agreeing to, with the ways the data is wrong stated first',
  },
  {
    file: 'SECURITY.md',
...
/**
 * WHERE TO READ THE SOURCE THIS BUILD CAME FROM.
 *
 * =============================================================================
 * THE BUG THIS FIXES: THE ONE LINK THAT HAD TO WORK, 404ED
 * =============================================================================
 * This pointed at `REPO_URL/commit/<sha>` using the build's own commit sha, and
 * every one of those links failed. Verified against the API: the private HEAD
 * sha returns 422 on the public repository, which holds exactly ONE commit.
 *
 * That is not a mistake in the URL, it is the publication model. The public
 * tree is produced by the public-seed builder as a single new ROOT commit - the
 * private history is deliberately never published, because it contains the
 * operational surface the seed exists to exclude. So there is no per-build
 * commit in the public repository for a link to resolve to, and there never
 * will be.
 *
 * A dead link here is worse than most dead links. This screen's entire argument
 * is "do not believe us, go and read it", and the button that says so was the
 * one thing on the page that did not work.
 *
 * =============================================================================
 * WHAT IT POINTS AT NOW
 * =============================================================================
 * The published tree, which always resolves, plus the build identifier printed
 * beside it so two builds can still be told apart. The copy says outright that
 * the tree is curated and single-commit rather than implying a history that is
 * not there - the honest version of a claim is the only version worth making on
 * a page about verifiability.
 */
export function commitUrl(): string | null {
  // Always resolvable, unlike a sha from a history that is not published.
  return REPO_URL;
}

/**
 * Whether this build can name itself at all.
 *
 * A development build has no stamped identifier, and saying so is better than
 * printing `dev` as though it were a version somebody could look up.
 */
export function buildIsStamped(): boolean {
  return BUILD.commitSha !== 'dev' && BUILD.commitSha !== 'nogit';
}

export const COMMIT_NOTE =
  'the published source, as a single curated tree - the private history is not in it, on purpose, ' +
  'because it holds the operational files the public tree exists to leave out. the build id above ' +
  'tells two builds apart; the tree is what you read.';
