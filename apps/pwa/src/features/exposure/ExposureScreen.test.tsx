...
describe('EXPOSURE counts passes, not log rows', () => {
  ...
  it('never shows an all-time count below the week it is sitting beside', () => {
    /*
     * `stores/history.ts` is explicit that null is not zero: the durable count
     * lives in IndexedDB and is null until a trip writes it -- which on a
     * phone that has only ever logged alerts is forever. The owner's own
     * screen read "29 this week" beside ALL TIME "—" (2026-09-09). The week's
     * passes are a floor the all-time figure cannot honestly sit below, so a
     * null counter beside one pass today prints 1. The dash survives only for
     * a phone with nothing logged at all, where it is a real "not known yet".
     */
    putArchive();
    act(() => {
    ...
    expect(allTime && within(allTime).getByText('1')).toBeInTheDocument();
    expect(allTime && within(allTime).queryByText(NO_VALUE)).toBeNull();
