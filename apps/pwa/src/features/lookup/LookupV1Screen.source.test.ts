...
describe('the v1 lookup screen', () => {
  it('NEVER reaches the network', () => {
    const code = [
      ...
      // The grouping. It only reshapes an array the search already produced,
      // which is exactly why it is the easy place to lose this: "just resolve
      // the proper street name for the header" is a one-line geocoder call in a
      // file nobody thinks of as the search.
      codeOf('features/lookup/streetGroups.ts'),
    ].join('\n');
