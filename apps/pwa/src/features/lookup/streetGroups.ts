...
/** "Flock" for "Flock Safety"; the tag itself for a maker the table has no short form for. */
export function shortMaker(maker: string): string {
  return MAKERS.get(maker) ?? maker;
}
...
export function contextLineOf(camera: CameraRecord): string {
  ...
  const parts: string[] = [];
  if (typeof maker === 'string' && maker !== '') parts.push(shortMaker(maker));
  if (typeof mount === 'string' && mount !== '') parts.push(MOUNTS.get(mount) ?? mount.replaceAll('_', ' '));
  /* THE ZONE IS ITS OWN FACT, not the mount's understudy. `traffic` is the
     87% default and says nothing; `parking`, `town`, `building` say where the
     lens is pointed, and a driver reading "Flock \u00b7 pole \u00b7 parking" knows it is
     a lot camera before the intel card opens. */
  if (typeof zone === 'string' && zone !== '' && zone !== 'traffic') parts.push(zone.replaceAll('_', ' '));
...
/**
 * The year the mapper says it went up, from `start_date`, or null.
 *
 * The tag is free text -- "2023", "2023-04", "2023-04-12" all occur -- and
 * only the leading four digits are trusted. A value that does not start with a
 * year prints nothing rather than something.
 */
export function installedYearOf(camera: CameraRecord): string | null {
  const raw = camera.tags?.['start_date'];
  if (typeof raw !== 'string') return null;
  const match = /^(\d{4})(?:\D|$)/.exec(raw.trim());
  return match?.[1] ?? null;
}

/*
 * THE ROW'S THIRD LINE: WHO, SINCE WHEN, AND WHICH RECORD. "Include more
 * details on each row. You have the room" -- owner, 2026-09-09. The operator
 * where the mapper recorded one, the plain fact where not, the year it went up
 * where one was written, and the id a driver quotes when a record is wrong --
 * back on the row now that it is no longer the only thing on it.
 */
export function detailLineOf(camera: CameraRecord): string {
  const operator = operatorOf(camera);
  const since = installedYearOf(camera);
  const parts = [operator ?? 'operator not recorded'];
  if (since !== null) parts.push(`since ${since}`);
  parts.push(camera.id);
  return parts.join(' \u00b7 ');
}
