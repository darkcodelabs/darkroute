...
describe('threshold span', () => {
  ...
  it('gives every threshold the driver can pick its own position', () => {
    ...
    // THE STOPS THE MAP-VIEW SLIDER OFFERS, which is what "the driver can pick"
    // means now. It used to be the nineteen bezel detents between 100 and 1000;
    // the band widened to 25 ft .. 1 mile by owner decision.
    for (const ft of ALERT_THRESHOLD_STOPS_FT) {
      spans.add(corridorThresholdSpan(corridor, ft));
    }
    // They need not all differ -- half a percent of three miles is 79 ft and
    // the closest stops are 25 apart -- but "more than a handful" is the
  ...
  it('moves outward as the threshold grows, never inward', () => {
    ...
    const far = corridorThresholdSpan(corridor, ALERT_THRESHOLD_DESIGN_MAX_FT);
    expect(near).not.toBeNull();
    expect(far).not.toBeNull();
    expect(far as number).toBeGreaterThan(near as number);
  });

  it('draws no band at all for a mile, rather than pinning one at a tenth', () => {
    // A tenth of the three-mile ladder is 1584 ft, and the slider now offers
    // the half mile and the mile. Those get NOTHING: a mark reading "your alert
    // zone ends here" at 1584 ft, for a driver who set 5280, is a wrong
    // measurement drawn as an instrument. See `corridorThresholdSpan`.
    const corridor = corridorAt(CORRIDOR_RANGE_FT);
    expect(corridorThresholdSpan(corridor, ALERT_THRESHOLD_MAX_FT)).toBeNull();
    expect(corridorThresholdSpan(corridor, 2640)).toBeNull();
    // The quarter mile still fits, so the widening did not empty the control.
    expect(corridorThresholdSpan(corridor, 1320)).not.toBeNull();
  });
