 ...
 * outside the range being drawn.
 *
 * IT HAPPENS AT THE SHIPPED RANGE NOW. This said it could not, and that was
 * true while the threshold stopped at 1000 ft: a tenth of the three-mile
 * ladder is 1584 ft, so every legal value fitted. The map-view slider offers
 * the quarter mile, the half and the mile (owner decision, see
 * `ALERT_THRESHOLD_STOPS_FT`), and the top two of those are past the tenth. A
 * driver who sets half a mile gets NO band on this ladder rather than a band
 * pinned at 1584 ft, which is the right half of the trade to keep: the ladder
 * is three miles of road and a mark that says "your alert zone ends here" in
 * the wrong place is worse than one that says nothing.
 */
