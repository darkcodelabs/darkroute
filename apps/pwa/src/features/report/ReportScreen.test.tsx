...
import { DETAIL_LABEL, PRIVACY_NOTE, ReportViewV1 } from './components/ReportViewV1.tsx';
/*
 * THE PHOTO CONSTANTS MOVED WITH THE PHOTOGRAPH.
 *
 * Brief 4 pushes the heading arc, the mount chips, the photo and the make and
 * model behind `Add detail`, so the labels a driver presses for them are
 * declared beside the sheet that draws them. Same strings, same component
 * tree, one tap further in - which is what `openDetail()` below is for.
 */
import {
  PHOTO_ADD_LABEL,
  PHOTO_ISSUE,
  ...
  PHOTO_REMOVE_LABEL,
  photoFacts,
} from './components/ReportDetailV1.tsx';
...
/**
 * OPEN `Add detail`, which is where four of this sheet's five controls now are.
 *
 * The modal a driver gets is position + side + submit; the heading arc, the
 * mount chips, the photograph, the make and model and the how-far-over chips
 * are one level under it. The container stays mounted while the sheet is open -
 * that is the whole reason the sheet is a branch of the view rather than a
 * second overlay - so the draft, the queue and the submit key are all still
 * live and still reachable by their own labels.
 */
async function openDetail(): Promise<void> {
  await tap(screen.getByRole('button', { name: new RegExp(DETAIL_LABEL) }));
}
...
async function mountWithCamera(
  ...
  // AND OPENS `Add detail`, because that is where the camera is now. Every test
  // in this file that mounts with a camera is about the photograph, and none of
  // them is about the row that reveals it - which is covered on its own below.
  await openDetail();
}
 ...
 * view. Three separate defects have shipped from a control that exists only in
 * the view the build does NOT render, so these press what a driver presses.
 *
 * =============================================================================
 * THE NEW CAMERA / CONFIRM ONE SWITCH IS GONE, AND THAT IS WHAT THESE ASSERT
 * =============================================================================
 * Brief 4 deletes it: "Confirming an existing camera is what `Still there` in
 * the camera modal already does. It does not need a second entry point." The
 * three tests that pressed it - reaching CONFIRM, filing a `confirm_existing`
 * payload, and reading the note that said why the tab was dead - went with the
 * control, and are replaced by the three below: the switch is not drawn, a
 * report filed from this sheet is a NEW camera carrying no nearby id, and the
 * controls the brief moved are exactly one tap away.
 *
 * `withMode` and v0's `ModeToggle` are untouched. The mode still exists in the
 * draft; nothing on the shipped sheet sets it any more.
 */
describe('ReportViewV1 - the routed view', () => {
  it('draws no mode switch at all, with a camera in range', async () => {
    lock();
    ingestAlertTick(tick()); // FWM-0442, in range, which is when the tab used to light
    await mountV1();

    // By ROLE rather than by label: the point is that there is no radio group
    // on this sheet whatever it might have been called.
    expect(screen.queryAllByRole('radio')).toHaveLength(0);
    // And the sheet does not claim a subject either. `cameraId` is null in new
    // mode whatever is nearby, and new mode is now the only mode.
    expect(document.querySelector('.fwm-reportv1')).not.toHaveAttribute(
      'data-fwm-report-camera',
    );
  });

  it('files a NEW camera, carrying no nearby id, even with one in range', async () => {
    // The behaviour the deleted switch used to gate. A report from this sheet
    // is about a camera the map does not have; confirming one the map DOES have
    // is `Still there` on the camera modal, which writes `confirm_camera`
    // through the same queue from the card that shows which camera it means.
    lock();
    ingestAlertTick(tick());
    await mountV1();
  ...
  it('keeps the one-tap path on the modal and everything else one tap under it', async () => {
    // The shape of the redesign, asserted once: position, side and submit are
    // on the card a driver gets, and the four detail controls are not - until
    // the row is pressed, at which point they are, with the draft still alive
    // because the container never unmounted.
    lock();
    await mountV1();

    expect(screen.getByRole('group', { name: 'which side of the road' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'SUBMIT REPORT' })).toBeInTheDocument();
    expect(screen.queryByRole('slider', { name: 'camera facing' })).toBeNull();
    expect(screen.queryByRole('button', { name: PHOTO_ADD_LABEL })).toBeNull();

    await openDetail();

    expect(screen.getByRole('slider', { name: 'camera facing' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: PHOTO_ADD_LABEL })).toBeInTheDocument();
    // Still one sheet, still one container: the key that files the report is
    // where it was, and pressing it still works from here.
    expect(screen.getByRole('button', { name: 'SUBMIT REPORT' })).toBeEnabled();
  });
});
...
describe('the OSM path, from the sheet a driver actually gets', () => {
  ...
  it('is OPEN once they press a side and an offset, which is what the chips are for', async () => {
    ...
    // The exact controls that were unreachable. If either half stops being
    // rendered, these queries fail and this test says why.
    // Buttons in a role="group", not radios: `role="radio"` cannot express
    // "none of them", and none-of-them is the state the sheet opens in.
    await tap(screen.getByRole('button', { name: 'RIGHT' }));
    // THE SECOND HALF IS ONE TAP UNDER THE MODAL. Brief 4 keeps the side on the
    // one-tap path and puts how-far-over behind `Add detail` with the rest of
    // the detail. Both halves still reach `projectSubject`, which is what this
    // test is about.
    await openDetail();
    await tap(screen.getByRole('button', { name: 'ONE LANE OVER' }));
  ...
  it('never asks how far over for a camera the driver said was overhead', async () => {
    ...
    // questions that have an answer - and `ReportScreen` synthesises the 15ft
    // that makes OVERHEAD complete on its own, which is why this one publishes
    // from the one-tap path and the two lateral sides do not.
    lock();
    ingestAlertTick(tick());
    await mountV1();
    ...
    expect(screen.queryByRole('group', { name: 'how far over' })).toBeNull();
    expect(screen.queryByRole('button', { name: 'ONE LANE OVER' })).toBeNull();
  ...
  it('LEAVES A LATERAL ONE-TAP REPORT UNPUBLISHABLE, which is a data decision nobody has made', async () => {
    /*
     * FLAGGED, NOT PAPERED OVER.
     *
     * Brief 4's one-tap path is position + side + submit. For LEFT and RIGHT
     * that is not enough: `ReportScreen` resolves the subject choice to null
     * without an offset, `projectSubject` returns null, and `osmBlocker` says
     * `no-subject-position`. The report signs, queues and can never reach
     * OpenStreetMap.
     *
     * Defaulting an offset for the lateral sides would fix it in one line and
     * would assert a distance nobody stated, on a record whose entire purpose
     * is to say where a camera is. That is a data call, so this test pins the
     * CURRENT behaviour rather than inventing the answer - and it is the thing
     * to delete when somebody makes it.
     */
    lock();
    ingestAlertTick(tick());
    await mountV1();

    await tap(screen.getByRole('button', { name: 'LEFT' }));
    await tap(screen.getByRole('button', { name: 'SUBMIT REPORT' }));
    ...
    // AND THE SHEET SAYS SO, one level down, where the missing half is. This is
    // the only place the product admits that a report which filed perfectly
    // well cannot be published.
    await openDetail();
    expect(document.querySelector('[data-fwm-report-where]')).toHaveAttribute(
      'data-fwm-report-where',
      'unset',
    );
  });
});
