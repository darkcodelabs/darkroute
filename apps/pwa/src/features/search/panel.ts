 ...
 * having one.
 * GAP: search-panel / c-nobody-can-compute-the-camera-counts-yet
 */
 ...
 * a typographic aside. Recorded in gap record search-panel: the spec
 ...
 * every row they contain, is recorded in gap record search-panel.
 */
...
/**
 * `'4.2 mi · Leawood, KS'` -- state 2's map result, which keeps its locality
 * (two identical street names have to be tellable apart) and carries the
 * distance when it is known. The spec draws both forms, one row apart, with
 * the distance AFTER the locality; it is drawn FIRST here on purpose.
 *
 * A geocoder's detail is a whole address -- "11601, East US Highway 40,
 * Kansas City, Jackson County, Missouri" -- and the row clips it with an
 * ellipsis. With the distance at the end it was the distance that got
 * clipped: every Walmart in Kansas City drew its count and not one of them
 * said how far it was ("this is also missing how far away this search is"
 * -- owner, 2026-09-09). Leading with it means the one fact that changes
 * which row the driver picks survives the clip on every width.
 */
export function placeSub(detail: string, estimate: RowEstimate | null): string {
  if (estimate === null) return detail;
  return `${estimate.miles.toFixed(1)} mi · ${detail}`;
}
