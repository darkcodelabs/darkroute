 ...
 */
/**
 * SET HOME / SET WORK FROM ANY PLACE, not only from a recent.
 *
 * First run's two keys are "Set home" and "Set work", and on a first run
 * there are no recents to promote -- the place is one the driver has just
 * typed and picked. Same rule as `promoteToSaved` about one home and one
 * work, and the same lift out of the recents if the place is already there.
 */
export function saveAsKind(
  book: PlaceBook,
  place: { readonly name: string; readonly detail: string; readonly lat: number; readonly lon: number },
  kind: SavedKind,
): PlaceBook {
  const id = placeKey(place);
  const saved: SavedPlace = { id, kind, name: place.name, detail: place.detail, lat: place.lat, lon: place.lon };
