...
export function DriveScreen(): ReactElement {
   ...
   */
  const requestRoute = useCallback((): void => {
    /* The body moved to `routeActions.planFromHere` so the intel card could
       plan too; it reads the fix and the archive off their stores at the press,
       which is the same pair this closure used to capture. */
          ...
          <div className="fwm-drive-top">
            <TopBar
              ...
              onDestination={(place) => {
                ...
                setPanned(false);
                /* AND BUILD THE DARK ROUTE. Choosing a destination is the
                   press that authorises it - a person who typed a place and
                   picked it off the list has asked to go there, and the route
                   this application draws is the one with the readers taken out. */
                requestRoute();
              }}
